...
void calc_statistic(std::ofstream & ofp, int * observation, float N)
{
    ...
    for(i = 0; i < 100; i++)
    {
        ...
        if(observation[i])
        {
            ...
            varians += vartmp;
            ofp << "| " << std::setw(3) << i << " | " << std::setw(5) <<
            observation[i] << " | " << std::setprecision(3) << std::setw(5) <<
            frekvens << " | " << std::setw(6) << x * frekvens << " | " <<
            std::setw(12) << vartmp << " | " << std::endl;
        }
    }
    ofp << "|-----+-------+-------+--------+--------------|" << std::endl <<
    "| sum | " << std::setprecision(0) << std::setw(5) << N << " | " <<
    std::setprecision(3) << std::setw(5) << sumfrekvens << " | " << std::setw(6) <<
    mean << " | " << std::setw(12) << varians << " |" << std::endl <<
    "`---------------------------------------------'" << std::endl << std::endl <<
    "Mean: " << std::setprecision(1) << mean << std::endl << "Variance = " << varians <<
    std::endl << "Standard deviation = " << sqrt(varians) << std::endl << std::endl;
} // calc_statistic

#endif // ifdef GOLDNODE_STATS
//  ------------------------------------------------------------------
...
static void read_nodelists()
{
    ...
    if(not quiet)
    {
        std::cout << std::endl << "* Compiling nodelists:" << std::endl;
    }
    ...
    // Compile nodelists
    for(realfno = 0, fno = nodelist.begin(), zno = nodezone.begin();
        fno != nodelist.end(); fno++, zno++)
    {
        if(nodes < maxnodes)
        {
            ...
            if(lfp.isopen())
            {
                ...
                while(lfp.Fgets(buf, sizeof(buf)))
                {
                                        ...
                                        '\x1A'))
                    {
                        ...
                        if(not quiet)
                        {
                            ...
                            std::cout << "\r* |--" << name << std::setw(
                                (len >
                                 0) ? len : 1) << " " << "Warning line " << line <<
                            " - Invalid NUL char encountered." << std::endl;
                        }
                    ...
                    if(*ptr != ';' and * ptr)
                    {
                        ...
                        if(ISTWIRLY(no))
                        {
                            ...
                            std::cout << "\r* \\--" << name << std::setw(
                                (len >
                                 0) ? len : 1) << " " << "Zone " << nlst.addr.zone <<
                            "   \tNet " << nlst.addr.net << "   \tNodes " <<
                            (uint32_t)no << "        " << std::flush;
                        }

                        bool include = true;

                        // Check address against the exclude masks
                        for(addr_iter n = excludenode.begin(); n != excludenode.end();
                            n++)
                        {
                        ...
                        {
                            for(addr_iter n =
                                    includenode.begin(); n != includenode.end();
                                n++)
                            {
                                if(match_addr_mask(&(*n), &nlst.addr))
                                {
                ...
                if(not quiet)
                {
                    ...
                    ((fno ==
                      nodelist.end() - 1) ? '\\' : '|') << "--" << name <<
                    std::setw((len >
                               0) ? len : 1) << " " << "Nodes read: " << (uint32_t)no <<
                    "\tTotal read: " << (uint32_t)nodes <<
                    ((nodes >=
                      maxnodes) ? " (Limit reached)" : "                ") << std::endl;
                }
            ...
            else
            {
                if(not quiet)
                {
                    std::cout << "Error opening nodelist " << fno->fn << '!' <<
                    std::endl;
                }
    ...
    if(userlist.size())
    {
        if(not quiet)
        {
            std::cout << std::endl << "* Compiling userlists:" << std::endl;
        }
    }

    pos = 0;

    for(fno = userlist.begin(), zno = userzone.begin();
        fno != userlist.end() and nodes < maxnodes; fno++, zno++)
    {
        no = 0;
            ...
            if(nodes < maxnodes)
            {
                while(lfp.Fgets(buf, sizeof(buf)))
                {
                    ...
                    *ptr = NUL;
                    ...
                    if(not include)
                    {
                        for(addr_iter n = includenode.begin(); n != includenode.end();
                            n++)
                        {
                            if(match_addr_mask(&(*n), &nlst.addr))
                            {
        ...
        else
        {
            if(not quiet)
            {
                std::cout << "Error opening userlist " << fno->fn << '!' << std::endl;
            }
        }
    }
    ...
    if(make_stats)
    {
        if(not quiet)
        {
            std::cout << "* Writing statistics to " << statfilename << NL;
        }
        ...
        if(not ofp)
        {
            if(not quiet)
            {
                std::cout << "Error opening statfile " << statfilename << '!' << NL;
            }
        }
        else
        {
    ...
    if(not quiet)
    {
        std::cout << NL << "* Sorting by name " << std::flush;
    }
    ...
    if(fp.isopen())
    {
        ...
        if(fidouser)
        {
            fido.Fopen(fidouserlst, "wt", sh_mod);
        }
        ...
        if(!fido.isopen())
        {
            if(not quiet)
            {
                std::cout << "\b, writing " << name << ' ' << std::flush;
            }
        ...
        else
        {
            if(not quiet)
            {
                std::cout << "\b, writing " << name << " and " << fidouserlst << ' ' <<
                std::flush;
            }
        }
        ...
        for(prev = nodeidx.end(), curr = nodeidx.begin(); curr != nodeidx.end();
            prev = curr++)
        {
            if(ISTWIRLY(nn++))
            {
            ...
            if(ignoredups)
            {
                if(prev != nodeidx.end() && match_addr_mask(&curr->addr, &prev->addr))
                {
                    if(strieql(curr->name, prev->name))
                    {
                        ...
                        if(not quiet)
                        {
                            std::cout << "* Dupe: " << curr->addr.zone << ':' <<
                            curr->addr.net << '/' << curr->addr.node << '.' <<
                            curr->addr.point << ' ' << curr->name << NL;
                        }
    ...
    if(not quiet)
    {
        std::cout << ' ' << NL "* Sorting by node " << std::flush;
    }
    ...
    if(fp.isopen())
    {
        ...
        if(not quiet)
        {
            std::cout << ' ' << NL "* Writing " << name << std::endl;
        }
    ...
    if(not quiet)
    {
        ...
        std::cout << NL "* Nodelist compile completed. Compile time: " <<
        (uint32_t)(runtime / 60) << " min, " << (uint32_t)(runtime % 60) << " sec." <<
        NL;
    }
...
static void check_nodelists(bool force)
{
    ...
    if(not quiet)
    {
        if(compilen)
        {
            std::cout << "* " << compilen << " new nodelist file" <<
            ((compilen == 1) ? "" : "s") << " found." NL;
        }
        else if(nodelist.size())
        {
            std::cout << "* The nodelist file" <<
            ((nodelist.size() == 1) ? " is" : "s are") << " up-to-date." NL;
        }
    }
    ...
    if(not quiet)
    {
        if(compileu)
        {
            std::cout << "* " << compileu << " new userlist file" <<
            ((compileu == 1) ? "" : "s") << " found." NL;
        }
        else if(userlist.size())
        {
            std::cout << "* The userlist file" <<
            ((userlist.size() == 1) ? " is" : "s are") << " up-to-date." NL;
        }
    }
                ...
                switch(crc)
                {
                    case CRC_IF:
                        ...
                        if(in_if)
                        {
                            if(not quiet)
                            {
                                std::cout << "* " << __configfile <<
                                ": Misplaced IF at line " << line <<
                                ". IF's cannot be nested." << NL;
                            }
                        }
                    ...
                    case CRC_ELSEIF:
                        ...
                        if((not in_if) or in_else)
                        {
                            ...
                            {
                                std::cout << "* " << __configfile <<
                                ": Misplaced ELIF/ELSEIF at line " << line << '.' << NL;
                            }
                        }
                    ...
                    case CRC_ELSE:
                        ...
                        if((not in_if) or in_else)
                        {
                            ...
                            {
                                std::cout << "* " << __configfile <<
                                "Misplaced ELSE at line " << line << '.' << NL;
                            }
                        }
                    ...
                    case CRC_ENDIF:
                        ...
                        if(not in_if)
                        {
                            ...
                            {
                                std::cout << "* " << __configfile <<
                                ": Misplaced ENDIF at line " << line << '.' << NL;
                            }
                        }
                ...
                if((not _gotcond) and cond_status)
                {
                    switch(crc)
                    {
                        ...
                        case CRC_INCLUDE:
                            ...
                            if(not parse_config(value, zoneaddr)) // NOTE! This is a recursive
                                                                  // call!
                            {
                                if(not quiet)
                                {
                                    std::cout <<
                                    "* Could not read configuration file " << value <<
                                    '!' <<
                                    NL;
                                }
                            }
...
static void run_gn(int argc, char * argv[])
{
    ...
    if(not quiet)
    {
        std::cout << __gver_name__ << " - Nodelist Compiler for Golded+ v." <<
        __gver_shortver__ << __gver_platform__ << __gver_postversion__ << "." NL <<
    ...
    if(not (force or conditional))
    {
        if(not quiet)
        {
            std::cout << "Commandline syntax: " << CleanFilename(argv[0]) <<
            " [-options] [configfile]" NL <<
    ...
    else
    {
        ...
        else
        {
            if(not quiet)
            {
                std::cout << NL "Could not find the configuration file!" NL;
            }
        }
    }
} // run_gn
