...
int SMBCALL smb_open(smb_t * smb)
{
    ...
    if(filelength(file) >= sizeof(smbhdr_t))
    {
        ...
        {
            sprintf(smb->last_error, "corrupt SMB header ID: %.*s", LEN_HEADER_ID,
                    hdr.id);
            smb_close(smb);
            return -2;
        }
        ...
        if(fread(&(smb->status), 1, sizeof(smbstatus_t),
                 smb->shd_fp) != sizeof(smbstatus_t))
        {
    ...
    while(1)
    {
        ...
        else if(gtime(NULL) - start >= smb->retry_time)
        {
            sprintf(smb->last_error,
                    "timeout opening %s (retry_time=%d)",
                    str,
                    smb->retry_time);
            return -2;
        }
...
int SMBCALL smb_trunchdr(smb_t * smb)
{
    ...
    while(1)
    {
        ...
        {
            sprintf(smb->last_error,
                    "timeout changing header file size (retry_time=%d)",
                    smb->retry_time);
            return -2;
        }
...
int SMBCALL smb_getmsghdr(smb_t * smb, smbmsg_t * msg)
{
    ...
    if(msg->hdr.total_dfields &&
       (msg->dfield = (dfield_t *)MALLOC(sizeof(dfield_t) * msg->hdr.total_dfields)) ==
       NULL)
    {
        smb_freemsgmem(msg);
        sprintf(smb->last_error,
                "malloc failure of %d bytes for %d data fields",
                (int)sizeof(dfield_t) * msg->hdr.total_dfields,
    ...
    while(l < msg->hdr.length)
    {
        ...
        if((vp = (hfield_t *)REALLOC(msg->hfield,
                                     sizeof(hfield_t) * (i + 1))) == NULL)
        {
            smb_freemsgmem(msg);
            sprintf(smb->last_error, "realloc failure of %d bytes for header fields",
        ...
        if((msg->hfield_dat[i] = (char *)MALLOC(msg->hfield[i].length + 1)) == NULL) /* Allocate
                                                                                        1 extra
                                                                                        for NULL
                                                                                        terminator
                                                                                     */
        ...
        if(msg->hfield[i].length && !fread(msg->hfield_dat[i],
                                           msg->hfield[i].length,
                                           1,
                                           smb->shd_fp))
        {
...
int SMBCALL smb_copymsgmem(smbmsg_t * msg, smbmsg_t * srcmsg)
{
    ...
    if((msg->dfield = (dfield_t *)MALLOC(msg->hdr.total_dfields * sizeof(dfield_t))) ==
       NULL)
    {
        return 1;
    }
...
int SMBCALL smb_addcrc(smb_t * smb, uint32_t crc)
{
    ...
    {
        close(file);
        sprintf(smb->last_error, "malloc failure of %d bytes", smb->status.max_crcs * 4);
        return -3;
    }
    ...
    if((uint32_t)length >= smb->status.max_crcs * 4L) /* Reached or exceeds max crcs */
    {
        ...
        if(l < smb->status.max_crcs)                /* Dupe CRC found */
        {
            close(file);
            FREE(buf);
            sprintf(smb->last_error, "duplicate message detected");
            return 1;
        }
...
/****************************************************************************/
int SMBCALL smb_freemsgdat(smb_t * smb,
                           uint32_t offset,
                           uint32_t length,
                           uint16_t headers)
{
    ...
    for(l = 0; l < blocks; l++)
    {
        ...
        {
            sprintf(smb->last_error, "seeking to %ld of allocation file",
