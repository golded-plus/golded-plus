...
#include <gmosqsh.h>
//  ------------------------------------------------------------------
SqshHdr::SqshHdr() : attr(0), orig(), dest(), utc_offset(0), replyto(0), umsgid(0)
{
    from[0] = NUL;
    to[0]   = NUL;
    subj[0] = NUL;
...
//  ------------------------------------------------------------------
void SquishInit(const char * userpath,
                int userno,
                int direct,
                int recycle,
                int squishscan)
{
    ...
    {
        squishwide->user->gufh = ::sopen(userfile,
