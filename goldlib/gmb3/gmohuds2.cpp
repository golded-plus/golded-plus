                                        ...
                                        __HUDSON>::refresh()
{
    ...
    msgidxptr  =
        (HudsIdx *)throw_realloc(msgidxptr, (uint)(msgidxsize + sizeof(HudsIdx)));
    // Load MSGIDX.BBS/DAT
    fhidx.LseekSet(0);
                                        ...
                                        __HUDSON>::lock()
{
    ...
    if(not islocked and WideCanLock)
    {
        ...
        {
            // Tell the world
            if(PopupLocked(++_tries, true,
                           AddPath(path,
                                   __HUDSON ? "msginfo" HUDS_EXT : "msginfo" GOLD_EXT))
               == false)
            {
                // User requested to exit
                WideLog->ErrLock();
                                        ...
                                        __HUDSON>::scan()
{
    ...
    throw_free(scn);
    scn =
        (HudsScan *)throw_calloc((__HUDSON ? HUDS_MAXBOARD : GOLD_MAXBOARD),
                                 sizeof(HudsScan));
            ...
            if(_idxboard and (_idxboard <= (__HUDSON ? HUDS_MAXBOARD : GOLD_MAXBOARD)))
            {
                ...
                if((_scan->lastmsgno >= _scan->lastread) and (_scan->lastreadreln == 0))
                {
                    ...
                    _scan->lastreadreln  = _scan->count -
                                           (_scan->lastmsgno != _scan->lastread ? 1 : 0);
                }
            }
            else
            {
...
{
    pmscan =
        (HudsIdx *)throw_realloc(pmscan, (pmscantotal + PMSCANBUFSZ) * sizeof(HudsIdx));
}
