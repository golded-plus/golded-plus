...
template <class msgn_t, class rec_t, class attr_t, class board_t, class last_t,
          bool __HUDSON> void _HudsWide<msgn_t, rec_t, attr_t, board_t, last_t,
                                        __HUDSON>::test_open(gfile & __file,
                                                             const char * __fname,
                                                             int __oaccess)
{
    GFTRK("HudsTestOpen");
    ...
    do
    {
        ...
        if(!__file.isopen())
        {
            ...
            if(PopupLocked(++_tries, false, _testfn) == false)
            {
                ...
                WideLog->ErrOpen();
                raw_close();
                WideLog->printf("! A %s msgbase file could not be opened.",
...
template <class msgn_t, class rec_t, class attr_t, class board_t, class last_t,
          bool __HUDSON> void _HudsWide<msgn_t, rec_t, attr_t, board_t, last_t,
                                        __HUDSON>::raw_open(int __oaccess, int __all)
{
    GFTRK("HudsRawOpen");
                                        ...
                                        __HUDSON>::refresh()
{
    ...
    msgidxptr  = (HudsIdx *)throw_realloc(msgidxptr,
                                          (uint)(msgidxsize + sizeof(HudsIdx)));
    // Load MSGIDX.BBS/DAT
    fhidx.LseekSet(0);
                                        ...
                                        __HUDSON>::lock()
{
    ...
    if(not islocked and WideCanLock)
    {
        ...
        {
            // Tell the world
            if(PopupLocked(++_tries, true, AddPath(path,
                                                   __HUDSON ? "msginfo" HUDS_EXT :
                                                   "msginfo" GOLD_EXT)) == false)
            {
                // User requested to exit
                WideLog->ErrLock();
                raw_close();
                WideLog->printf("! A %s msgbase file could not be locked.",
                                __HUDSON ? HUDS_NAME : GOLD_NAME);
                                        ...
                                        __HUDSON>::unlock()
{
    ...
    {
        fhinf.Unlock(sizeof(HudsInfo) + 1,
                     1);
        islocked = false;
    }
                                        ...
                                        __HUDSON>::scan()
{
    ...
    // Alloc scan array
    throw_free(scn);
    scn = (HudsScan *)throw_calloc((__HUDSON ? HUDS_MAXBOARD : GOLD_MAXBOARD),
                                   sizeof(HudsScan));
        ...
        if(_msgidx_ptr->msgno != (__HUDSON ? HUDS_DELETEDMSGNO : GOLD_DELETEDMSGNO))
        {
            ...
            if(_idxboard and (_idxboard <= (__HUDSON ? HUDS_MAXBOARD : GOLD_MAXBOARD)))
            {
                ...
                if((_scan->lastmsgno >= _scan->lastread) and (_scan->lastreadreln == 0))
                {
                    ...
                    _scan->lastreadreln  = _scan->count - (_scan->lastmsgno !=
                                                           _scan->lastread ? 1 : 0);
                }
            }
            else
    ...
    if(_invalidboards)
    {
        WideLog->printf("! Found %u msgs with an invalid board number (0 or >%u).",
                        _invalidboards,
                        (__HUDSON ? HUDS_MAXBOARD : GOLD_MAXBOARD));
        WideLog->printf("! In the %s msgbase at %s.",
                        __HUDSON ? HUDS_NAME : GOLD_NAME,
                        path);
    ...
    while(_board < (__HUDSON ? HUDS_MAXBOARD : GOLD_MAXBOARD))
    {
        ...
        {
            WideLog->printf("! Counted %u active msgs in %s board %u.",
                            _scan->count,
                            __HUDSON ? HUDS_NAME : GOLD_NAME,
                            _board + 1);
            WideLog->printf("! According to msginfo%s there should be %u active msgs.",
                            __HUDSON ? HUDS_EXT : GOLD_EXT,
                            _scan->active);
            WideLog->printf(": Info: A program did not update msginfo%s correctly.",
                            __HUDSON ? HUDS_EXT : GOLD_EXT);
...
template <class msgn_t, class rec_t, class attr_t, class board_t, class last_t,
          bool __HUDSON> void _HudsArea<msgn_t, rec_t, attr_t, board_t, last_t,
                                        __HUDSON>::raw_scan(int __keep_index)
{
    GFTRK("HudsRawScan");
    ...
    else
    {
        ...
        while(_msgidx_count < _msgidx_total)
        {
            ...
            if((_msgidx_ptr->board == _board) and (_msgidx_ptr->msgno !=
                                                   (__HUDSON ? HUDS_DELETEDMSGNO :
                                                    GOLD_DELETEDMSGNO)))
            {
                // Get message number
        ...
        {
            WideLog->printf("! Counted %u active msgs in %s board %u (%s).",
                            _msg_count,
                            __HUDSON ? HUDS_NAME : GOLD_NAME,
                            board(),
                            echoid());
...
#define HudsIdxCmp(a, \
                   b) ((a.board - b.board != \
                        0) ? a.board - b.board : ((int)(a.msgno - b.msgno)))
//  ------------------------------------------------------------------
...
template <class msgn_t, class rec_t, class attr_t, class board_t, class last_t,
          bool __HUDSON> void _HudsWide<msgn_t, rec_t, attr_t, board_t, last_t,
                                        __HUDSON>::realloc_pm_scan()
{
    pmscan = (HudsIdx *)throw_realloc(pmscan,
                                      (pmscantotal + PMSCANBUFSZ) * sizeof(HudsIdx));
}
                                        ...
                                        __HUDSON>::scan_pm()
{
    ...
    for(uint k = pmscantotal >> 1; k; k >>= 1)
    {
        ...
        {
            for(uint j = i - k; (j >= 0) and HudsIdxCmp(pmscan[j], pmscan[j + k]) > 0;
                j -= k)
            {
                HudsIdx e = pmscan[j];
                pmscan[j]     = pmscan[j + k];
