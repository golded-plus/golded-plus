...
void EzycomArea::save_message(int __mode, gmsg * __msg, EzycHdr & __hdr)
{
    ...
    _fh = test_open(AddPath(wide->msgbasepath, "MSGEXPRT.BBS"),
    ...
    _fh = test_open(AddPath(wide->msgbasepath, "MSGFAST.BBS"),
                    O_WRONLY | O_CREAT | O_BINARY,
                    SH_DENYWR);
    ...
    ::close(_fh);
    // Update MSGCOUNT.BBS
    _fh = test_open(AddPath(wide->msgbasepath, "MSGCOUNT.BBS"),
                    O_WRONLY | O_CREAT | O_BINARY,
                    SH_DENYNO);
    ...
    if(WideCanLock)
    {
        ...
        while(::lock(_fh, (board() - 1) * 2, 2) == -1)
        {
            ...
            if(PopupLocked(++_tries, true,
                           AddPath(wide->msgbasepath, "MSGCOUNT.BBS")) == false)
            {
