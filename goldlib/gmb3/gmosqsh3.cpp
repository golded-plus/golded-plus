...
int SquishArea::load_message(int __mode, gmsg * __msg, SqshHdr & __hdr)
{
    ...
    if(rwresult != sizeof(SqshFrm))
    {
        if(rwresult < 0)
        {
            WideLog->printf("! SquishArea::load_message: data file read error \"%s\"",
                            strerror(errno));
        }
    ...
    {
        WideLog->printf(
            "! SquishArea::load_message: reln=%d frame type=%d is not normal",
            _reln,
            (int)_frm.type);
        GFTRK(0);
        return false;
    ...
    if(rwresult != sizeof(SqshHdr))
    {
        ...
        {
            WideLog->printf(
                "! SquishArea::load_message: can't read Squish message header");
        }
        ...
        if(_frm.ctlsize + _frm.totsize >= sizeof(SqshHdr))
        {
            ...
            *_src    = NUL;
            ...
            if(rwresult != _frm.ctlsize)
            {
                ...
                {
                    WideLog->printf(
                        "! SquishArea::load_message: data file read error \"%s\"",
                        strerror(errno));
                }
                ...
                {
                    WideLog->printf(
                        "! SquishArea::load_message: can't read Squish message kludges");
                }

                WideLog->printf(": Info: Your msgbase is corrupted.");
                WideLog->printf(
                    "+ Advice: Run a msgbase index rebuild/recover utility.");
            ...
            while(*_src and (*_src == CTRL_A) and _src[1])
            {
                ...
                if(not ((*(dword *)(_src + 1) == CHR4_AREA) and strneql(_src + 1,
                                                                        "AREA:",
                                                                        5)))
                {
                    *_dest++ = *_src;
            ...
            *_dest = NUL;
            ...
            if(rwresult != _txtlen)
            {
                ...
                {
                    WideLog->printf(
                        "! SquishArea::load_message: can't read Squish message text");
                }
