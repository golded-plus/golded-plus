...
void PcbArea::save_message(int __mode, gmsg * __msg, PcbHdr & __hdr)
{
    ...
    memcpy(__hdr.date, strftimei(_dtbuf,
                                 9,
                                 __msg->attr.uns() ? "%d-%m-%y" : "%d-%m\xC4%y",
                                 &_tm), 8);
    ...
    if(__msg->link.first())
    {
        ...
        __hdr.replydate = L2B((10000L * _year) + (100L * (_tm.tm_mon + 1)) +
                              _tm.tm_mday);
    ...
    if(__mode & GMSG_TXT)
    {
        ...
        __hdr.blocks = (byte)(1 + (__msg->txtlength / 128) +
                              ((__msg->txtlength % 128) ? 1 : 0));
