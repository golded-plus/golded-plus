...
void SMBArea::open()
{
    ...
    if(isopen == 1)
    {
        ...
        for( ; ; )
        {
            ...
            {
                if((errno !=
                    EACCES) or (PopupLocked(++_tries, false, data->file) == false))
                {
...
int SMBArea::load_hdr(gmsg * __msg, smbmsg_t * smsg)
{
    ...
    if(not fread(&smsgp->idx, 1, sizeof(idxrec_t),
                 data->sid_fp) or (smb_lockmsghdr(data, smsgp) != 0))
    {
        GFTRK(0);
        return false;
    }
...
int SMBArea::load_msg(gmsg * msg)
{
    ...
    for(i = 0; i < smsg.hdr.total_dfields; i++)
    {
        switch(smsg.dfield[i].type)
        {
            ...
            common: fseek(data->sdt_fp, smsg.hdr.offset + smsg.dfield[i].offset,
                          SEEK_SET);
                l = sizeof(xlat);
...
void SMBArea::save_hdr(int mode, gmsg * msg)
{
    ...
    if(not (mode & GMSG_NEW))
    {
        ...
        if(not fread(&smsg.idx, 1, sizeof(idxrec_t),
                     data->sid_fp) or (smb_lockmsghdr(data, &smsg) != 0))
        {
            GFTRK(0);
            return;
        }
...
Line * SMBArea::make_dump_msg(Line * & lin, gmsg * msg, char * lng_head)
{
    ...
    line =
        AddLineF(line, "Number            : %d (%d)", smsg.hdr.number,
    ...
    line =
        AddLineF(line,
                 "Reserved          : %s",
                 HexDump16(buf, (const char *)smsg.hdr.reserved, 16,
    ...
    if(smsg.to_net.type)
    {
        ...
        line =
            AddLineF(line, "To net addr       : %s",
                     (char *)(smsg.to_net.type ==
                              NET_FIDO ? faddrtoa(*(fidoaddr_t *)smsg.to_net.addr) : smsg
                              .to_net.addr));
    }
        ...
        line =
            AddLineF(line,
                     ...
                     (char *)(smsg.replyto_net.type ==
                              NET_FIDO ? faddrtoa(
                                  *(fidoaddr_t *)smsg.replyto_net.addr) : smsg.
                              replyto_net.addr));
    }
    ...
    for(i = 0; i < smsg.total_hfields; i++)
    {
        ...
        line =
            AddLineF(line, "hfield_dat[%02u]    : %s", i,
                     binstr((char *)smsg.hfield_dat[i], smsg.hfield[i].length));
    }
    line = AddLine(line, "");
