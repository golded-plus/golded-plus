...
void SMBArea::data_open()
{
    ...
    data->sdt_fp = data->shd_fp = data->sid_fp = data->sda_fp = data->sha_fp =
        NULL;
    data->retry_time    = 1;
...
void SMBArea::open()
{
    ...
    if(isopen == 1)
    {
        ...
        for( ;; )
        {
            if(smb_open(data) != 0)
            {
                if((errno != EACCES) or (PopupLocked(++_tries, false,
                                                     data->file) == false))
                {
                    // User requested to exit
                    WideLog->ErrOpen();
                    WideLog->printf(
                        "! Synchronet message base could not be opened (%s).",
                        data->last_error);
        ...
        if(not fsize(data->shd_fp))
        {
            ...
            if(smb_create(data) != 0)
            {
                smb_close(data);
                WideLog->ErrOpen();
                WideLog->printf("! Synchronet message base could not be created (%s).",
                                data->last_error);
                WideLog->printf(": %s", real_path());
                WideLog->ErrOSInfo();
                OpenErrorExit();
    ...
    for( ;; )
    {
        if(smb_open(data) != 0)
        {
            if((errno != EACCES) or (PopupLocked(++_tries, false, data->file) == false))
            {
                // User requested to exit
                WideLog->ErrOpen();
                WideLog->printf("! Synchronet message base could not be opened (%s).",
                                data->last_error);
                WideLog->printf(": %s", real_path());
                WideLog->ErrOSInfo();
                OpenErrorExit();
...
int SMBArea::load_hdr(gmsg * __msg, smbmsg_t * smsg)
{
    ...
    if(not fread(&smsgp->idx,
                 1,
                 sizeof(idxrec_t),
                 data->sid_fp) or (smb_lockmsghdr(data,
                                                  smsgp)
                                   != 0))
    {
        GFTRK(0);
    ...
    if(smsgp->from_net.type == NET_FIDO)
    {
        __msg->orig.zone = __msg->oorig.zone =
            ((fidoaddr_t *)smsgp->from_net.addr)->zone;
        __msg->orig.net  = __msg->oorig.net = ((fidoaddr_t *)smsgp->from_net.addr)->net;
        __msg->orig.node = __msg->oorig.node =
            ((fidoaddr_t *)smsgp->from_net.addr)->node;
        __msg->orig.point = __msg->oorig.point =
            ((fidoaddr_t *)smsgp->from_net.addr)->point;
    }
    else
    {
    ...
    if(smsgp->to_net.type == NET_FIDO)
    {
        ...
        __msg->dest.point = __msg->odest.point =
            ((fidoaddr_t *)smsgp->to_net.addr)->point;
    }
    else
    {
...
int SMBArea::load_msg(gmsg * msg)
{
    ...
    for(i = 0; i < smsg.total_hfields; i++)
    {
        switch(smsg.hfield[i].type)
        {
            ...
            case FIDOCTRL:
                ...
                if(not strncmp((char *)smsg.hfield_dat[i], "Via ",
                               4) or not strncmp((char *)smsg.hfield_dat[i], "Recd ", 5))
                {
                    break;
                }
    ...
    for(i = 0; i < smsg.hdr.total_dfields; i++)
    {
        switch(smsg.dfield[i].type)
        {
            ...
            common:     fseek(data->sdt_fp,
                              smsg.hdr.offset + smsg.dfield[i].offset,
                              SEEK_SET);
                l = sizeof(xlat);
                ...
                if(lzh)
                {
                    ...
                    msg->txt = (char *)throw_realloc(msg->txt, txt_len + outlen);
                    lzh_decode(inbuf, smsg.dfield[i].length - l,
                               (uint8_t *)(msg->txt + txt_len - 1));
                    throw_xfree(inbuf);
                }
                else if(l < smsg.dfield[i].length)
                {
                    ...
                    msg->txt = (char *)throw_realloc(msg->txt, txt_len + outlen);
                    fread(msg->txt + txt_len - 1,
                          smsg.dfield[i].length - l,
                          1,
                          data->sdt_fp);
                }
                else
    ...
    for(i = 0; i < smsg.total_hfields; i++)
    {
        switch(smsg.hfield[i].type)
        {
            ...
            case FIDOCTRL:
                ...
                if(strncmp((char *)smsg.hfield_dat[i], "Via ",
                           4) and strncmp((char *)smsg.hfield_dat[i], "Recd ", 5))
                {
                    break;
                }

                sprintf(buf, "\r\001%s", (char *)smsg.hfield_dat[i]);
            add2:       outlen = strlen(buf);
                msg->txt       = (char *)throw_realloc(msg->txt, txt_len + outlen);
                strcpy(msg->txt + txt_len - 1, buf);
                txt_len += outlen;
                break;
...
void SMBArea::save_hdr(int mode, gmsg * msg)
{
    ...
    if(not (mode & GMSG_NEW))
    {
        ...
        if(not fread(&smsg.idx, 1, sizeof(idxrec_t),
                     data->sid_fp) or (smb_lockmsghdr(data,
                                                      &smsg) != 0))
        {
            GFTRK(0);
    ...
    if(msg->attr.pvt())
    {
        smsg.hdr.attr |= MSG_PRIVATE;
    }

    if(msg->attr.rcv())
    {
        smsg.hdr.attr |= MSG_READ;
    }

    if(msg->attr.lok())
    {
        smsg.hdr.attr |= MSG_LOCKED;
    }
    ...
    if(msg->attr.r_o())
    {
        smsg.hdr.attr |= MSG_PERMANENT;
    }

    if(msg->attr.loc())
    {
        smsg.hdr.netattr |= MSG_LOCAL;
    }

    if(msg->attr.cra())
    {
        smsg.hdr.netattr |= MSG_CRASH;
    }

    if(msg->attr.imm())
    {
        smsg.hdr.netattr |= MSG_IMMEDIATE;
    }

    if(msg->attr.dir())
    {
        smsg.hdr.netattr |= MSG_DIRECT;
    }

    if(msg->attr.hld())
    {
        smsg.hdr.netattr |= MSG_HOLD;
    }

    if(msg->attr.snt())
    {
        smsg.hdr.netattr |= MSG_SENT;
    }

    if(msg->attr.trs())
    {
        smsg.hdr.netattr |= MSG_INTRANSIT;
    }

    if(msg->attr.orp())
    {
        smsg.hdr.netattr |= MSG_ORPHAN;
    }

    if(msg->attr.k_s())
    {
        smsg.hdr.netattr |= MSG_KILLSENT;
    }

    if(msg->attr.a_s())
    {
        smsg.hdr.netattr |= MSG_ARCHIVESENT;
    }

    if(msg->attr.zon())
    {
        smsg.hdr.netattr |= MSG_GATE;
    }

    if(msg->attr.att())
    {
        smsg.hdr.auxattr |= MSG_FILEATTACH;
    }

    if(msg->attr.frq())
    {
        smsg.hdr.auxattr |= MSG_FILEREQUEST;
    }

    if(msg->attr.rrq())
    {
        smsg.hdr.auxattr |= MSG_RECEIPTREQ;
    }

    if(msg->attr.cfm())
    {
        smsg.hdr.auxattr |= MSG_CONFIRMREQ;
    }

    if(msg->attr.tfs())
    {
        smsg.hdr.auxattr |= MSG_TRUNCFILE;
    }
    ...
    for(l = bodylen = taillen = done = 0, cr = true; (ch = fbuf[l]) != NUL; l++)
    {
        if(ch == CTRL_A and cr)
        {
            ...
            if(not strncmp(fbuf + l, "MSGID:", 6))
            {
                ...
                while(fbuf[l] and fbuf[l] == ' ')
                {
                    l++;
                }
                m = l;

                while(fbuf[m] and not iscntrl(fbuf[m]))
                {
                    m++;
                }
            ...
            {
                if(strncmp(fbuf + l, "TOPT ", 5) and strncmp(fbuf + l, "FMPT ",
                                                             5) and strncmp(fbuf + l,
                                                                            "INTL ", 5))
                {
                    while(fbuf[l] and fbuf[l] == ' ')
                    {
        ...
        if(ch != LF and ch != SOFTCR)
        {
            if(cr)
            {
                if(not done and not strncmp(fbuf + l, "--- ", 4) or not strncmp(fbuf + l,
                                                                                "---\r",
                                                                                4))
                {
...
void SMBArea::del_msg(gmsg * msg)
{
    ...
    if(not fread(&smsg.idx, 1, sizeof(idxrec_t), data->sid_fp) or (smb_lockmsghdr(data,
                                                                                  &smsg)
                                                                   != 0))
    {
        GFTRK(0);
...
Line * SMBArea::make_dump_msg(Line *& lin, gmsg * msg, char * lng_head)
{
    ...
    Line * line = lin = AddLine(NULL,
                                "Hexdump of Synchronet-style message header and text");
    AddLineF(line,
             "------------------------------------------------------------------------------");
    line = AddLine(line, "");
    smbmsg_t smsg;
    ...
    line = AddLineF(line, "Number            : %d (%d)", smsg.hdr.number,
                    (int32_t)(ftell(data->sid_fp) / sizeof(idxrec_t)));
    ...
    line = AddLineF(line, "Reserved          : %s", HexDump16(buf,
                                                              (const char *)smsg.hdr.
                                                              reserved,
                                                              16,
    ...
    if(smsg.from_net.type)
    {
        ...
        line = AddLineF(line, "From net addr     : %s",
                        (char *)(smsg.from_net.type ==
                                 NET_FIDO ? faddrtoa(*(fidoaddr_t *)smsg.from_net.addr) :
                                 smsg.from_net.addr));
    }
    ...
    if(smsg.to_net.type)
    {
        ...
        line = AddLineF(line, "To net addr       : %s",
                        (char *)(smsg.to_net.type ==
                                 NET_FIDO ? faddrtoa(*(fidoaddr_t *)smsg.to_net.addr) :
                                 smsg.to_net.addr));
    }
    ...
    if(smsg.replyto_net.type)
    {
        ...
        line = AddLineF(line, "Replyto net addr  : %s",
                        (char *)(smsg.replyto_net.type ==
                                 NET_FIDO ?
                                 faddrtoa(*(fidoaddr_t *)smsg.replyto_net.addr) :
                                 smsg.replyto_net.addr));
    }
    ...
    for(i = 0; i < smsg.total_hfields; i++)
    {
        ...
        line = AddLineF(line, "hfield_dat[%02u]    : %s", i,
                        binstr((char *)smsg.hfield_dat[i],
                               smsg.hfield[i].length));
    }
    line = AddLine(line, "");
