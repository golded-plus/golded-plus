...
{
    wide->user->gufh = ::sopen(AddPath(wide->userbasepath, "USERS.BBS"),
                               O_RDWR | O_BINARY,
                               WideSharemode,
                               S_STDRW);
...
Line * EzycomArea::make_dump_msg(Line *& lin, gmsg * msg, char * lng_head)
{
    ...
    char buf[100];
    Line * line = lin = AddLine(NULL, "Hexdump of Ezycom-style message header and text");
    AddLineF(line,
             "------------------------------------------------------------------------------");
    line = AddLine(line, "");
    ...
    AddLineF(line,
             "PostDate : %s (%08Xh)",
             FTimeToStr(buf, _hdr.posttimedate),
             *(dword *)&_hdr.posttimedate);
    AddLineF(line,
             "RecvDate : %s (%08Xh)",
             FTimeToStr(buf, _hdr.recvtimedate),
             *(dword *)&_hdr.recvtimedate);
