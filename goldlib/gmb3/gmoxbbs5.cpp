...
Line * XbbsArea::make_dump_msg(Line *& lin,
                               gmsg * msg,
                               char * lng_head)
{
    GFTRK("XbbsMakeDump");
    ...
    char buf[100];
    Line * line = lin = AddLine(NULL, "Hexdump of AdeptXBBS message header and text");
    AddLineF(line,
             "------------------------------------------------------------------------------");
    line = AddLine(line, "");
    ...
    AddLineF(line,
             "TimeRecv   : %s (%08Xh)",
             TimeToStr(buf, hdr.timerecv),
             (int32_t)hdr.timerecv);
    ...
    AddLineF(line, "MsgidSerialNo   : %08Xh (%u)", idx->msgidserialno,
             idx->msgidserialno);
    AddLineF(line, "ReplyidCrc      : %08Xh (%u)", idx->replycrc, idx->replycrc);
    AddLineF(line, "ReplyidSerialNo : %08Xh (%u)", idx->replyserialno,
             idx->replyserialno);
    line = AddLine(line, "");
