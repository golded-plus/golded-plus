...
//  ------------------------------------------------------------------
void SquishArea::excess_frm(dword __lastframe,
                            dword __newframe,
                            SqshFrm * __newfrm,
                            dword __totsize)
{
    ...
    if(_excesslength >= sizeof(SqshFrm))
    {
        ...
        //WideLog->printf("- Created excess free frame 0x%08X of length %u.", _exframe,
        // _exfrm.length);
    }
} // SquishArea::excess_frm
...
void SquishArea::save_message(int __mode, gmsg * __msg)
{
    ...
    if((__mode & GMSG_NEW) or find_msgn(__msg->msgno))
    {
        ...
        SqshIdx * _idx   = data->idx;
        SqshBase & _base = data->base;
        dword _hash      = strHash32(__hdr.to) |
                           ((__hdr.attr & MSGREAD) ? 0x80000000LU : 0);
        ...
        if(__mode & GMSG_TXT)
        {
            ...
            if(_newframe == SQFRAME_NULL)
            {
                ...
                while(1)
                {
                    ...
                    if(_newframe == SQFRAME_NULL)
                    {
                        ...
                        init_frm(&_newfrm);
                        //WideLog->printf("- Allocated new frame 0x%08X of length %u.",
                        // _newframe, _totsize);
                        break;
                    }
                    ...
                    //WideLog->printf("- Found free frame 0x%08X of length %u.", _newframe,
                    // _newfrm.length);
                    if(_newfrm.length >= _totsize)
                    {
                        ...
                        if(wide->recycle == SQUISHRECYCLE_YES)
                        {
                            ...
                            //WideLog->printf("- Frame was large enough (%u bytes wasted).",
                    ...
                    if(wide->recycle and (wide->recycle != SQUISHRECYCLE_MSGAPI2))
                    {
                        ...
                        while((_newfrm.next ==
                               (_newframe + _newfrm.length +
                                sizeof(SqshFrm))) and (_newfrm.length < _totsize))
                        {
                            SqshFrm _lastfrm;
                            read_frm(_newfrm.next, &_lastfrm);
                            _newfrm.length += _lastfrm.length + sizeof(SqshFrm);
                            //WideLog->printf("- Merged frames 0x%08X and 0x%08X. New length:
                            // %u.", _newframe, _newfrm.next, _newfrm.length);
                        ...
                        if(_newfrm.length >= _totsize)
                        {
                            ...
                            if(wide->recycle == SQUISHRECYCLE_YES)
                            {
                                ...
                                //WideLog->printf("- Merged frame was large enough (%u bytes
                                // wasted).", _newfrm.length - _totsize);
                            }
            ...
            if(__mode & GMSG_NEW)
            {
                ...
                data->idx = _idx = (SqshIdx *)throw_realloc(data->idx,
                                                            (uint)(_base.totalmsgs *
                                                                   sizeof(SqshIdx)));
            }
