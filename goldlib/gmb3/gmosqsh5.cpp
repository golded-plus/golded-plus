...
Line * SquishArea::make_dump_msg(Line * & lin, gmsg * msg, char * lng_head)
{
    ...
    char buf[100];
    Line * line = lin = AddLine(NULL, "Hexdump of Squish-style message header and text");
    AddLineF(line,
             "------------------------------------------------------------------------------");
    line = AddLine(line, "");
    ...
    AddLineF(line, "DateWritten    : %s (%08Xh)", FTimeToStr(buf, _hdr.date_written),
             *(dword *)&_hdr.date_written);
    AddLineF(line, "DateArrived    : %s (%08Xh)", FTimeToStr(buf, _hdr.date_arrived),
             *(dword *)&_hdr.date_arrived);
    ...
    AddLineF(line,
             "FirstFreeFrame : %08Xh (%u)",
             _base.firstfreeframe,
             _base.firstfreeframe);
    AddLineF(line,
             "LastFreeFrame  : %08Xh (%u)",
             _base.lastfreeframe,
             _base.lastfreeframe);
    ...
    AddLineF(line,
             "Frame-ID       : %08Xh%s",
             _frm.id,
             (_frm.id != SQFRAMEID) ? " (error, should be AFAE4453h!)" : "");
