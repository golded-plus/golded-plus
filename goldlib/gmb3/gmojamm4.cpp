...
//  ------------------------------------------------------------------
void JamArea::add_subfield(JamHdr & __hdr,
                           byte *& __subfield,
                           word __loid,
                           word __hiid,
                           char * __data,
                           uint32_t __maxlen)
{
    ...
    __subfield = (byte *)throw_realloc(__subfield,
                                       (uint)__hdr.subfieldlen + sizeof(JamSubFieldHdr) +
                                       _datlen);
...
void JamArea::save_message(int __mode, gmsg * __msg, JamHdr & __hdr)
{
            ...
            if(_pdptr->control > CTRL_KLUDGE)
            {
                ...
                switch(_pdptr->control)
                {
                    ...
                    case CTRL_MSGID:
                        ...
                        _offset = 8;
                        strxcpy(__msg->msgids, _pdptr->text + _offset,
                                sizeof(__msg->msgids));
                        break;
                    ...
                    case CTRL_REPLY:
                        ...
                        _offset = 8;
                        strxcpy(__msg->replys, _pdptr->text + _offset,
                                sizeof(__msg->replys));
                        break;
    ...
    if(__mode & GMSG_NEW)
    {
        __msg->msgno = data->hdrinfo.basemsgnum +
                       (filelength(data->fhjdx) / sizeof(JamIndex));
    ...
    {
        __hdr.messagenumber = __msg->msgno;
        lseekset(data->fhjdx, __hdr.messagenumber - data->hdrinfo.basemsgnum,
                 sizeof(JamIndex));
    ...
    if(_idx.hdroffset != 0xFFFFFFFFL)
    {
        ...
        __hdr.msgidcrc    = strCrc32(jamstrlwr(strcpy(_buf, __msg->msgids)),
                                     NO,
                                     CRC32_MASK_CCITT);
        __hdr.replycrc = strCrc32(jamstrlwr(strcpy(_buf, __msg->replys)),
                                  NO,
                                  CRC32_MASK_CCITT);
        _idx.usercrc = strCrc32(jamstrlwr(strcpy(_buf, __msg->to)),
                                NO,
                                CRC32_MASK_CCITT);
