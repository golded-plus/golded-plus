...
//  ------------------------------------------------------------------
Line * AddLineF(Line *& line, const char * format, ...) __attribute__ ((format(printf,
                                                                               2,
                                                                               3)));
...
class gmsg_links
{
private: uint32_t reply_to;
    uint32_t reply_first;
    uint32_t reply_next;
    std::vector<uint32_t> reply_list;
public: gmsg_links() : reply_to(0), reply_first(0), reply_next(0)
    {}
...
class gmsg
{
public: uint board;               // Board number (if applicable)
    ...
    gmsg() : board(0), msgno(0), link(), oorig(), orig(), odom(), odest(), dest(),
        ddom(), written(0), arrived(0), received(0), attr(), cost(0), timesread(0),
        msgid(), txt(NULL)
    {
        by[0]     = 0;
        to[0]     = 0;
