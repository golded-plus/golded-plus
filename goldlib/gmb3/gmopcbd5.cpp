...
Line * PcbArea::make_dump_msg(Line *& lin, gmsg * __msg, char * lng_head)
{
    ...
    Line * line = lin = AddLine(NULL, "Hexdump of PCBoard message header and text");
    AddLineF(line,
             "------------------------------------------------------------------------------");
    line = AddLine(line, "");
    ...
    AddLineF(line,
             "Active    : %u%s",
             _hdr.activestatus,
             (_hdr.activestatus == 226) ? " (Deleted)" : "");
    AddLineF(line, "Echoed    : \'%c\'", _hdr.echoed);
    AddLineF(line,
             "Reserved  : %02Xh %02Xh %02Xh %02Xh ",
             _hdr.reserved[0],
             _hdr.reserved[1],
             _hdr.reserved[2],
             _hdr.reserved[3]);
