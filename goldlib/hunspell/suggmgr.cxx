...
int SuggestMgr::suggest(char *** slst, const char * w, int nsug)
{
    ...
    for(int cpdsuggest = 0; (cpdsuggest < 2) && (nocompoundtwowords == 0); cpdsuggest++)
    {
        ...
        {
            nsug =
                (utf8) ? swapchar_utf(wlst, word_utf, wl, nsug, cpdsuggest) : swapchar(
                    wlst,
                    word,
                    nsug,
                    cpdsuggest);
        ...
        {
            nsug =
                (utf8) ? forgotchar_utf(wlst, word_utf, wl, nsug,
                                        cpdsuggest) : forgotchar(
                    wlst,
                    word,
                    nsug,
                    cpdsuggest);
        ...
        {
            nsug =
                (utf8) ? extrachar_utf(wlst, word_utf, wl, nsug, cpdsuggest) : extrachar(
                    wlst,
                    word,
                    nsug,
                    cpdsuggest);
        ...
        if((nsug < maxSug) && (nsug > -1))
        {
            nsug = (utf8) ? badchar_utf(wlst, word_utf, wl, nsug, cpdsuggest) : badchar(
                wlst,
                word,
                nsug,
                cpdsuggest);
...
int SuggestMgr::mapchars(char ** wlst, const char * word, int ns, int cpdsuggest)
{
    ...
    {
        ns = map_related(word, 0, wlst, ns, maptable, nummap, &timer, &timelimit);
    }
                            ...
                            time_t * timelimit)
{
    ...
    if(c == 0)
    {
        ...
        if((cwrd) && (wl = strlen(word)) &&
           (check(word, wl, 0, timer,
                  timelimit) || check(word, wl, 1, timer, timelimit)))
        {
            if(ns < maxSug)
            {
    ...
    return ns;
} // SuggestMgr::map_related

int SuggestMgr::map_related_utf(w_char * word,
                                int len,
                                int i,
                                char ** wlst,
                                int ns,
                                ...
                                time_t * timelimit)
{
    if(i == len)
    {
        ...
        if((cwrd) && (wl = strlen(s)) &&
           (check(s, wl, 0, timer, timelimit) || check(s, wl, 1, timer, timelimit)))
        {
            if(ns < maxSug)
            {
                wlst[ns] = mystrdup(s);
                            ...
                            int cpdsuggest)
{
    ...
    for(int i = 0; i < wl; i++)
    {
        ...
        for(int j = 0; j < ctryl; j++)
        {
            if((ctry_utf[j].l == tmpc.l) && (ctry_utf[j].h == tmpc.h))
            {
                continue;
            }
            ...
            if((cwrd) &&
               check(candidate, strlen(candidate), cpdsuggest, &timer, &timelimit))
            {
                if(ns < maxSug)
                {
                    wlst[ns] = mystrdup(candidate);
...
int SuggestMgr::ngsuggest(char ** wlst, char * w, HashMgr * pHMgr)
{
    ...
    while((hp = pHMgr->walk_hashtable(col, hp)))
    {
        ...
        if((hp->astr) && (pAMgr) &&
           (TESTAFF(hp->astr, pAMgr->get_forbiddenword(),
                    hp->alen) ||
            TESTAFF(hp->astr, pAMgr->get_nosuggest(),
                    hp->alen) ||
    ...
    for(i = 0; i < MAX_ROOTS; i++)
    {
        if(roots[i])
        {
            ...
            int nw             = pAMgr->expand_rootword(glst,
                                                        MAX_WORDS,
                                                        rp->word,
                                                        rp->wlen,
                                                        rp->astr,
    ...
    for(i = 0; i < MAX_GUESS; i++)
    {
        if(guess[i])
        {
            if((ns < maxngramsugs) && (ns < maxSug) && (!same || (gscore[i] > 1000)))
            {
                ...
                for(j = 0; j < ns; j++)
                {
                    // don't suggest previous suggestions or a previous suggestion with prefixes
                    // or affixes
                    if(strstr(guess[i], wlst[j]) ||
                       // check forbidden words
                       !check(guess[i], strlen(guess[i]), 0, NULL, NULL))
                    {
                        unique = 0;
                    }
                }
...
int SuggestMgr::check(const char * word,
                      int len,
                      int cpdsuggest,
                      int * timer,
                      time_t * timelimit)
{
    ...
    if(pAMgr)
    {
        if(cpdsuggest == 1)
        {
            ...
            {
                rv =
                    pAMgr->compound_check(word, len, 0, 0, 0, 0, NULL, 0, NULL, NULL, 1);
        ...
        {
            if((rv->astr) &&
               (TESTAFF(rv->astr, pAMgr->get_forbiddenword(),
                        rv->alen) ||
            ...
            if(rv->astr &&
               (TESTAFF(rv->astr, pAMgr->get_pseudoroot(),
                        rv->alen) ||
        ...
        {
            rv = pAMgr->prefix_check(word, len, 0); // only prefix, and prefix + suffix XXX
        }
        ...
        if((rv) && (rv->astr) &&
           (TESTAFF(rv->astr, pAMgr->get_forbiddenword(),
                    rv->alen) ||
            TESTAFF(rv->astr, pAMgr->get_nosuggest(),
                    rv->alen) ||
            TESTAFF(rv->astr, pAMgr->get_onlyincompound(), rv->alen)))
        {
            return 0;
...
int SuggestMgr::check_forbidden(const char * word, int len)
{
    ...
    if(pAMgr)
    {
        ...
        {
            rv = pAMgr->suffix_check(word, len, 0, NULL, NULL, 0, NULL); // prefix+suffix,
                                                                         // suffix
        }
            ...
            if(*p == '-')
            {
                *p   = '\0';
                ...
                if((nsug == prevnsug) && (nsug < maxSug) && (nsug >= 0))
                {
                    ...
                    for(t = s; (t[0] != '\0') && ((t[0] >= '0') || (t[0] <= '9')); t++) // is a
                                                                                        // number?
                    {}
...
int SuggestMgr::fixstems(char ** wlst, const char * word, int ns)
{
    ...
    if(pAMgr)
    {
        ...
        else
        {
            ...
            if((prefix) && (strncmp(prefix, "leg", 3) == 0))
            {
                prefix[0] = '\0';                                         // (HU)
            }
        }
    ...
    if((rv) && (ns < maxSug))
    {
        ...
        if((ns < maxSug) && (dicstem < 2))
        {
            ...
            else
            {
                ...
                if(rv->description && (strchr(wordchars, *(rv->description))))
                {
                    char * desc = (rv->description) + 1;

                    while(strchr(wordchars, *desc))
                    {
                        desc++;
                    }
                    ...
                    if((rv2) && (rv2->astr) && (ns < maxSug))
                    {
                        if(0)
                        {
                            ...
                            if((dicstem == 2) && (ns < maxSug))
                            {
                                ...
                                if(pAMgr->get_compound() &&
                                   (pAMgr->compound_check(buf, strlen(buf), 0, 0, 100, 0,
                                                          NULL, 0, NULL, NULL, 1)))
                                {
                                    wlst[ns] = mystrdup(buf);
                        ...
                        else
                        {
                            ...
                            do
                            {
                                ...
                                if((strlen(pos) > 0) && (pos[strlen(pos) - 1] == '-'))
                                {
                                    ...
                                    if((dicstem != 0) && (strcmp(buf, word) != 0))
                                    {
                                        suggest = 0;
                                    }
                                }
...
char * SuggestMgr::suggest_morph(const char * w)
{
    ...
    *result = '\0';
    ...
    while(rv)
    {
        ...
        {
            if(rv->description &&
               ((!rv->astr) || !TESTAFF(rv->astr, pAMgr->get_lemma_present(), rv->alen)))
            {
                strcat(result, word);
            }
    ...
    {
        pAMgr->compound_check_morph(word, strlen(word), 0, 0, 100, 0, NULL, 0, &r, NULL);
    }
...
int SuggestMgr::commoncharacterpositions(char * s1, const char * s2, int * is_swap)
{
    ...
    *is_swap = 0;
    ...
    return num;
} // SuggestMgr::commoncharacterpositions
