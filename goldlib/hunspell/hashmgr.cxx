    ...
    if(dp->word == NULL)
    {
        ...
        else
        {
            ...
            if(dp->description && complexprefixes)
            {
                if(utf8)
                {
                    reverseword_utf(dp->description);
                }
                else
                {
                    reverseword(dp->description);
                }
            }
        }
    }
    else
    {
        ...
        else
        {
            ...
            if(dp->description && complexprefixes)
            {
                if(utf8)
                {
                    reverseword_utf(hp->description);
                }
                else
                {
                    reverseword(hp->description);
                }
            }
        }
...
int HashMgr::load_tables(const char * tpath)
{
    ...
    if((*ts < '1') || (*ts > '9'))
    {
        fprintf(stderr, "error - missing word count in dictionary file\n");
    }
            ...
            for(p = flags; *p; p++)
            {
                if(*p == ',')
                {
                    *dest = (unsigned short)atoi(src);
                    ...
                    if(*dest == 0)
                    {
                        fprintf(stderr, "error: 0 is wrong flag id\n");
                    }
...
int HashMgr::load_config(const char * affpath)
{
    ...
    while(fgets(line, MAXDELEN, afflst))
    {
        ...
        if((strncmp(line, "SET", 3) == 0) && isspace(line[3]) && strstr(line, "UTF-8"))
        {
            utf8 = 1;
        }
        ...
        if(((strncmp(line, "SFX",
                     3) == 0) || (strncmp(line, "PFX", 3) == 0)) && isspace(line[3]))
        {
            break;
        }
    }
