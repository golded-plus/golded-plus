...
int throw_index_find_free()
{
    ...
    {
        throw_index =
            (Throw **)realloc(throw_index,
                              (throw_index_size + throw_index_increment) *
                              sizeof(Throw *));
        throw_new(throw_index);
        memset(throw_index + throw_index_size, 0,
               throw_index_increment * sizeof(Throw *));
...
void throw_printdl(Throw * dl)
{
    ...
    TLOG->printf(": Ptr (%p,%u) at [%s,%u].", ptr, (uint)dl->nbytes,
                 CleanFilename(dl->file), (uint)dl->line);
...
{
    char * _ptr =
        (char *)(__str ? throw_calloc_debug(1, strlen(__str) + 1, __file,
                                            __line) : NULL);
...
//  ------------------------------------------------------------------
//  Debugging realloc()
void * throw_realloc_debug(void * __oldptr,
                           size_t __size,
                           const char * __file,
                           int __line)
{
...
static void throw_term(void)
{
    if(throw_inited)
    {
        ...
        if(throw_count)
        {
            TLOG->printf("! Detected %i unfree'd memory allocation%s.",
                         throw_count,
                         throw_count == 1 ? "" : "s");
        }
