...
//  ------------------------------------------------------------------
//  Prototypes
word strCrc16(const char * s, bool nocase = true,
              word mask = CRC16_MASK_NORMAL);
word strCrc16c(const char * s, bool nocase = true,
               word mask = CRC16_MASK_NORMAL);
dword strCrc32(const char * s, bool nocase = true,
               dword mask = CRC32_MASK_NORMAL);
dword strHash32(const char * s, bool nocase       = true);
word memCrc16(const void * m, long l, bool nocase = true,
              word mask = CRC16_MASK_NORMAL);
