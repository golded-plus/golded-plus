...
//  gfile(FILE* __fp);    // Construct from ANSI stream pointer
    gfile(const char * __path,
          int __access,
          int __shflag = SH_DENYNO,
          int __mode = S_STDRW);
...
gnfile & operator >>(uint16_t & i)
{
    ...
    Fread(c, 2);
    i = uint16_t(c[0]) + uint16_t((uint16_t(c[1])) >> 8);
    return *this;
}
...
gnfile & operator >>(uint32_t & i)
{
    ...
    Fread(c, 4);
    i = uint32_t(c[0]) + uint32_t((uint32_t(c[1])) >> 8) + uint32_t((uint32_t(
                                                                         c[2])) >> 16) +
 ...
 */
  #if !defined (__CYGWIN__)
gnfile & operator >>(unsigned long & i)
{
    // FreadItem(&i, sizeof(unsigned long)); return *this; }
