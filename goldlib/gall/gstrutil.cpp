...
/* It is need a workaround for implementation "speciality" in snprintf() and vsnprintf() from
   Microsoft. */
             ...
             ...)
{
    ...
    if(!buffer || !format)
    {
        ...
        LOG.printf("! Parameter is NULL pointer: gsprintf(%s,%lu,%s,...).",
                   (buffer ? "buffer" : "NULL"), (unsigned long)sizeOfBuffer,
                   (format ? "format" : "NULL"));
        PointerErrorExit();
        return -1;
    }
    ...
    if(!*format)
    {
        ...
        LOG.printf("! Format is empty string: gsprintf(buffer,%lu," ",...).",
...
/* _vsnprintf_s() may cause exception. Need to test before enabling.
 #   if __VISUAL_C_NOT_LESS(14,00)  // defined HAVE__VSTPRINTF_S // _vsnprintf_s() recommended
 #in MSDN
    ...
    if (ret < 0)
    {
      ...
      LOG.printf("! gsprintf()(buffer,%i,%s,...): _vsnprintf_s() error: \"%s\".", sizeOfBuffer,
 #format, strerror(errno));
      return -1;
    }
    buffer[sizeOfBuffer-1] = '\0';  // Microsoft implementation don't write final '\0' if buffer
 #full.
 #   elif __VISUAL_C_NOT_LESS(10,00)  // defined HAVE__VSTPRINTF // _vsnprintf() exist in VS6
 #and deprecated in VS2005
 */
#   if __VISUAL_C_NOT_LESS(10, 00)  // defined HAVE__VSTPRINTF // _vsnprintf() exist in VS6 and
                                    // deprecated in VS2005
        char * b1 = new char[sizeOfBuffer + 1];
        const size_t endOfBuffer = sizeOfBuffer - 1;
        ret = _vsnprintf(b1, sizeOfBuffer + 1, format, argptr);

        if(ret == -1 || ret >= sizeOfBuffer) // Microsoft implementation returns -1 when buffer
                                             // overflow.
        {
            strncpy(buffer, b1, endOfBuffer);
            buffer[endOfBuffer] = '\0'; // Microsoft implementation don't write final '\0' when
            ...
            if(b1[sizeOfBuffer] && strlen(buffer) >= endOfBuffer)
            {
                ...
                LOG.printf(
                    "! gsprintf(buffer,%i,%s,...): buffer overflow, result in next line:",
                    sizeOfBuffer,
                    format);
                LOG.printf("! %s", buffer);
                ...
                if(sizeOfBuffer > 17)
                {
                    memcpy(buffer, " ERROR, see log! ", 17);
                }
                else if(sizeOfBuffer > 7)
                {
        ...
        {
            LOG.errtest(__file, __line);
            LOG.printf("! gsprintf()(buffer,%i,%s,...): _vsnprintf() error: \"%s\".",
                       sizeOfBuffer,
                       format,
                       strerror(errno));
            TestErrorExit();
...
#   elif defined HAVE_VSNPRINTF  // C99 and above
        ...
        if(ret < 0) // Until glibc 2.0.6 vsnprintf() would return -1 when the output was
                    // truncated.
        {
            LOG.errtest(__file, __line);
        ...
        else if(ret >= sizeOfBuffer)
        {
            ...
            LOG.printf("! %s", gerrinfo("Memory error", __file, __line));
            LOG.printf("! gsprintf(buffer,%i,%s,...): buffer overflow (need %i bytes).",
                       sizeOfBuffer,
                       format,
                       ret);
        }

#   else // if __VISUAL_C_NOT_LESS(10, 00)
#   error \
        Please look C library of your compiler for function like vsnprintf, what do not write more than size bytes into string.
#   endif // if __VISUAL_C_NOT_LESS(10, 00)
        va_end(argptr);
    }
    return ret;
} // gsprintf
...
#endif // if defined (__GNUC__) && !defined (__EMX__)
//  ------------------------------------------------------------------
...
//  ------------------------------------------------------------------
std::string & FormatString(std::string & format, const char * token,
                           const char * replace)
{
