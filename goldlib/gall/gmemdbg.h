...
#define throw_xdelete(p) {if(p) {delete (p);} \
                          (p) = NULL;}

#define throw_deletearray(p) {delete[] (p); (p) = NULL;}
#define throw_xdeletearray(p) {if(p) {delete[] (p);} \
                               (p) = NULL;}

#define throw_mallox(__items, __size, __cache) throw_malloc( \
        (__items + __cache + 1) * __size)
#define throw_callox(__items, __size, __cache) throw_calloc(__items + __cache + 1, \
                                                            __size)
#define throw_reallox(__ptr, __items, __size, \
                      __cache) (((__items % \
                                  __cache) and __ptr) ? (void *)__ptr : throw_realloc( \
                                    __ptr, \
                                    ( \
                                        __items \
                                        + __cache + 1) * __size));

#define throw_xmallox(__items, __size, __cache) throw_xmalloc( \
        (__items + __cache + 1) * __size)
#define throw_xcallox(__items, __size, __cache) throw_xcalloc(__items + __cache + 1, \
                                                              __size)
#define throw_xreallox(__ptr, __items, __size, \
                       __cache) (((__items % \
                                   __cache) and __ptr) ? (void *)__ptr : throw_xrealloc( \
                                     __ptr, \
                                     ( \
                                         __items \
                                         + __cache + 1) * __size));
