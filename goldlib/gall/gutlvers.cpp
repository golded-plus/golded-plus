...
{
    __asm{mov eax, cpuidfun cpuid mov esi, CPUInfo mov[esi + 0], eax mov[esi + 4],
          ebx mov[esi + 8], ecx mov[esi + 12], edx}
}

#elif defined (__GNUC__) && defined (__x86_64__)
static void __cpuid(int CPUInfo[4], int cpuidfun)
{
    asm volatile ("cpuid" : "=a" (CPUInfo[0]), "=b" (CPUInfo[1]), "=c" (CPUInfo[2]),
                  "=d" (CPUInfo[3]) : "a" (cpuidfun), "c" (0));
}
...
#endif // if defined (__GNUC__) && (defined (__i386__) || defined (__x86_64__)) || defined
       // (_MSC_VER)
...
//      "movl   %%edx,%7\n\t"       // store cpu_feature
        ...
        : /* modified registers */
        "eax", "ebx", "ecx", "edx");
    cpuname(scpuid.family ? scpuid.family : scpuid.cpu,
            scpuid.model,
            scpuid.vendor,
            _cpuname);

#else // if defined (__GNUC__) && defined (__x86_64__) || defined (_MSC_VER)
...
#endif
#endif // if defined (__GNUC__) && defined (__x86_64__) || defined (_MSC_VER)
...
char * ggetosstring(void)
{
    ...
    if(*osstring == NUL)
    {
...
#else // if defined (__UNIX__) || defined (__DJGPP__) || defined (__EMX__)
        ...
        sprintf(osstring, "%s %d.%02d %s", osname, _osmajor, _osminor,
                gcpuid(processor));

#endif // if defined (__UNIX__) || defined (__DJGPP__) || defined (__EMX__)
    }
