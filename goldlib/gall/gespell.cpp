...
#define CHECK_ERROR(jump) if(error != ERROR_SUCCESS) \
        goto jump
#define CHECK_NULL(ptr, jump) if(ptr == NULL) \
        goto jump
#define CHECK_SEC(jump) if((sec & 0xFF) != SC_SEC_NoErrors) \
        goto jump
//  ------------------------------------------------------------------
...
bool CMSSpellLang::Init(HKEY hKey, const char * name)
{
    ...
    if(!name)
    {
        ...
        LOG.printf(
            "! Parameter is NULL pointer: CMSSpellLang::Init(hkey,NULL). "
...
bool CMSSpellLang::Load(const char * codeset, const char * userdic)
{
    ...
    if(!codeset)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMSSpellLang::Load(NULL,userdic). "
            "This is bug in program, please make report to developers.");
        return false;
    }
...
//  mSpellVer(&w1, &w2, &w3);
    ...
    sec         = mSpellOptions(mSLID,
                                SC_SO_SuggestFromUserDict | SC_SO_IgnoreAllCaps | SC_SO_IgnoreMixedDigits | SC_SO_IgnoreRomanNumerals |
...
void CMSSpellLang::BuildRTable(const char * codeset)
{
    ...
    if(!codeset)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMSSpellLang::BuildRTable(NULL). "
            "This is bug in program, please make report to developers.");
        return;
    }
...
void CMSSpellLang::BuildSuggest(const char * text, CSpellSuggestV & suggest)
{
    ...
    for(int idx = 0; idx < mSRB.cChrMac; idx++)
    {
        ...
        if(flag && mSZ[idx])
        {
            ...
            suggest.push_back(std::pair<byte, std::string>(0,
                                                           "  " +
                                                           std::string(&mSZ[idx]) +
                                                           char(' ')));
        }
...
bool CMSSpellLang::SpellCheck(const char * text)
{
    if(!text)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMSSpellLang::SpellCheck(NULL). "
            "This is bug in program, please make report to developers.");
        return false;
    }
...
bool CMSSpellLang::SpellSuggest(const char * text, bool more)
{
    if(!text)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMSSpellLang::SpellSuggest(NULL). "
            "This is bug in program, please make report to developers.");
        return false;
    }
...
bool CMSSpellLang::AddWord(const char * text)
{
    if(!text)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMSSpellLang::AddWord(NULL). "
            "This is bug in program, please make report to developers.");
        return false;
    }
...
bool CMYSpellLang::Init(const gdirentry * entry)
{
    if(!entry)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMYSpellLang::Init(NULL). "
            "This is bug in program, please make report to developers.");
        return false;
    }
...
bool CMYSpellLang::Load(const char * codeset, const char *)
{
    if(!codeset)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMYSpellLang::Load(NULL,). "
            "This is bug in program, please make report to developers.");
        return false;
    }
...
void CMYSpellLang::BuildRTable(const char * codeset)
{
    if(!codeset)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMYSpellLang::BuildRTable(NULL). "
            "This is bug in program, please make report to developers.");
        return;
    }
...
bool CMYSpellLang::SpellCheck(const char * text)
{
    if(!text)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMYSpellLang::SpellCheck(NULL). "
            "This is bug in program, please make report to developers.");
        return false;
    }
...
bool CMYSpellLang::AddWord(const char * text)
{
    if(!text)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CMYSpellLang::AddWord(NULL). "
            "This is bug in program, please make report to developers.");
    }
...
void CSpellLang::RecodeText(const char * srcText, char * dstText, bool flag)
{
    if(!srcText || !dstText)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer in CSpellLang::RecodeText. "
            "This is bug in program, please make report to developers.");
        return;
    }
...
bool CSpellChecker::Init(const char * codeset, const char * dicPath)
{
    if(!codeset)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CSpellChecker::Init(NULL,dicPath). "
            "This is bug in program, please make report to developers.");
        return false;
    }
    ...
    HKEY hKeySpelling;
    error =
        RegOpenKeyEx(HKEY_LOCAL_MACHINE, SC_RKEY_Prooftools, 0, KEY_READ, &hKeyPTools);
    CHECK_ERROR(cleanup0);
...
bool CSpellChecker::Load(const char * langId, const char * userDic)
{
    if(!langId)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CSpellChecker::Load(NULL,userDic). "
            "This is bug in program, please make report to developers.");
        return false;
    }
...
void CSpellChecker::UnLoad(const char * langId)
{
    if(!langId)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CSpellChecker::UnLoad(NULL). "
            "This is bug in program, please make report to developers.");
        return;
    }
...
bool CSpellChecker::Check(const char * text)
{
    ...
    if(!text)
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Parameter is NULL pointer: CSpellChecker::Check(NULL). "
            "This is bug in program, please make report to developers.");
        return false;
    }
