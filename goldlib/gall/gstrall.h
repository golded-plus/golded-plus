...
char * longdotstr(long num);                              // Convert a long to a dotted string:

// xxx.xxx.xxx.xxx
char * longdotstr(char * str, long num);                  // Convert a long to a dotted string:
...
inline char * strskip_digits(char * p)
{
    ...
    defined (__SUNOS__))) || defined (__WATCOMC__) || defined (__FreeBSD__) || \
...
class GTok
{
protected: const TCHAR * separator;
    TCHAR * token;
#if defined (_tcstok_s)
    TCHAR * next_token;
#endif

public: GTok(TCHAR * sep = NULL);
