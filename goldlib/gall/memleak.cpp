...
// Size of Callstack-trace in bytes (0x500 => appr. 5-9 functions, depending on parameter count
// for each function)
...
public: CMallocSpy(void)
    ...
    STDMETHOD_(void *, PostAlloc)(void * pActual){
        ...
        if(DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(),
                           &hThread, 0, false, DUPLICATE_SAME_ACCESS) != 0)
        {
            // Ok
            CONTEXT c;
            memset(&c, '\0', sizeof c);
            ...
            {
#else
            __asm{call x x : pop eax mov c.Eip, eax mov c.Ebp, ebp}
    ...
    STDMETHOD_(ULONG, PreRealloc)(void * pRequest, ULONG cbRequest, void ** ppNewRequest,
                                  BOOL /* fSpyed */){
        IMallocHashRemove(pRequest);
        m_cbRequest   = cbRequest;
...
typedef struct _IMAGEHLP_SYMBOL64
{
    ...
    DWORD64 Address;                                    // virtual address including dll base
                                                        // address
    DWORD Size;                                         // estimated size of symbol, can be zero
    DWORD Flags;                                        // info about the symbols, see the SYMF
                                                        // defines
...
typedef DWORD64 (__stdcall * PTRANSLATE_ADDRESS_ROUTINE64)(HANDLE hProcess,
                                                           HANDLE hThread,
                                                           LPADDRESS64 lpaddr);
#endif // if API_VERSION_NUMBER < 9
// #############################################################################################
// Forward definitions of functions:
static void ShowStackRM(HANDLE hThread,
                        CONTEXT & c,
                        FILE * fLogFile,
...
static void WriteDateTime(FILE * fFile, BOOL asXMLAttrs = FALSE)
{
    ...
    if(fFile != NULL)
    {
        ...
        if(asXMLAttrs == FALSE)
        {
            _ftprintf(fFile, _T("%s %s"), pszTemp, pszTemp2); // also ok after year 2038
                                                              // (asctime is NOT ok)
        }
        else
        {
            _ftprintf(fFile, _T("date=\"%s\" time=\"%s\" "), pszTemp, pszTemp2); // also ok
                                                                                 // after year
                                                                                 // 2038
                                                                                 // (asctime is
                                                                                 // NOT ok)
...
{
    long lRequestID;                        // RequestID from CRT (if 0, then this entry is
                                            // empty)
...
void IMallocHashInsert(void * pData, CONTEXT & Context, size_t nDataSize)
{
    ...
    else
    {
        ...
        pHashEntry->Next =
            (IMallocHashEntryType *)_calloc_dbg(sizeof(IMallocHashEntryType),
                                                1,
                                                _CRT_BLOCK,
                                                __FILE__,
                                                __LINE__);
    ...
    if(ReadProcessMemory(GetCurrentProcess(), (LPCVOID)Context.Eip,
                         &(pHashEntry->pcEIPAddr), MAX_EIP_LEN_BUF,
    ...
    if(ReadProcessMemory(GetCurrentProcess(), (LPCVOID)Context.Ebp,
                         &(pHashEntry->pcESPAddr), MAX_ESP_LEN_BUF,
                         &(pHashEntry->dwESPLen)) == 0)
    {
        ...
        if(GetLastError() == ERROR_PARTIAL_COPY)
        {
            ...
            DWORD dwRet = VirtualQuery((LPCVOID)Context.Ebp,
                                       &MemBuffer,
                                       sizeof(MemBuffer));
            ...
            if(dwRet > 0)
            {
                ...
                DWORD len = ((DWORD)MemBuffer.BaseAddress + MemBuffer.RegionSize) -
                            Context.Ebp;
        ...
        if(pHashEntry->pData == pData)
        {
            ...
            if(pHashEntryLast == NULL)
            {
                ...
                if(pHashEntry->Next == NULL)
                {
                    ...
                    memset(&IMallocHashTable[HashIdx], 0,
                           sizeof(IMallocHashTable[HashIdx]));
                }
                else
                {
                    // Es sind noch Einträge verkettet, überschreibe einfach den nicht mehr
                    // gebrauchten...
...
static BOOL __stdcall ReadProcMemoryFromIMallocHash(HANDLE pData,
        ...
        if(pHashEntry->pData != 0)
        {
            while(pHashEntry != NULL)
            {
                ...
                if((pHashEntry->cRemovedFlag <= 0) || (pHashEntry->cRemovedFlag > 1))
                {
                    ...
                    {
                        _ftprintf(fFile,
                                  _T("<LEAK requestID=\"%u\" size=\"%u\">\n"),
                                  pHashEntry->pData,
                                  pHashEntry->nDataSize);
                    }
                    else
                    {
                        _ftprintf(fFile,
                                  _T(
                                      "Pointer (RequestID): %12i, Removed: %i, Size: %12i\n"),
                                  pHashEntry->pData,
                                  pHashEntry->cRemovedFlag,
                                  pHashEntry->nDataSize);
                    }
                    ...
                    {
                        ulLeaksByte++; // Wenn zwar Speicher allokiert wurde, dieser aber 0
                                       // Bytes lang war, so reserviere für diesen zumindest 1
                                       // Byte
                    }
            ...
            while(pHashEntry != NULL)
            {
                if((pHashEntry->cRemovedFlag <= 0) || (pHashEntry->cRemovedFlag > 1))
                {
                    ...
                    if(g_CallstackOutputType == ACOutput_XML)
                    {
                        _ftprintf(fFile,
                                  _T("<LEAK requestID=\"%u\" size=\"%u\">\n"),
                                  pHashEntry->lRequestID,
                                  pHashEntry->nDataSize);
                    }
                    else
                    {
                        _ftprintf(fFile,
                                  _T("RequestID: %12i, Removed: %i, Size: %12i\n"),
                                  pHashEntry->lRequestID,
                                  pHashEntry->cRemovedFlag,
                                  pHashEntry->nDataSize);
                    }
                    ...
                    {
                        ulLeaksByte++; // If memory was allocated with zero bytes, then just
...
// MyAllocHook is Single-Threaded, that means the the calls are serialized in the calling
// function!
// Special case for VC 5
#if _MSC_VER <= 1100
static int MyAllocHook(int nAllocType,
                       void * pvData,
                       size_t nSize,
                       int nBlockUse,
                       long lRequest,
...
{
#endif
    static TCHAR * operation[] = {_T(""), _T("ALLOCATIONG"), _T("RE-ALLOCATING"), _T(
                                      "FREEING")};
    static TCHAR * blockType[] = {_T("Free"), _T("Normal"), _T("CRT"), _T("Ignore"), _T(
                                      "Client")};
    ...
    _ASSERT((nAllocType == _HOOK_ALLOC) || (nAllocType == _HOOK_REALLOC) ||
            (nAllocType == _HOOK_FREE));
    ...
    {
        _ftprintf(g_fFile,
                  _T("##### Memory operation: %s a %d-byte '%s' block (# %ld)"),
                  operation[nAllocType],
                  nSize,
                  blockType[_BLOCK_TYPE(nBlockUse)],
                  lRequest);
    ...
    if(nAllocType == _HOOK_FREE)  // freeing:
    {
        if(lRequest != 0) // RequestID was found
        {
            ...
            if(g_ulShowStackAtAlloc > 0)
            {
                if(bRet == FALSE)
                {
                    ...
                    _ftprintf(g_fFile,
                              _T(
                                  "###### RequestID not found in hash table for FREEING (%i)!\n"),
                              lRequest);
                }
        ...
        else
        {
            if(g_ulShowStackAtAlloc > 0)
            {
                ...
                _ftprintf(g_fFile,
                          _T("###### No valid RequestID for FREEING! (0x%X)\n"),
                          pvData);
            }
        }
    } // freeing
        ...
        if(_CrtIsValidHeapPointer(pvData)) // it is a valid Heap-Pointer
        {
            ...
            if(g_ulShowStackAtAlloc > 0)
            {
                if(bRet == FALSE)
                {
                    ...
                    _ftprintf(g_fFile,
                              _T(
                                  "###### RequestID not found in hash table for RE-ALLOCATING (%i)!\n"),
                              lReallocRequest);
                }
                else
                {
                    _ftprintf(g_fFile,
                              _T(
                                  "##### Implicit freeing because of re-allocation (# old: %ld, new: %ld)\n"),
                              lReallocRequest,
                              lRequest);
                }
            } // g_ulShowStackAtAlloc > 0
    ...
    if(MyGetCurrentThreadContext(hThread, &c) == 0)
    {
        ...
        {
            _ftprintf(g_fFile,
                      _T("###### Could not call 'GetThreadContext' successfully\n"));
        }
    ...
    if(nAllocType != _HOOK_FREE)
    {
        if(lRequest != 0) // Always a valid RequestID should be provided (see comments in the
                          // header)
        {
...
static bool GetModuleListPSAPI(ModuleList & modules,
                               DWORD pid,
                               HANDLE hProcess,
                               FILE * fLogFile)
{
    ...
    {
        _ftprintf(fLogFile,
                  _T("%lu: EPM failed, GetLastError = %lu\n"),
                  g_dwShowCount,
                  gle);
        goto cleanup;
    }
    ...
    {
        _ftprintf(fLogFile,
                  _T("%lu: More than %lu module handles. Huh?\n"),
                  g_dwShowCount,
                  lenof(hMods));
        goto cleanup;
    }
...
static bool GetModuleList(ModuleList & modules,
                          DWORD pid,
                          HANDLE hProcess,
                          FILE * fLogFile)
{
...
ULONG DeInitAllocCheck(void)
{
    ...
    {
#ifdef _DEBUG
        InterlockedIncrement(&g_lMallocCalled); // No deactivate MyAllocHook, because
                                                // StackWalker will allocate some memory)
...
void OnlyInstallUnhandeldExceptionFilter(eAllocCheckOutput eOutput)
{
    if(s_bUnhandledExeptionFilterSet == FALSE)
    {
        ...
        // set it again; WARNING: this will override the setting for a possible
        // AllocCheck-Setting
        g_CallstackOutputType = eOutput;
        // set global exception handler (for handling all unhandled exceptions)
        SetUnhandledExceptionFilter(CrashHandlerExceptionFilter);
        ...
        case EXCEPTION_ACCESS_VIOLATION:
            ...
            if(pExceptionRecord->NumberParameters == 2)
            {
                switch(pExceptionRecord->ExceptionInformation[0])
                {
                    case 0: // read attempt
                        _stprintf(szTemp,
                                  _T(" read attempt to address 0x%8.8X "),
                                  pExceptionRecord->ExceptionInformation[1]);
                        return szTemp;

                    case 1: // write attempt
                        _stprintf(szTemp,
                                  _T(" write attempt to address 0x%8.8X "),
                                  pExceptionRecord->ExceptionInformation[1]);
                        return szTemp;
...
DWORD StackwalkFilter(EXCEPTION_POINTERS * ep, DWORD status, LPCTSTR pszLogFile)
{
    ...
    {
        _ftprintf(fFile,
                  _T("<EXCEPTION code=\"%8.8X\" addr=\"%8.8X\" "),
                  ep->ExceptionRecord->ExceptionCode,
                  ep->ExceptionRecord->ExceptionAddress);
        WriteDateTime(fFile, TRUE);
        _ftprintf(fFile,
                  _T("code_desc=\"%s\" more_desc=\"%s\">\n"),
    ...
    {
        _ftprintf(fFile,
                  _T("######## EXCEPTION: 0x%8.8X at address: 0x%8.8X"),
                  ep->ExceptionRecord->ExceptionCode,
                  ep->ExceptionRecord->ExceptionAddress);
        _ftprintf(fFile,
                  _T(": %s %s\n"),
                        ...
                        HANDLE hSWProcess)
{
    ...
    HANDLE hProcess = GetCurrentProcess(); // hProcess normally comes from outside but we only
                                           // do the stackdump in our own process
    ...
    if(g_bInitialized == FALSE)
    {
        ...
        _ftprintf(fLogFile,
                  _T(
                      "%lu: Stackwalker not initialized (or was not able to initialize)!\n"),
                  g_dwShowCount);
        return;
    }
...
// Critical section begin...
    ...
    if(bFirstTime)
    {
        ...
        if(!pSI(hProcess, tt, false))
        {
            ...
            {
                _ftprintf(fLogFile,
                          _T("%lu: SymInitialize(): GetLastError = %lu\n"),
                          g_dwShowCount,
                          gle);
            }
    ...
    for(frameNum = 0; ; ++frameNum)
    {
        ...
        if(!pSW(imageType, hSWProcess, hThread, &s, NULL, ReadMemoryFunction, pSFTA,
                pSGMB, NULL))
        {
            break;
        }
        ...
        if(s.AddrPC.Offset == 0)
        {
            ...
            switch(g_CallstackOutputType)
            {
                ...
                case ACOutput_XML:
                    // TODO: ....
                    _ftprintf(fLogFile,
                              _T("<STACKENTRY decl=\"(-nosymbols- PC == 0)\"/>\n"));
                    break;
            }
        }
        else
        {
            ...
            if(!pSGSFA(hProcess, s.AddrPC.Offset, &offsetFromSymbol, pSym))
            {
                if(g_CallstackOutputType == ACOutput_Advanced)
                {
                    ...
                    {
                        _ftprintf(fLogFile,
                                  _T("   SymGetSymFromAddr(): GetLastError = %lu\n"),
                                  gle);
                    }
                    else
                    {
            ...
            //if (g_CallstackOutputType == ACOutput_XML)
            //  fprintf(fLogFile, "decl=\"%s\" decl_offset=\"%+ld\" ",
            // SimpleXMLEncode(undName).c_str(), (long) offsetFromSymbol);
                ...
                if(!pSGLFA(hProcess, s.AddrPC.Offset, &offsetFromLine, &Line))
                {
                    if((gle != 487) && (frameNum > 0)) // ignore error for first frame
                    {
                        if(g_CallstackOutputType == ACOutput_XML)
                        {
                            ...
                            bXMLTagWrote = TRUE;
                            fprintf(fLogFile,
                                    "decl=\"%s\" decl_offset=\"%+ld\" ",
                                    SimpleXMLEncode(undName).c_str(),
                                    (long)offsetFromSymbol);
                            _ftprintf(fLogFile,
                                      _T(
                                          "srcfile=\"SymGetLineFromAddr(): GetLastError = %lu\" "),
                                      gle);
                        }
                        else
                        {
                            _ftprintf(fLogFile,
                                      _T(
                                          "%lu: SymGetLineFromAddr(): GetLastError = %lu\n"),
                                      g_dwShowCount,
                                      gle);
                        }
                    }
                    ...
                    switch(g_CallstackOutputType)
                    {
                        ...
                        case ACOutput_XML:
                            ...
                            fprintf(fLogFile,
                                    ...
                                    SimpleXMLEncode(Line.FileName).c_str(),
                                    Line.LineNumber,
                                    offsetFromLine,
                                    undName);
                            break;
            ...
            if((g_CallstackOutputType == ACOutput_Advanced) ||
               (g_CallstackOutputType == ACOutput_XML))
            {
                if(!pSGMI(hProcess, s.AddrPC.Offset, &Module))
                {
                    ...
                    {
                        _ftprintf(fLogFile,
                                  _T("%lu: SymGetModuleInfo): GetLastError = %lu\n"),
                                  g_dwShowCount,
                                  gle);
                    }
                }
                else // got module info OK
                {
...
#endif
                        default:
                            _snprintf(ty, sizeof ty, "symtype=%ld",
                                      (long)Module.SymType);
                            break;
                    } // switch
                    ...
                    if(g_CallstackOutputType == ACOutput_Advanced)
                    {
                        ...
                        if(Module.SymType == SymNone) // Gebe nur aus, wenn keine Symbole
                                                      // vorhanden sind!
                        {
                            _ftprintf(fLogFile,
                                      _T("%lu:     Offset: 0x%8.8x\n"),
                                      g_dwShowCount,
                                      s.AddrPC.Offset);
                            fprintf(fLogFile,
                                    "%lu:     Sym:  type: %s, file: %s\n",
                                    g_dwShowCount,
                                    ty,
