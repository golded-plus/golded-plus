...
const char * get_charset(void)
{
    ...
    {
        cp++;
        /* Commented on reason: Non-standard charset values isn't used in unix locale, may be in
...
const char * get_dos_charset(const char * cpfrom)
{
    ...
    if(cp)
    {
        gsprintf(PRINTF_DECLARE_BUFFER(cpto), "CP%u", cp);
    }
    else
    {
        cpto[0] = '\0';
    ...
    {
        char * from, * to;
    } cpmap[] = {{"IBMPC",  "CP850"}, {"LATIN-1", "CP437" }, {"KOI8", "CP866"},
        {"KOI8-R", "CP866"}, {"KOI8-U",  "CP1125"}, {NULL,   NULL   }};
    int i;
