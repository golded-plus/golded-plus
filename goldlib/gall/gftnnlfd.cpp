...
{
    return namebrowse ? *((_FDGdx *)&block.node.name[node -
                                                     1]) :
           *((_FDGdx *)&block.node.addr[node - 1]);
}
...
} // ftn_frontdoor_nodelist_index::getstatus
...
void ftn_frontdoor_nodelist_index::fetchdata()
{
    ...
    if(infdnode)
    {
        int fd = ::sopen(AddPath(nlpath, "FDNODE.FDA"),
                         O_RDONLY | O_BINARY,
                         SH_DENYNO,
                         S_STDRD);
        ...
        if(fd != -1)
        {
            ...
            while(read(fd, &fda, sizeof(_FDFdn)) == sizeof(_FDFdn))
            {
                if(not fda.erased)
                {
                    if((fda.zone == data.addr.zone) and (fda.net ==
                                                         data.addr.net) and (fda.node ==
                                                                             data.addr.
                                                                             node) and (
                           fda.point == data.addr.point))
                    {
...
} // ftn_frontdoor_nodelist_index::fetchdata
...
bool ftn_frontdoor_nodelist_index::search()
{
    ...
    getnodedata();
    return exactmatch;
} // ftn_frontdoor_nodelist_index::search
...
bool ftn_frontdoor_nodelist_index::prevnode()
{
    ...
    return true;
} // ftn_frontdoor_nodelist_index::prevnode
...
bool ftn_frontdoor_nodelist_index::nextnode()
{
    ...
    else if(node >= block.info.nodes)
    {
        ...
        if(depth > 0)
        {
            if((stack[depth - 1].node ==
                stack[depth - 1].maxnodes) and (stack[depth - 1].block_num == blockno))
            {
                return false;
            }
    ...
    return true;
} // ftn_frontdoor_nodelist_index::nextnode
...
bool ftn_frontdoor_nodelist_index::open()
{
    ...
    fdfd = ::sopen(AddPath(nlpath, "NODELIST.FDX"),
                   O_RDONLY | O_BINARY,
                   SH_DENYNO,
                   S_STDRD);
    ...
    ufd = ::sopen(AddPath(nlpath, "USERLIST.FDX"),
                  O_RDONLY | O_BINARY,
                  SH_DENYNO,
                  S_STDRD);
    ...
    pfd = ::sopen(AddPath(nlpath, is_intermail ? "IMNET.PVT" : "FDNET.PVT"),
                  O_RDONLY | O_BINARY,
                  SH_DENYNO,
                  S_STDRD);
    // Failing to open the point nodelist is not an error, since there may not be one!
    ppfd = ::sopen(AddPath(nlpath, is_intermail ? "IMPOINT.PVT" : "FDPOINT.PVT"),
                   O_RDONLY | O_BINARY,
                   SH_DENYNO,
                   S_STDRD);
    isopen = true;
    return true;
} // ftn_frontdoor_nodelist_index::open
...
void ftn_frontdoor_nodelist_index::close()
{
    ...
    ufd    = -1;
    isopen = false;
} // ftn_frontdoor_nodelist_index::close
...
bool ftn_frontdoor_nodelist_index::find(const char * lookup_name)
{
    ...
    return search();
} // ftn_frontdoor_nodelist_index::find
