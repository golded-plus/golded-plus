...
{
    return (vattr)(((attr >> 4) & 0x07) | ((attr << 4) & 0x70) | (attr & 0x80) |
                   (attr & 0x08));
}
...
chtype gvid_tcpr(vchar chr)
{
    ...
    {(chtype)' ',            (chtype)'@',        (chtype)'@',                (chtype)'x',
     (chtype)ACS_DIAMOND,
     (chtype)'x',            (chtype)'x',        ACS_BULLET,                 ACS_BULLET,
     ACS_BULLET,
     ACS_BULLET,
     (chtype)'x',            (chtype)'x',        (chtype)'x',                (chtype)'x',
     ACS_LANTERN,
     (chtype)ACS_RARROW,     ACS_LARROW,         (chtype)'x',                (chtype)'!',
     (chtype)'x',
     (chtype)'x',            ACS_S1,             (chtype)'x',                ACS_UARROW,
     ACS_DARROW,
     ACS_LARROW,
     (chtype)ACS_RARROW,     (chtype)'x',        (chtype)'x',                ACS_UARROW,
     ACS_DARROW};
...
vsavebuf * vsave(int srow, int scol, int erow, int ecol)
{
    ...
    vsavebuf * sbuf =
        (vsavebuf *)throw_xmalloc(
            sizeof(vsavebuf) + (erow - srow + 1) * (ecol - scol + 1) * sizeof(vatch));
...
void vcurset(int sline, int eline)
{
    ...
    if(eline == 0)
    {
        ...
        sline =
            ((gvid->adapter >= V_HGC) and (gvid->adapter <= V_INCOLOR)) ? 0x3F : _dvhide;
    }
