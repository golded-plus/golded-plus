...
#endif // if defined (__MSDOS__) || defined (__UNIX__)
//  ------------------------------------------------------------------
...
//  ------------------------------------------------------------------
static int _atr_to_ans[8] = {0,  // BLACK    0
                             4, // BLUE     1
                             2, // GREEN    2
                             6, // CYAN     3
                             1, // RED      4
...
{
    return (vattr)(((attr >> 4) & 0x07) | ((attr <<
                                            4) & 0x70) | (attr & 0x80) | (attr & 0x08));
}
...
#define _THUNK_PTR_SIZE_OK(ptr, \
                           size) (((ULONG)(ptr) & ~0xffff) == \
                                  (((ULONG)(ptr) + (size) - 1) & ~0xffff))
#endif
...
static USHORT VioWrtCharStrAtt_(PCCH str,
                                USHORT cb,
                                USHORT row,
                                USHORT col,
                                PBYTE attr,
...
{
    const chtype gvid_cpr[] = {(chtype)' ',         (chtype)'@',
                               (chtype)'@',         (chtype)'x',
                               (chtype)ACS_DIAMOND, (chtype)'x',
                               (chtype)'x',         ACS_BULLET,
                               ACS_BULLET,          ACS_BULLET,
                               ACS_BULLET,          (chtype)'x',
                               (chtype)'x',         (chtype)'x',
                               (chtype)'x',         ACS_LANTERN,
                               (chtype)ACS_RARROW,  ACS_LARROW,
                               (chtype)'x',         (chtype)'!',
                               (chtype)'x',         (chtype)'x',
                               ACS_S1,              (chtype)'x',
                               ACS_UARROW,          ACS_DARROW,
                               ACS_LARROW,
                               (chtype)ACS_RARROW,  (chtype)'x',
                               (chtype)'x',         ACS_UARROW,
                               ACS_DARROW};
...
void vputws(int row, int col, vatch * buf, uint len)
{
    ...
    if(gvid->isdma())
    {
        gdmacpy(_dos_ds, (gdma)gdmaptr(col, row), _my_ds(), (gdma)buf,
                len * sizeof(word));
    }
    else if(gvid->isbios() or gvid->iscga())
    {
...
void vputns(int row, int col, vattr atr, const char * str, uint width)
{
    ...
    uint len = strlen(str);
    VioWrtCharStrAtt((PCCH)str,
                     (USHORT)minimum_of_two(len, width),
                     (USHORT)row,
                     (USHORT)col,
                     (PBYTE)&atr,
                     0);
    ...
    if(width > len)
    {
        ...
        VioWrtNCell((BYTE *)&filler, (USHORT)(width - len), (USHORT)row,
                    (USHORT)(col + len), 0);
    }
...
static void _vscroll(int srow, int scol, int erow, int ecol, int atr, int lines)
{
    ...
    if(lines > 0)
    {
        while(lines--)
        {
            ...
            while(nrow++ < erow)
            {
                gdmacpy(_dos_ds, (gdma)scrptr, _dos_ds,
                        (gdma)(scrptr + ATTRSIZE * gvid->numcols), l * sizeof(word));
...
#endif // if (defined (__MSDOS__) || defined (__UNIX__)) && !defined (__USE_NCURSES__)
//  ------------------------------------------------------------------
//  Scroll screen area
...
vsavebuf * vsave(int srow, int scol, int erow, int ecol)
{
    ...
    vsavebuf * sbuf = (vsavebuf *)throw_xmalloc(sizeof(vsavebuf) + (erow - srow + 1) *
                                                (ecol - scol + 1) * sizeof(vatch));
...
void vcurset(int sline, int eline)
{
    ...
    if(eline == 0)
    {
        ...
        sline = ((gvid->adapter >= V_HGC) and (gvid->adapter <=
                                               V_INCOLOR)) ? 0x3F : _dvhide;
    }
