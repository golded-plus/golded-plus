...
int wshadow(vattr attr)
{
    ...
    vatch * wsbuf =
        (vatch *)throw_xmalloc((((erow - srow) * sizeof(vatch)) + ecol - scol + 1) *
                               sizeof(vatch));
...
int whide()
{
    ...
    // save active window
    p =
        vsave(gwin.active->srow, gwin.active->scol, gwin.active->erow,
              gwin.active->ecol);
...
static void swap_contents(vatch * pfound, vatch * pcurr, int shadow)
{
    ...
    chat =
        ((vgattr(temp) & BLINK) and shadow) ? vsattr(*pcurr,
    ...
    if(str)
    {
        ...
        if(gwin.active->border)
        {
            ...
            switch(tpos & ~TBOTTOM)
            {
                ...
                case TCENTER:
                    start =
                        (len > (width - 2)) ? left : (((width / 2) + left) - (len / 2));
                    break;
...
{
    vattr attr =
        (gwin.active->sbattr == DEFATTR) ? gwin.active->battr : gwin.active->sbattr;
