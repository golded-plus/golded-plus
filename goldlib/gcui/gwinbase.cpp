...
{
    return (wrow < 0) or (wrow >
                          ((gwin.active->erow - gwin.active->border) -
                           (gwin.active->srow + gwin.active->border)));
}

static GOLD_INLINE int _wchkcol(int wcol)
{
    return (wcol < 0) or (wcol >
                          ((gwin.active->ecol - gwin.active->border) -
                           (gwin.active->scol + gwin.active->border)));
}
...
{
    return _wchkcoord(wsrow, wscol) or _wchkcoord(werow,
                                                  wecol) or (wsrow > werow) or (wscol >
                                                                                wecol);
}
...
int wgotoxy(int wrow, int wcol)
{
    ...
    const int & col = gwin.active->column = gwin.active->scol + wcol +
                                            gwin.active->border;
    // set cursor location
    vposset(row, col);
...
int wshadow(vattr attr)
{
    ...
    vatch * wsbuf = (vatch *)throw_xmalloc((((erow - srow) * sizeof(vatch)) + ecol -
                                            scol + 1) * sizeof(vatch));
...
int wscroll(int count, int direction)
{
    ...
    vscroll(gwin.active->srow + border,
...
int wscrollbox(int wsrow, int wscol, int werow, int wecol, int count, int direction)
{
    // check for window border
    const int & border = gwin.active->border;
    vscroll(gwin.active->srow + wsrow + border,
...
int wprintc(int wrow, int wcol, vattr atr, vchar chr)
{
    ...
    vputc(wrow + gwin.active->srow + gwin.active->border,
            ...
            const char * str)
{
    ...
    const int & border = gwin.active->border;
    vputs(gwin.active->srow + wrow + border, gwin.active->scol + wcol + border, attr,
          str);
    return gwin.werrno = W_NOERROR;
}
...
int wprints_box(int wrow, int wcol, vattr attr, const char * str)
{
    ...
    const int & border = gwin.active->border;
    vputs_box(gwin.active->srow + wrow + border,
              gwin.active->scol + wcol + border,
              attr,
              str);
    return gwin.werrno = W_NOERROR;
}
...
int wprintvs(int wrow, int wcol, vattr attr, const vchar * str)
{
    ...
    const int & border = gwin.active->border;
    vputvs(gwin.active->srow + wrow + border,
           gwin.active->scol + wcol + border,
           attr,
           str);
    return gwin.werrno = W_NOERROR;
...
{
    const int & border = gwin.active->border;
    vputx(gwin.active->srow + wrow + border,
          gwin.active->scol + wcol + border,
          attr,
          chr,
          len);
...
{
    const int & border = gwin.active->border;
    vputy(gwin.active->srow + wrow + border,
          gwin.active->scol + wcol + border,
          attr,
          chr,
          len);
...
int whide()
{
    ...
    // save active window
    p = vsave(gwin.active->srow, gwin.active->scol, gwin.active->erow,
              gwin.active->ecol);
...
{
    return (__crow >= __curr->srow and __crow <= __curr->erow and __ccol >=
            __curr->scol and __ccol <= __curr->ecol) ? YES : NO;
}
...
{
    return wrec->wbuf->data + ((__crow - wrec->srow) * (wrec->ecol - wrec->scol + 1)) +
           (__ccol - wrec->scol);
}
...
static void swap_contents(vatch * pfound, vatch * pcurr, int shadow)
{
    ...
    chat = ((vgattr(temp) & BLINK) and shadow) ? vsattr(*pcurr,
                                                        vgattr(*pcurr) | BLINK) : *pcurr;
            ...
            for(__ccol = startcol; __ccol <= stopcol; __ccol++)
            {
                ...
                for(__curr = __found->next; __curr != NULL; __curr = __curr->next)
                {
                    ...
                    if(__curr->wsbuf != NULL)
                    {
                        ...
                        if(rshadow_blocking())
                        {
                            swap_contents(calc_rshadow(__found), calc_rshadow(__curr),
                                          3);
                            break;
                        }
                        ...
                        if(bshadow_blocking())
                        {
                            swap_contents(calc_rshadow(__found), calc_bshadow(__curr),
                                          3);
                            break;
                        }
                    }
    ...
    *pcurr = vcatch(*__p, __gattr);
    ...
    if(shadow)
    {
        if(__curr->next == NULL)
        {
            vputc(__crow,
                  __ccol,
                  __gattr & BLINK ? (__curr->wsattr | BLINK) : __curr->wsattr,
                  *__p);
        }
        else
        {
...
int wwprintc(int whandle, int wrow, int wcol, vattr attr, const vchar chr)
{
    ...
    // display character
    vputc(found->srow + wrow + found->border,
          found->scol + wcol + found->border,
          attr,
          chr);
    // return to caller
    return gwin.werrno = W_NOERROR;
...
int wwprintstr(int whandle, int wrow, int wcol, vattr attr, const char * str)
{
    ...
    // display string
    vputs(found->srow + wrow + found->border,
          found->scol + wcol + found->border,
          attr,
          str);
    // return to caller
...
int wborder(int btype)
{
    ...
    if((gwin.active->row == gwin.active->srow)   or (gwin.active->row ==
                                                     gwin.active->erow)    or (gwin.
                                                                               active->
                                                                               column ==
                                                                               gwin.
                                                                               active->
...
int wfill(int wsrow, int wscol, int werow, int wecol, vchar chr, vattr atr)
{
    ...
    vfill(gwin.active->srow + wsrow + border,
          gwin.active->scol + wscol + border,
          gwin.active->srow + werow + border,
          gwin.active->scol + wecol + border,
          chr,
...
int wmessage(const char * str, int border, int leftofs, vattr attr)
{
    ...
    // display string
    vputs(border ? gwin.active->erow : gwin.active->srow,
          gwin.active->scol + leftofs,
          attr,
          str);
    // return normally
    return gwin.werrno = W_NOERROR;
...
int wtitle(const char * str, int tpos, vattr tattr)
{
    ...
    if(str == NULL or gwin.active->title != NULL)
    {
        if(gwin.active->border)
        {
            vputx(((tpos & TBOTTOM) ? gwin.active->erow : gwin.active->srow),
                  gwin.active->scol + 1,
    ...
    if(str)
    {
        ...
        if(gwin.active->border)
        {
            ...
            switch(tpos & ~TBOTTOM)
            {
                ...
                case TCENTER:
                    start = (len >
                             (width - 2)) ? left : (((width / 2) + left) - (len / 2));
                    break;
            ...
            *(p + width) = NUL;
            ...
            vputs((tpos & TBOTTOM) ? gwin.active->erow : gwin.active->srow,
                  start,
                  tattr,
                  p);
...
{
    vattr attr                 = (gwin.active->sbattr ==
                                  DEFATTR) ? gwin.active->battr : gwin.active->sbattr;
    ...
    {
        visiblelen = (gwin.active->erow - (gwin.active->srow + sadd)) + 1 -
                     (gwin.active->border ? 2 : 0);
    }
    else
    {
        visiblelen = (gwin.active->ecol - (gwin.active->scol + sadd)) + 1 -
                     (gwin.active->border ? 2 : 0) - 2;
    }
    ...
    if(orientation == W_VERT)
    {
        ...
        vputc((sadd++) + gwin.active->srow + gwin.active->border,
              gwin.active->ecol,
              invattr | ACSET,
              arrowupchar);
    ...
    else
    {
        ...
        vputc(gwin.active->erow,
              (sadd++) + gwin.active->scol + gwin.active->border,
              invattr | ACSET,
              arrowleftchar);
