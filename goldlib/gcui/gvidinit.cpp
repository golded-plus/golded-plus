...
void GVid::init()
{
  ...
  #elif defined (__UNIX__)
    dmaptr =
        (gdma)throw_xcalloc((orig.screen.rows + 1) * orig.screen.columns, sizeof(word));
    dmadir = NULL;
  #else
...
void GVid::detectinfo(GVidInfo * _info)
{
    ...
    if(r <= 0 or c <= 0)
    {
        ...
        do
        {
            if((ioctl(1, TIOCGSIZE,
                      &sz) ==
                0) or (ioctl(0, TIOCGSIZE, &sz) == 0) or (ioctl(2, TIOCGSIZE, &sz) == 0))
            {
                c = (int)sz.ts_cols;
                r = (int)sz.ts_lines;
                break;
    ...
    if(r <= 0 or c <= 0)
    {
        ...
        do
        {
            if((ioctl(1, TIOCGWINSZ,
                      &wind_struct) ==
                0) or (ioctl(0, TIOCGWINSZ,
                             &wind_struct) ==
                       0) or (ioctl(2, TIOCGWINSZ, &wind_struct) == 0))
            {
