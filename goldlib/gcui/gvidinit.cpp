...
void GVid::init()
{
    ...
    dmaptr = dmadir = NULL;
#elif defined (__UNIX__)
    dmaptr = (gdma)throw_xcalloc((orig.screen.rows + 1) * orig.screen.columns,
                                 sizeof(word));
    dmadir = NULL;
#else
...
void GVid::setcolorpairs(bool enabletransparent)
{
#if defined (__USE_NCURSES__)
    /* init colors */
    short mycolors[] = {COLOR_BLACK,   COLOR_BLUE,                   COLOR_GREEN,
    ...
    for(int i = 1; i < 64 and i < COLOR_PAIRS; i++)
    {
        ...
        init_pair(i,
                  mycolors[(~i) & 0x07],
                  ((bg == COLOR_BLACK) && enabletransparent) ? -1 : bg);
    }
#endif
}
...
void GVid::detectinfo(GVidInfo * _info)
{
    ...
    if(r <= 0 or c <= 0)
    {
        ...
        do
        {
            if((ioctl(1, TIOCGSIZE, &sz) == 0) or (ioctl(0, TIOCGSIZE,
                                                         &sz) == 0) or (ioctl(2,
                                                                              TIOCGSIZE,
                                                                              &sz) == 0))
            {
    ...
    if(r <= 0 or c <= 0)
    {
        ...
        do
        {
            if((ioctl(1, TIOCGWINSZ, &wind_struct) == 0) or (ioctl(0,
                                                                   TIOCGWINSZ,
                                                                   &wind_struct) ==
                                                             0) or (ioctl(2,
                                                                          TIOCGWINSZ,
