...
static _item_t *(*mnu_funcs[4])(_item_t *) = {left_item, right_item, up_item, down_item};
//  ------------------------------------------------------------------
...
static void disp_item(_item_t * witem, int bar)
{
    ...
    if(width > (gvid->numcols - 2))
    {
        width = gvid->numcols - 2;
    }
    ...
    if(witem->fmask & M_SEPAR)
    {
        ...
        if(border)
        {
            *ptr++ = vcatch(_box_table(btype, 9), attr);
        }
        ...
        if(border)
        {
            *ptr = vcatch(_box_table(btype, 10), attr);
        }
...
static _item_t * first_item(void)
{
    ...
    {
        if((temp->wrow <
            best->wrow) or ((temp->wrow == best->wrow) and (temp->wcol < best->wcol)))
        {
            best = temp;
        }
    }
...
static void free_menu(_menu_t * wmenu)
{
    ...
    {
        if(wmenu->item->child != NULL)
        {
            free_menu((_menu_t *)wmenu->item->child);
        }
...
static _item_t * last_item(void)
{
    ...
    {
        if((temp->wrow >
            best->wrow) or ((temp->wrow == best->wrow) and (temp->wcol > bcol)))
        {
            best = temp;
            bcol = best->wcol;
        }
...
static _item_t * goto_item(_item_t * citem, int which)
{
    ...
    else if(which == ITM_LS)
    {
        item = last_item();
    }
    else
    {
        item = (*mnu_funcs[which])(citem);
    }
        ...
        while(!kbxhit() and gkbd.kbuf == NULL)
        {
            ...
            if(gmou.Count())
            {
                ...
                if(item == NULL)
                {
                    ...
                    {
                        item = mouse_on_item(gwin.cmenu->parent,
                                             gmou.Row(),
                                             gmou.Column());
             ...
             vattr barattr)
{
    ...
    if((gwin.cmenu = gwin.cmenu->parent) == NULL)
    {
        gwin.cmenu = gwin.menu;
    }
...
int wmenuget()
{
    ...
    if(!gwin.cmenu->usecurr)
    {
        ...
        if(!wopen(gwin.cmenu->srow, gwin.cmenu->scol, gwin.cmenu->erow, gwin.cmenu->ecol,
                  gwin.cmenu->btype, gwin.cmenu->battr, gwin.cmenu->wattr,
                  gwin.cmenu->sbattr,
                  gwin.cmenu->loattr))
        {
            return -1;
    ...
    if(citem == NULL)
    {
        ...
        if((citem == NULL) or (citem->fmask & M_NOSEL))
        {
            citem = first_item();
        }
    }
...
int wmenubegc()
{
    ...
    if(wmenubeg(gwin.active->srow, gwin.active->scol, gwin.active->erow,
                gwin.active->ecol, gwin.active->btype, gwin.active->battr,
                gwin.active->wattr, NULL))
    {
        return gwin.werrno;
    }
