...
typedef enum
{
    ...
    /* Set the following two-byte relative address to the
       ...
       bytes of number.  */
    set_number_at, wordchar, /* Matches any word-constituent character.  */
...
/* This table gives an error message for each of the error codes listed
   ...
   but why not be nice?  */
static const char * re_error_msgid[] = {gettext_noop("Success"),   /* REG_NOERROR */
        ...
        switch(c)
        {
            ...
            case '[':
            {
                ...
                for( ;; )
                {
                    ...
                    {
                        reg_errcode_t ret = compile_range(&p, pend, translate, syntax,
                                                          b);
...
# endif
                            ...
                            had_char_class = true;
#else // if defined _LIBC || WIDE_CHAR_SUPPORT
                            int ch;
                ...
                switch(c)
                {
                    ...
                    handle_close:
...

                        {
                            ...
                            {
                                unsigned char * inner_group_loc = bufp->buffer +
                                                                  COMPILE_STACK_TOP.
                                                                  inner_group_offset;
            ...
            normal_char:
                ...
                if(!pending_exact
                   ...
                   || *p == '*' || *p == '^' ||
                   ((syntax & RE_BK_PLUS_QM) ? *p == '\\' &&
                    (p[1] == '+' || p[1] == '?') : (*p == '+' || *p == '?')) ||
                   ((syntax & RE_INTERVALS) &&
                    ((syntax & RE_NO_BK_BRACES) ? *p == '{' : (p[0] == '\\' &&
                                                               p[1] == '{'))))
                {
...
# ifdef emacs
            ...
            if(!fail_stack.stack)
            {
                fail_stack.stack = (fail_stack_elt_t *)xmalloc(fail_stack.size *
                                                               sizeof(fail_stack_elt_t));
            }
            else
            {
...
int re_compile_fastmap(struct re_pattern_buffer * bufp)
{
    ...
    while(true)
    {
        ...
        switch(SWITCH_ENUM_CAST((re_opcode_t)*p++))
        {
            ...
            handle_on_failure_jump: EXTRACT_NUMBER_AND_INCR(j, p);
                ...
                int stop)
{
    ...
    if(bufp->used > 0 && range > 0 && ((re_opcode_t)bufp->buffer[0] == begbuf
                                       /* `begline' is like `begbuf' if it cannot match at
                                          newlines.  */
    ...
    for( ;; )
    {
        ...
        if(fastmap && startpos < total_size && !bufp->can_be_null)
        {
            ...
            else              /* Searching backwards.  */
            {
                char c = (size1 == 0 ||
                          startpos >=
                          size1 ? string2[startpos - size1] : string1[startpos]);
...
#endif
#endif /* not emacs */
static bool group_match_null_string_p(unsigned char ** p,
                                      unsigned char * end,
                                      register_info_type * reg_info);
static bool alt_match_null_string_p(unsigned char * p,
                                    unsigned char * end,
                                    register_info_type * reg_info);
    ...
    for( ;; )
    {
        ...
        succeed_label:
            ...
            if(regs && !bufp->no_sub)
            {
                ...
                {
                    regs->start[0] = pos;
                    regs->end[0]   = (MATCHING_IN_FIRST_STRING ? ((regoff_t)(d -
                                                                             string1)) :
        ...
        switch(SWITCH_ENUM_CAST((re_opcode_t)*p++))
        {
            ...
            case start_memory:
                ...
                old_regstart[*p] =
                    REG_MATCH_NULL_STRING_P(reg_info[*p]) ? REG_UNSET(regstart[*p]) ? d :
                    regstart[*p] : regstart[*p];
                regstart[*p]            = d;
            ...
            case stop_memory:
                /* We need to save the string position the last time we were at
                   ...
                   the string in case this attempt to match fails.  */
                old_regend[*p] =
                    REG_MATCH_NULL_STRING_P(reg_info[*p]) ? REG_UNSET(regend[*p]) ? d :
                    regend[*p] : regend[*p];
                regend[*p] = d;
            ...
            case duplicate:
            {
                ...
                for( ;; )
                {
                    ...
                    /* Compare that many; failure if mismatch, else move
                       past them.  */
                    if(translate ? bcmp_translate(d, d2, mcnt, translate) : memcmp(d, d2,
                                                                                   mcnt))
                    {
                        goto fail;
                    }
            ...
            case on_failure_jump:
            on_failure: EXTRACT_NUMBER_AND_INCR(mcnt, p);
