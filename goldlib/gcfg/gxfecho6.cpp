...
void gareafile::ReadFastecho142(int fh)
{
    ...
    OriginLines * origin = (OriginLines *)throw_calloc(cfg->OriginCnt,
                                                       sizeof(OriginLines));
...
void gareafile::ReadFastecho(char * tag)
{
    ...
    *file = NUL;
    ...
    if(fh != -1)
    {
        ...
        {
            STD_PRINTNL("* Error: FastEcho system file revision level " << revision <<
                        " is not supported - Skipping.");
        }

        close(fh);
    }
} // gareafile::ReadFastecho
