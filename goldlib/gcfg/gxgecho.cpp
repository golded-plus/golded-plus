        ...
        if(fp)
        {
            ...
            if(sysrev == GE_THISREV)
            {
                ...
                if(ge_version >= 102)
                {
                    ...
                    {
                        STD_PRINTNL(
                            "* Warning - FTS-1 format is used for *.MSG. For better compatibility set this");
                        STD_PRINTNL(
                            "* in GSETUP: Miscellanous->GEcho Options->MSG compatibilty = Opus (not Fido).");
                        STD_PRINTNL(
                            "* To disable this warning, set FIDOMSGTYPE FTS1 in your GoldED setup.");
                    }
                }
                ...
                // Bad msgs
                uint _badboard = (ge_version >=
                                  110) ? gesetup->badarea : gesetup->oldbadboard;
                ...
                if(ge_version < 120)
                {
                    //uint _dupboard = (ge_version >= 110) ? gesetup->duparea :
                    // gesetup->olddupboard;
                    uint _dupboard = (ge_version >=
                                      110) ? gesetup->reserved5 : gesetup->olddupboard;
                ...
                // Received msgs
                uint _rcvdboard = (ge_version >=
                                   110) ? gesetup->rcvdarea : gesetup->oldrcvdboard;
                ...
                // Sent msgs
                uint _sentboard = (ge_version >=
                                   110) ? gesetup->sentarea : gesetup->oldsentboard;
                ...
                {
                    int u = USERS - n - 1;
                    uint _persmailboard = (ge_version >=
                                           110) ? gesetup->persmailarea[u] :
                                          gesetup->oldpersmailboard[u];
                ...
                {
                    int u           = USERS - n - 1;
                    uint _userboard = (ge_version >=
                                       110) ? gesetup->userarea[u] :
                                      gesetup->olduserboard[u];
                ...
                {
                    int a          = _AKAS - n - 1;
                    uint _akaboard = (ge_version >=
                                      110) ? gesetup->akaarea[a] :
                                     gesetup->oldakaboard[a];
                ...
                if(fp)
                {
                    ...
                    uint arearecsize = (ahdr.maxconnections * sizeof(CONNECTION)) +
                                       ahdr.recsize;
                    ...
                    if(area)
                    {
                        while(fread(area, arearecsize, 1, fp) == 1)
                        {
                            ...
                            {
                                memset(((byte *)area) + ahdr.recsize,
                                       0,
                                       arearecsize - ahdr.recsize);
                                aa.reset();
                                    ...
                                    default:
                                        ...
                                        if(area->areanumber and area->areanumber < 201)
                                        {
                                            ...
                                            aa.board    = area->areanumber;
                                                 ...
                                                 SDM) and not strblank(area->path))
                                        {
                                            aa.basetype = _fidomsgtype;
                                            aa.setpath(area->path);
                                        }
                                } // switch
                                aa.groupid = area->group + ((ge_version >=
                                                             120) ? 0x8001 : 0);
                                ...
                                aa.setorigin(area->originlinenr ?
                                             gesetup->originline[area->originlinenr -
                                                                 1] : area->originline);
                                AddNewArea(aa);
                            }
                        }
            ...
            {
                STD_PRINTNL("* Error: GEcho system file revision level " << sysrev <<
