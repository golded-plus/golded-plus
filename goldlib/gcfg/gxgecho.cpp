    ...
    if(gesetup)
    {
        ...
        if(fp)
        {
            ...
            if(sysrev == GE_THISREV)
            {
                ...
                // Bad msgs
                uint _badboard =
                    (ge_version >= 110) ? gesetup->badarea : gesetup->oldbadboard;
                ...
                if(ge_version < 120)
                {
                    ...
                    // gesetup->olddupboard;
                    uint _dupboard =
                        (ge_version >= 110) ? gesetup->reserved5 : gesetup->olddupboard;
                ...
                // Received msgs
                uint _rcvdboard =
                    (ge_version >= 110) ? gesetup->rcvdarea : gesetup->oldrcvdboard;
                ...
                // Sent msgs
                uint _sentboard =
                    (ge_version >= 110) ? gesetup->sentarea : gesetup->oldsentboard;
                ...
                for(n = 0; n < USERS; n++)
                {
                    ...
                    uint _persmailboard =
                        (ge_version >=
                         110) ? gesetup->persmailarea[u] : gesetup->oldpersmailboard[u];
                ...
                for(n = 0; n < USERS; n++)
                {
                    ...
                    uint _userboard =
                        (ge_version >=
                         110) ? gesetup->userarea[u] : gesetup->olduserboard[u];
                ...
                for(n = 0; n < _AKAS; n++)
                {
                    ...
                    uint _akaboard =
                        (ge_version >=
                         110) ? gesetup->akaarea[a] : gesetup->oldakaboard[a];
                    ...
                    if(area)
                    {
                        while(fread(area, arearecsize, 1, fp) == 1)
                        {
                            if(not (area->options & (PASSTHRU | REMOVED)))
                            {
                                ...
                                aa.groupid = area->group +
                                             ((ge_version >= 120) ? 0x8001 : 0);
                                ...
                                aa.setorigin(area->originlinenr ? gesetup->originline[
                                                 area->originlinenr -
                                                 1] : area->originline);
                                AddNewArea(aa);
                            }
            ...
            {
                STD_PRINTNL(
                    "* Error: GEcho system file revision level " << sysrev <<
