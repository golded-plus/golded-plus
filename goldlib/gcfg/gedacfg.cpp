...
{
    desclist = (DescList **)throw_reallox(desclist,
                                          (descs + 1),
                                          sizeof(DescList *),
                                          50);
...
{
    echolist = (EchoList **)throw_reallox(echolist,
                                          (echos + 1),
                                          sizeof(EchoList *),
                                          50);
...
void gareafile::GetAreasBBS(char * name, char * origin, char * options)
{
    ...
    if(fp.isopen())
    {
        ...
        while(fp.Fgets(buf, 255))
        {
            ...
            if(*ptr != ';' and * ptr != '%' and * ptr != '-' and * ptr !=
               '#' and * ptr !=
               '@' and strnicmp(ptr, "P ", 2) and strnicmp(ptr, "PASSTHRU", 8) and * ptr)
            {
                // Check for comment
                desc = strchr(ptr, ';');
...
bool gareafile::ReadAreafile(word crc, char * parameters)
{
    ...
    switch(crc)
    {
        ...
        case CRC_D_BRIDGE:
        case CRC_DBRIDGE:
            ReadDBridge(parameters);
            break;

#endif
#ifndef GCFG_NODUTCHIE
        case CRC_DUTCHIE:
            ReadDutchie(parameters);
            break;
        ...
        case CRC_FIDOPCB:
            ReadFidoPCB(parameters);
            break;

#endif
#ifndef GCFG_NOFMAIL
        case CRC_FMAIL:
            ReadFMail(parameters);
            break;
...
#ifndef GCFG_NOPCB
        case CRC_PCBOARD:
            ReadPCBoard(parameters);
            break;

#endif
#ifndef GCFG_NOPORTAL
        case CRC_PORTAL:
            ReadPortal(parameters);
            break;

#endif
#ifndef GCFG_NOPROBOARD
        case CRC_PROBOARD:
            ReadProBoard(parameters);
            break;
        ...
        case CRC_RA_ECHO:
        case CRC_RAECHO:
            ReadRaEcho(parameters);
            break;

#endif
#ifndef GCFG_NORA
        case CRC_REMOTEACCESS:
            ReadRemoteAccess(parameters);
            break;
...
#ifndef GCFG_NOTIMED
        case CRC_TIMED:
            ReadTimed(parameters);
            break;

#endif
#ifndef GCFG_NOTOSSCAN
        case CRC_TOSSCAN:
            ReadTosScan(parameters);
            break;

#endif
#ifndef GCFG_NOWATERGATE
        case CRC_WATERGATE:
            ReadWtrGte(parameters);
            break;

#endif
#ifndef GCFG_NOWMAIL
        case CRC_WMAIL:
            ReadWMail(parameters);
            break;

#endif
#ifndef GCFG_NOXMAIL
        case CRC_XMAIL:
            ReadXMail(parameters);
            break;
