...
void gareafile::ReadSuperBBS(char * tag)
{
    ...
    throw_new(sconfig);
    ConfigRecordP config = new ConfigRecordT;
    throw_new(config);
    MsgBoardRecordP boards = new MsgBoardRecordT;
    throw_new(boards);
    *sbpath = NUL;
    *origin = NUL;
    ptr     = getenv("SUPERBBS");
    ...
    if(fp)
    {
        ...
        if(fp)
        {
                ...
                if(fp)
                {
                    ...
                    for(int n = 0; n < 200; n++)
                    {
                        ...
                        if(*boards->Name)
                        {
                            ...
                            aa.setorigin(*config->OriginLine ? config->OriginLine :
                                         origin);
            ...
            {
                STD_PRINTNL("* Error: Unsupported version of SuperBBS: " <<
                            (word)(sconfig->VersionNumber >> 8) << '.' <<
                            (word)(sconfig->VersionNumber & 0xFF));
            }
        }
    }
