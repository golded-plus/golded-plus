    ...
    if(fexist(file))
    {
        ...
        if(fp)
        {
            ...
            while(fread(&msgsys, sizeof(_msgsys), 1, fp) == 1)
            {
                ...
                aa.type     = ((msgsys.Attrib & SYSMAIL) ? GMB_NET : (msgsys.Attrib &
                                                                      _ECHOMAIL) ?
                               GMB_ECHO : GMB_LOCAL);
                aa.attr     = ((msgsys.Attrib & SYSMAIL) ? attribsnet : (msgsys.Attrib &
                                                                         _ECHOMAIL) ?
                               attribsecho : attribslocal);
            ...
            while((de = d.nextentry("system*.dat", true)) != NULL)
            {
                ...
                if(fp)
                {
                    ...
                    if(*sysdat.msgpath and * sysdat.msgtitle)
                    {
                        ...
                        aa.type = ((sysdat.attrib & SYSMAIL) ? GMB_NET : (sysdat.attrib &
                                                                          _ECHOMAIL) ?
                                   GMB_ECHO : GMB_LOCAL);
                        aa.attr = ((sysdat.attrib &
                                    SYSMAIL) ? attribsnet : (sysdat.attrib &
                                                             _ECHOMAIL) ? attribsecho :
                                   attribslocal);
                        AddNewArea(aa);
                    }
