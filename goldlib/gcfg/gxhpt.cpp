...
void gareafile::ReadHPTFile(char * path, char * file, char * origin, int group)
{
            ...
            if(not s.empty())
            {
                ...
                switch(strCrc16(key))
                {
                    ...
                    case CRC_VERSION:
                    {
                        ...
                        {
                            STD_PRINTNL("* Error: Unknown fidoconfig version " <<
                                        ver_maj << '.' << ver_min << " - Skipping.");
                            throw_xfree(alptr);
                            goto skip_config;
                        }
                    }
    ...
    {
        STD_PRINTNL(" * AREAFILE FIDOCONFIG: Can't open file '" << path <<
                    GOLD_SLASH_STR << file << ": " << strerror(errno));
    }
