...
void gareafile::ReadHPTFile(char * path, char * file, char * origin, int group)
{
            ...
            if(not s.empty())
            {
                ...
                switch(strCrc16(key))
                {
                    ...
                    case CRC_VERSION:
                    {
                        ...
                        {
                            STD_PRINTNL(
                                "* Error: Unknown fidoconfig version " << ver_maj << '.' << ver_min <<
                                " - Skipping.");
                            throw_xfree(alptr);
                            goto skip_config;
                        }
                    ...
                    if(not strieql("Passthrough", key))
                    {
                        ...
                        for( ; ; )
                        {
                            if(*key == '-')
                            {
                                ...
                                else if(strieql(opt, "p") or strieql(opt,
                                                                     "$m") or strieql(opt,
                                                                                      "lr")
                                        or strieql(opt,
                                                   "lw")
                                        or strieql(opt, "dupeCheck") or strieql(opt,
                                                                                "dupehistory")
                                        or strieql(opt, "r") or strieql(opt,
                                                                        "w")
                                        or strieql(opt, "l") or strieql(opt,
                                                                        "fperm")
                                        or strieql(opt, "fowner") or strnieql(opt,
                                                                              "sbadd(",
                                                                              6) or
                                        strnieql(opt, "sbign(", 6))
                                {
                                    gettok(&key, &val);
                                }

                                //else if(strieql(opt, "h") or strieql(opt, "manual")
                                //        or strieql(opt, "nopause") or strieql(opt,
                                // "mandatory")
                                //        or strieql(opt, "dosfile") or strieql(opt, "ccoff")
                                //        or strieql(opt, "b") or strieql(opt, "tinysb")
                                //        or strieql(opt, "killsb") or strieql(opt,
                                // "keepunread")
    ...
    {
        STD_PRINTNL(" * AREAFILE FIDOCONFIG: Can't open file '" << path << GOLD_SLASH_STR << file << ": " << strerror(
                        errno));
    }
} // gareafile::ReadHPTFile
