...
{
    char version[9], deflanguage[9], freespace50[68], logpath[61], textpath[61],
         menupath[61], mnurampath[61], netmailpath[61], nodelistpath[61], msgpath[61],
    ...
    char shellswap, highbit, disppass, asklocalpass, fastlogon, sysopremote,
         printerlog, phone1ask, colourask, aliasask, dobask, phoneforce, direct_video,
         snow_check;
    char    freespace54;
    char    screen_blank;
    char    oneword;
    AskType checkmail, checkfile, ansiask, fullscreenask, clearask, moreask,
            avatarask, extendask, usdateask;
    char phone2ask, phoneformat[15], nameprompt[61], pwdprompt[61],
         shellprompt[61], shell2prompt[61], enterprompt[61], chatprompt[61],
         listprompt[61];
    char f7keylinetop[80], f7keylinebot[80], freespace55[84], chat2prompt[61],
         screenlengthprompt[61], screenclearprompt[61], locationprompt[61],
         freeprompt[61], loadprompt[61], avatarprompt[61], aliasprompt[61];
    ...
    char freespace56, comport, passlogons, doblogon, printerport, passtries;
    char topmenu[9];
    char freespace100[4];
    word watchmess,
    /* constant */ netmailcredit, ansiminbaud,
    /* unused */ freespace57, slowbaud, minloginbaud, lowsecuritystart, lowsecurityend,
         slowstart, slowend;
    char quotestring[6], freespace58, offhook;
    word forcecrashmail, optioncrashmail, netmailfileattach;
    char popuphighlight, freespace59, maxpages, maxpagefiles, pagelength;
    word pagestart[7],
    /* constant */ pagemessboard,                      // Message Board for Paging (0=Not In
                                                       // Use)
         localfattachsec, sectouploadmess, sectoupdateusers, readsecnewecho,
         writesecnewecho, sysopsecnewecho;
    ...
    word    pageend[7];
    char    freespace61[22];
    word    incomingcallstart, incomingcallend,
    /* unused */ freespace62;
    char altf[10][61], ctrlf[10][41],
    ...
    char ks_per_fp,                    // Number of Kilobytes per FP
         ...
         rego_warn_1, rego_warn_2;
    char freespace64[2];
    word min_space_1;
    char swapbimodem, modembusy,       // Toggle DTR or ATH1
         scrheight,                    // 43/50 line mode
         msgtmptype,                   // True  = MSGTMP False = MSGTMP.<node>
         swapupload;
    char phonelogon, carrierdetect;    // Carrier Detect (Default=$80)
    char newfileshighlight;
    char max_descrip, min_descrip;
    word requestreceipt;
    char ushowdate;
    char ufilesizek;
    char uuploader, udownloadcount,
    /* unused */ freespace4, ushowsecurity, sshowdate;
    char sfilesizek;
    char suploader, sdownloadcount,
    /* unused */ freespace5, sshowsecurity, ushowtime, ushowfp, sshowtime, sshowfp;
    word fp_percent;                   // Download Filepoints Credit
    char autodetect;
    char dispsecurityfile, askforpagereason, delincompletefiles;
    char freespace65;
    char swaponfeditview;
    char freespace6, secfileschar, passchar;
    char localinactivity;
    char conversiononmaster;
    char leftbracket[2], rightbracket[2];
    ...
    char usercol1_2, usercol3_4, usercol5_6, usercol7_8, userbkcol, newusercol2,
         chstatcol, getentercol;
    char       usdateforsysop, ezyovrpath[61];
    UserString sysfree3;
    char       ovrems;
    char       swapezy, filesecpath[61];
    char       freespace28;
    char       multitasker;
    word       maxbaud;                // longint in 1.02
    char       lockedport;
    word       filereqsec;
    char       autoanswer, initresponse[11], ringstring[11];
    char       inittries, initstring1[61], initstring2[61], busystring[21],
               answerstring[21], mailerstring[61];
    word modemstart, modemend;
    char modemdelay;
    char sendbreak;
    char externaleditor[61], defaultorigin[51], connectfax[16];
    char freespace22[16];
    word uploadcredit;
    char sysfree4[36];
    char shownewfileschar;
    word
    /* unused */ zonea[16], neta[16], nodea[16], pointa[16],
    /* unused */ freespace24[16];
    char       freespace9[16][21], nocarrierstring[21];
...
typedef struct CONSTANTRECORD
{
    ...
    char     freespace2[11];
    word     scantossattr, constantattr;
    word     maxmsgsrescan;            // maximum msgs to rescan (0=disable)
    char     qwkfilename[9];
    word     qwkmaxmail, qwkmsgboard;
    ...
    char     swaponezymail;
    char     unknownarea;
    char     swaponfeditview, swaponarchive;
    word     minspaceupload;
...
typedef struct MESSAGERECORD
{
    ...
    char         attribute, attribute2, attribute3;
    char         dayskill, recvkill;
    word         countkill, kilobytekill, readsecurity;
