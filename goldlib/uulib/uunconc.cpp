...
int UUNetscapeCollapse(char * string)
{
    ...
    *p2 = '\0';
    ...
    while(*p1)
    {
        if(*p1 == '<')
        {
            if((_FP_strnicmp(p1, "<ahref=", 7) == 0 || _FP_strnicmp(p1, "<a href=",
                                                                    8) == 0) &&
...
_t_UU: len = i;
     ...
     */
    if(len != j && ((ptr[0] == '-' && ptr[1] == '-' && strstr(ptr,
                                                              "part") != NULL) ||
                    !(*ptr != 'M' && *ptr != 'h' && len > j &&
                      len <= UUxlen[UUxlat['M']])))
    {
 ...
 */
int UUDecodeQP(FILE * datain, FILE * dataout, long maxpos, int flags, char * boundary)
{
           ...
           (!(flags & FL_PROPER) && uu_fast_scanning)))
    {
        ...
        if(boundary && line[0] == '-' && line[1] == '-' && strncmp(line + 2,
                                                                   boundary,
                                                                   strlen(boundary)) ==
           0)
        {
 ...
 */
int UUDecodePT(FILE * datain, FILE * dataout, long maxpos, int flags, char * boundary)
{
                 ...
                 char * boundary)
{
           ...
           (!(flags & FL_PROPER) && uu_fast_scanning)))
    {
        ...
        if(*state == BEGIN)
        {
            if((method == UU_ENCODED || method == XX_ENCODED) && (strncmp(line,
                                                                          "begin ",
                                                                          6) == 0 ||
                                                                  strncmp(line,
                                                                          "section ",
            ...
            else if(method == YENC_ENCODED && strncmp(line, "=ybegin ",
                                                      8) == 0 && _FP_strstr(line,
                                                                            " name=") !=
                    NULL)
            {
...
int UUDecode(uulist * data)
{
    ...
    if(data->filename != NULL)
    {
        _FP_strncpy(progress.curfile,
                    (strlen(data->filename) >
                     255) ? (data->filename + strlen(data->filename) -
                             255) : data->filename,
                    256);
    }
    else
    {
        _FP_strncpy(progress.curfile,
                    (strlen(data->binfile) >
                     255) ? (data->binfile + strlen(data->binfile) -
                             255) : data->binfile,
                    256);
    }
    ...
    if(data->uudet == BH_ENCODED && data->binfile)
    {
        ...
        fread(r, 1, 8, datain);
        dsize = (((long)1 << 24) * (long)r[0]) + (((long)1 <<
                                                   16) * (long)r[1]) + (((long)1 <<
                                                                         8) *
                                                                        (long)r[2]) +
                ((long)r[3]);
        rsize = (((long)1 << 24) * (long)r[4]) + (((long)1 <<
                                                   16) * (long)r[5]) + (((long)1 <<
                                                                         8) *
                                                                        (long)r[6]) +
                ((long)r[7]);
        ...
        if(numbytes)
        {
            UUMessage(uunconc_id,
                      ...
                      (data->filename) ? data->filename : (data->subfname) ?
                      data->subfname : "???",
                      numbytes);
        }
