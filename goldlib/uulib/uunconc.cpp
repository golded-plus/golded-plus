...
int UUNetscapeCollapse(char * string)
{
    ...
    *p2 = '\0';
    ...
    while(*p1)
    {
        if(*p1 == '<')
        {
            if((_FP_strnicmp(p1, "<ahref=",
                             7) == 0 ||
                _FP_strnicmp(p1, "<a href=",
                             8) == 0) &&
               ...
               char * boundary)
{
           ...
           (!(flags & FL_PROPER) && uu_fast_scanning)))
    {
        ...
        {
            UUMessage(uunconc_id, __LINE__, UUMSG_NOTE, uustring(S_DECODE_CANCEL));
            return UURET_CANCEL;
        }
        ...
        while(*p2)
        {
            ...
            *p2 = '\0';
            ...
            if(isxdigit(*p2) && isxdigit(*(p2 + 1)))
            {
                ...
                val |=
                    ((isdigit(*(p2 + 1))) ? (*(p2 + 1) - '0') : (g_tolower(*(p2 + 1)) -
                                                                 'a' +
                                                                 10));
                fputc(val, dataout);
                p2 += 2;
                 ...
                 char * boundary)
{
    ...
    {
        return UUDecodeQP(datain, dataout, state, maxpos, method, flags, boundary);
    }
    else if(method == PT_ENCODED)
    {
        return UUDecodePT(datain, dataout, state, maxpos, method, flags, boundary);
    }
    ...
    regexp1.compile("sum -r/size [0-9]+/[0-9]+ section.*",
                    gregex::extended | gregex::icase);
           ...
           (!(flags & FL_PROPER) && uu_fast_scanning)))
    {
        ...
        if(*state == BEGIN)
        {
            if((method == UU_ENCODED || method == XX_ENCODED) &&
               (strncmp(line, "begin ",
                        6) == 0 ||
                strncmp(line, "section ",
                        8) == 0 || _FP_strnicmp(line, "<pre>begin ", 11) == 0)) /* for LYNX */
            {
                *state = DATA;
                continue;
            }
        ...
        else if((*state == END2) && (method == UU_ENCODED || method == XX_ENCODED))
        {
        ...
        if(*state == DATA && method == YENC_ENCODED && strncmp(line, "=yend ", 6) == 0)
        {
        ...
        if(*state == DATA || *state == END || *state == END2)
        {
            ...
            if(!vflag)
            {
                vflag = !endsection && UURepairData(datain,
                                                    line,
                                                    tf ? method : 0,
                                                    &bhflag);
            }
             ...
             */
            if((method == XX_ENCODED || method == UU_ENCODED) && vflag == B64ENCODED)
            {
...
/*  if (tf) { */
    ...
    if(*state == DONE ||
       (*state == DATA && method == B64ENCODED && vflag == B64ENCODED &&
        (flags & FL_PROPER || haddh)))
    {
        for(tf = 0; tf < tc; tf++)
        {
...
int UUDecode(uulist * data)
{
    ...
    {
        UUMessage(uunconc_id, __LINE__, UUMSG_ERROR, uustring(S_NO_TEMP_NAME));
        return UURET_NOMEM;
    }
    ...
    if(data->filename != NULL)
    {
        _FP_strncpy(progress.curfile,
                    (strlen(data->filename) > 255) ? (data->filename + strlen(
                                                          data->filename) - 255) : data->filename,
                    256);
    }
    else
    {
        _FP_strncpy(progress.curfile,
                    (strlen(data->binfile) > 255) ? (data->binfile + strlen(
                                                         data->binfile) - 255) : data->binfile,
                    256);
    }
    ...
    while(iter)
    {
        ...
        {
            if((res =
                    (*uu_FileCallback)(uu_FileCBArg, iter->data->sfname, uugen_fnbuffer,
                                       1)) != UURET_OK)
            {
                break;
            }
    ...
    if(data->uudet == BH_ENCODED && data->binfile)
    {
        ...
        fread(r, 1, 8, datain);
        dsize = (((long)1 << 24) * (long)r[0]) + (((long)1 << 16) * (long)r[1]) +
                (((long)1 << 8) * (long)r[2]) + ((long)r[3]);
        rsize = (((long)1 << 24) * (long)r[4]) + (((long)1 << 16) * (long)r[5]) +
        ...
        else
        {
            ...
            UUMessage(uunconc_id, __LINE__, UUMSG_NOTE, uustring(S_BINHEX_BOTH));
            fseek(datain, hb, SEEK_SET);
            numbytes = dsize;
        }
        ...
        if(numbytes)
        {
            UUMessage(uunconc_id,
                      ...
                      uustring(
                          S_SHORT_BINHEX),
                      (data->filename) ? data->filename : (data->subfname) ? data->subfname : "???",
                      numbytes);
        }
