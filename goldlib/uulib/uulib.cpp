...
int UUEXPORT UULoadFile(char * filename, char * fileid, int delflag)
{
    ...
    _FP_strncpy(progress.curfile,
                (strlen(filename) > 255) ? (filename + strlen(
...
int UUEXPORT UUDecodeFile(uulist * thefile, char * destname)
{
    ...
    progress.action = 0;
    _FP_strncpy(progress.curfile,
                (strlen(uugen_fnbuffer) > 255) ? (uugen_fnbuffer + strlen(
                                                      uugen_fnbuffer) - 255) : uugen_fnbuffer,
                256);
    progress.partno   = 0;
    progress.numparts = 1;
    ...
    if((fildes =
            open(uugen_fnbuffer, O_WRONLY | O_CREAT | O_BINARY | O_TRUNC,
...
int UUEXPORT UUInfoFile(uulist * thefile, void * opaque, int (*func)(void *, char *))
{
    ...
    if(uu_FileCallback)
    {
        (*uu_FileCallback)(uu_FileCBArg, thefile->thisfile->data->sfname, uugen_fnbuffer,
                           0);
    }
