...
headercount hlcount   = {3, /* restarting after a MIME body             */
...
static allomap toallocate[] = {{&uugen_fnbuffer,
    1024                                                    },/* generic filename buffer */
    {&uugen_inbuffer,
                                1024                                                                               },
 ...
 */
int UUEXPORT UUSetMsgCallback(void * opaque, void (*func)(void *, char *, int))
{
    uu_MsgCallback = func;
    uu_MsgCBArg    = opaque;
    return UURET_OK;
...
int UUEXPORT UUSetFileCallback(void * opaque, int (*func)(void *, char *, char *, int))
{
...
int UUEXPORT UUSetFNameFilter(void * opaque, char * (*func)(void *, char *))
{
    uu_FNameFilter = func;
    uu_FFCBArg     = opaque;
    return UURET_OK;
...
int UUEXPORT UULoadFile(char * filename, char * fileid, int delflag)
{
    ...
    _FP_strncpy(progress.curfile,
                (strlen(filename) >
                 255) ? (filename + strlen(filename) - 255) : filename,
                256);
    ...
    while(!feof(datei) && !ferror(datei))
    {
        ...
        if((loaded->uudet == QP_ENCODED || loaded->uudet == PT_ENCODED) &&
           (loaded->filename == NULL || *(loaded->filename) == '\0') && !uu_handletext &&
           (loaded->flags & FL_PARTIAL) == 0)
        {
            /*
             * Don't want text
             */
    ...
    {
        UUMessage(uulib_id, __LINE__, UUMSG_NOTE, uustring(S_NO_DATA_FOUND), filename);
    }
...
int UUEXPORT UUDecodeFile(uulist * thefile, char * destname)
{
    ...
    progress.action = 0;
    _FP_strncpy(progress.curfile,
                (strlen(uugen_fnbuffer) >
                 255) ? (uugen_fnbuffer + strlen(uugen_fnbuffer) - 255) : uugen_fnbuffer,
                256);
    progress.partno   = 0;
    progress.numparts = 1;
    ...
    if((fildes = open(uugen_fnbuffer, O_WRONLY | O_CREAT | O_BINARY | O_TRUNC,
    ...
    while(!feof(source))
    {
        ...
        {
            UUMessage(uulib_id, __LINE__, UUMSG_NOTE, uustring(S_DECODE_CANCEL));
            fclose(source);
            fclose(target);
 ...
 */
int UUEXPORT UUInfoFile(uulist * thefile, void * opaque, int (*func)(void *, char *))
{
    ...
    while(!feof(inpfile) && (uu_fast_scanning || ftell(inpfile) < maxpos))
    {
        ...
        else if(thefile->uudet == YENC_ENCODED && strncmp(uugen_inbuffer, "=ybegin ",
                                                          8) == 0)
        {
            break;
        }
