...
{"From ",              "Return-Path:",                 "Received:",
 ...
 "Precedence:",
 "Path:",
 "Newsgroups:",        "Organization:",                "Lines:",
 "NNTP-Posting-Host:", NULL};
static char * knownmimeheaders[] =
{"Mime-Version:",        "Content-Transfer-Encoding:",        "Content-Type:",
 "Content-Disposition:",
 "Content-Description:", "Content-Length:",                   NULL};
/*
                    ...
                    fileread * result)
{
    ...
    *errcode = UURET_OK;
    ...
    while(!feof(datei))
    {
        ...
        if(boundary != NULL && line[0] == '-' && line[1] == '-' &&
        ...
        {
            if(_FP_strnicmp(line, "Content-Type",
                            12) == 0 ||
        ...
        if(!(result->uudet))
        {
            ...
            if(vflag)
            {
                ...
                {
                    cbb64 =
                        (isb64[(iscnt - 1) % 10] && isb64[(iscnt - 2) % 10] &&
                         isb64[(iscnt - 3) % 10] && isb64[(iscnt - 4) % 10]);
                    cbuue =
                        (isuue[(iscnt - 1) % 10] && isuue[(iscnt - 2) % 10] &&
                         isuue[(iscnt - 3) % 10] && isuue[(iscnt - 4) % 10]);
                    cbxxe =
                        (isxxe[(iscnt - 1) % 10] && isxxe[(iscnt - 2) % 10] &&
                         isxxe[(iscnt - 3) % 10] && isxxe[(iscnt - 4) % 10]);
                    cbbhx =
                        (isbhx[(iscnt - 1) % 10] && isbhx[(iscnt - 2) % 10] &&
...
//          }
    ...
    else if(boundary && p3 && line[0] == 'C' && line[1] == 'o' &&
            _FP_strnicmp(line, "Content-Type:", 13) == 0 && strcmp(p3, boundary) == 0)
    {
        return 2;
    }
...
fileread * ScanPart(FILE * datei, char * fname, int * errcode)
{
    ...
    *errcode = UURET_OK;
    ...
    if(sstate.ismime && sstate.mimestate == MS_EPILOGUE)
    {
        ...
        while(!feof(datei))
        {
            ...
            if(mssdepth > 0 && line[0] == '-' && line[1] == '-' &&
               strncmp(line + 2, multistack[mssdepth - 1].envelope.boundary, blen) == 0)
            {
                break;
            }
            ...
            {
                if(strncmp(line, "begin ",
                           6) == 0 || _FP_strnicmp(line, "<pre>begin ", 11) == 0)
                {
                    preenc  = prevpos;
                    begflag = 1;
                }
    ...
    if(sstate.ismime && sstate.mimestate == MS_SUBPART)
    {
         ...
         */
        if((res =
                ScanData(datei, fname, errcode, sstate.envelope.boundary, 1, 0,
                         result)) == -1)
        {
            /* oops, something went wrong */
            sstate.isfolder = 0;
         ...
         */
        if(strcmp(localenv.mimevers,
                  "1.0") == 0 &&
           _FP_stristr(localenv.ctype,
                       "text") != NULL && sstate.ismime &&
           sstate.mimestate == MS_SUBPART &&
           !uu_desperate)
        {
    ...
    if(sstate.isfolder && sstate.ismime && sstate.mimestate == MS_BODY &&
       (_FP_stristr(sstate.envelope.ctenc,
        ...
        _FP_stristr(sstate.envelope.ctenc,
                    "x-yenc") != NULL ||
        _FP_stristr(sstate.envelope.ctype,
                    "message") != NULL || sstate.envelope.fname != NULL))
    {
        if(sstate.envelope.subject)
        {
            result->subject = _FP_strdup(sstate.envelope.subject);
        ...
        result->flags =
            ((res == 1 ||
              uu_fast_scanning) ? FL_PROPER : 0) | ((uu_fast_scanning) ? FL_TOEND : 0);
        result->mimeid = _FP_strdup(sstate.envelope.mimeid);
    ...
    if((sstate.envelope.ctype == NULL ||
        _FP_stristr(sstate.envelope.ctype, "multipart") != NULL) && !uu_more_mime)
    {
        prevpos = ftell(datei);
    ...
    if((res = ScanData(datei, fname, errcode, NULL, sstate.ismime, 1, result)) == -1)
    {
        /* oops, something went wrong */
        sstate.isfolder = 0;
        sstate.ismime   = 0;
