...
static char * UUGetFileName(char * subject, char * ptonum, char * ptonend)
{
     ...
     **/
    if(uu_ignreply && (subject[0] == 'R' || subject[0] == 'r') &&
    ...
    while((iter = strchr(ptr, '[')) != NULL)
    {
        ...
        while(iter[count] && (isalnum(iter[count]) || strchr(uufnchars,
                                                             iter[count]) != NULL))
        {
    ...
    if(length == 0)
    {
        ...
        while(*ptr && *ptr != 0x0a && *ptr != 0x0d && ptr != part)
        {
            ...
            while((isalnum(*iter) || strchr(uufnchars,
                                            *iter) != NULL || *iter == '/') && *iter &&
                  iter != ptonum && *iter != '.')
            {
                if(g_isalpha(*iter))
            ...
            if(_FP_strnicmp(iter, "edu", 3) == 0 || _FP_strnicmp(iter, "gov", 3) == 0)
            {
                /* hey, that's an ftp address */
            ...
            while((isalnum(iter[count]) || strchr(uufnchars,
                                                  iter[count]) != NULL ||
    ...
    if(length == 0)    /* No filename found, use subject line for ident */
    {
        ...
        while((isalnum(ptr[length]) || strchr(uufnchars,
                                              ptr[length]) != NULL ||
               ptr[length] == '/') && ptr[length] && ptr + length != part &&
              ptr + length != ptonum)
        {
            length++;
        }
 ...
 * number.
 **/
static int UUGetPartNo(char * subject, char ** where, char ** whend)
{
    ...
    *whend  = NULL;
    ...
    if(uu_ignreply && (subject[0] == 'R' || subject[0] == 'r') && /* Ignore replies, but not */
    ...
    for(bpc = 0, length = 0; brackchr[uu_bracket_policy][bpc]; bpc += 2)
    {
        ...
        while((iter = strchr(ptr, brackchr[uu_bracket_policy][bpc])) != NULL)
        {
            ...
            while(iter[count] == ' ' || iter[count] == '#' || iter[count] == '/' ||
                  iter[count] == '\\')
            {
                count++;
            }
    ...
    if(length == 0)
    {
        if((iter = _FP_stristr(subject, "part ")) != NULL)
        {
            ...
            if(length == 0)
            {
                if(_FP_strnicmp(iter, "one", 3) == 0)
                {
                    length = 1;
                }
                else if(_FP_strnicmp(iter, "two", 3) == 0)
                {
                    length = 2;
                }
                else if(_FP_strnicmp(iter, "three", 5) == 0)
                {
                    length = 3;
                }
                else if(_FP_strnicmp(iter, "four", 4) == 0)
                {
                    length = 4;
                }
                else if(_FP_strnicmp(iter, "five", 4) == 0)
                {
                    length = 5;
                }
                else if(_FP_strnicmp(iter, "six", 3) == 0)
                {
                    length = 6;
                }
                else if(_FP_strnicmp(iter, "seven", 5) == 0)
                {
                    length = 7;
                }
                else if(_FP_strnicmp(iter, "eight", 5) == 0)
                {
                    length = 8;
                }
                else if(_FP_strnicmp(iter, "nine", 4) == 0)
                {
                    length = 9;
                }
                else if(_FP_strnicmp(iter, "ten", 3) == 0)
                {
                    length = 10;
                }
    ...
    if(length == 0)
    {
        ...
        while(count > 0)
        {
            if(!isdigit(ptr[count]) || g_isalpha(ptr[count + 1]) ||
               ptr[count + 1] == '.')
            {
                count--;
                continue;
            ...
            if(count >= 0 &&
               ((g_isalpha(ptr[count]) && (ptr[count] != 's' || ptr[count + 1] != 't') &&
                 (ptr[count] != 'n' || ptr[count + 1] != 'd')) || ptr[count] == '/' ||
                ptr[count] == '.' || ptr[count] == '-' || ptr[count] == '_'))
            {
                length = 0;
                continue;
            }
    ...
    if(length == 0)
    {
         ...
         */
        if((iter = _FP_stristr(subject, "first")) != NULL)
        {
            length = 1;
        }
        else if((iter = _FP_stristr(subject, "second")) != NULL)
        {
            length = 2;
        }
        else if((iter = _FP_stristr(subject, "third")) != NULL)
        {
            length = 3;
        }
        else if((iter = _FP_stristr(subject, "forth")) != NULL)
        {
            length = 4;
        }
        else if((iter = _FP_stristr(subject, "fourth")) != NULL)
        {
            length = 4;
        }
        else if((iter = _FP_stristr(subject, "fifth")) != NULL)
        {
            length = 5;
        }
        else if((iter = _FP_stristr(subject, "sixth")) != NULL)
        {
            length = 6;
        }
        else if((iter = _FP_stristr(subject, "seventh")) != NULL)
        {
            length = 7;
        }
        else if((iter = _FP_stristr(subject, "eigth")) != NULL)
        {
            length = 8;
        }
        else if((iter = _FP_stristr(subject, "nineth")) != NULL)
        {
            length = 9;
        }
        else if((iter = _FP_stristr(subject, "ninth")) != NULL)
        {
            length = 9;
        }
        else if((iter = _FP_stristr(subject, "tenth")) != NULL)
        {
            length = 10;
        }
        else
...
uufile * UUPreProcessPart(fileread * data, int * ret)
{
    ...
    {
        UUMessage(uucheck_id, __LINE__, UUMSG_ERROR, uustring(S_OUT_OF_MEMORY),
                  sizeof(uufile));
        *ret = UURET_NOMEM;
        return NULL;
    }
    ...
    if(result->partno == -1 && (data->uudet == PT_ENCODED || data->uudet == QP_ENCODED))
    {
        result->partno = 1;
    }
    ...
    if(data->flags & FL_SINGLE)
    {
        ...
        if(result->filename == NULL || result->subfname == NULL)
        {
            UUMessage(uucheck_id, __LINE__, UUMSG_ERROR, uustring(S_OUT_OF_MEMORY),
                      (result->filename ==
                       NULL) ? (strlen(temp) + 1) : (strlen(result->filename) + 1));
            *ret = UURET_NOMEM;
            UUkillfile(result);
            return NULL;
        }
              ...
              (data->subject != NULL || result->filename != NULL))))
    {
        ...
        {
            UUMessage(uucheck_id, __LINE__, UUMSG_ERROR, uustring(S_OUT_OF_MEMORY),
                      (result->filename) ? (strlen(result->filename) +
                                            1) : (strlen(temp) + 1));
            *ret = UURET_NOMEM;
            UUkillfile(result);
            return NULL;
...
int UUInsertPartToList(uufile * data)
{
    ...
    while(iter)
    {
        ...
        else if((_FP_stricmp(data->subfname,
                             iter->subfname) == 0 ||
                 (data->mimeid && iter->mimeid && strcmp(data->mimeid,
                                                         iter->mimeid) == 0)) &&
                !(iter->begin && data->data->begin) && !(iter->end && data->data->end) &&
                !(data->mimeid && iter->mimeid && strcmp(data->mimeid,
                                                         iter->mimeid) != 0) &&
                !(data->filename && iter->filename && strcmp(data->filename,
                                                             iter->filename) != 0) &&
                !(iter->flags & FL_SINGLE))
        {
             ...
             * file was really XX
             */
            if(data->data->uudet == B64ENCODED && iter->uudet == XX_ENCODED &&
               iter->begin)
            {
            ...
            while(fiter)
            {
                ...
                if(data->partno == fiter->partno)
                {
                    ...
                    else if(_FP_stristr(fiter->data->subject,
                                        "repost") != NULL &&
...
uulist * UUCheckGlobalList(void)
{
    ...
    while(liter)
    {
        ...
        else if((liter->uudet == QP_ENCODED || liter->uudet == PT_ENCODED) &&
        ...
        if(liter->uudet == B64ENCODED || liter->uudet == QP_ENCODED ||
           ...
           (liter->filename == NULL || liter->filename[0] == '\0'))
        {
            ...
            if(liter->subfname && liter->subfname[0] && _FP_strpbrk(liter->subfname,
                                                                    "()[];: ") == NULL)
            {
