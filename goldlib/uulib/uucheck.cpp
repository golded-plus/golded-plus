 ...
 **/
    ...
    if(length == 0)
    {
        ...
        while(*ptr && *ptr != 0x0a && *ptr != 0x0d && ptr != part)
        {
            ...
            while((isalnum(*iter) ||
                   strchr(uufnchars,
                          *iter) != NULL || *iter == '/') && *iter && iter != ptonum &&
                  *iter != '.')
            {
                if(g_isalpha(*iter))
                {
            ...
            while((isalnum(iter[count]) ||
                   strchr(uufnchars,
                          iter[count]) != NULL || iter[count] == '/') && iter[count] &&
                  iter[count] != '.')
            {
                count++;
            }
            ...
            while(iter[count] &&
                  (isalnum(iter[count]) ||
                   strchr(uufnchars, iter[count]) != NULL || iter[count] == '/'))
            {
                count++;
            }
    ...
    if(length == 0)  /* No filename found, use subject line for ident */
    {
        ...
        while((isalnum(ptr[length]) ||
               strchr(uufnchars,
                      ptr[length]) != NULL || ptr[length] == '/') && ptr[length] &&
              ptr + length != part && ptr + length != ptonum)
        {
            length++;
        }
        ...
        if(data->data->flags & FL_SINGLE)
        {
            /* this space intentionally left blank */
                 ...
                 (data->mimeid && iter->mimeid &&
                  strcmp(data->mimeid,
                         iter->mimeid) == 0)) && !(iter->begin && data->data->begin) &&
                !(iter->end && data->data->end) &&
                !(data->mimeid && iter->mimeid &&
                  strcmp(data->mimeid,
                         iter->mimeid) != 0) &&
                !(data->filename && iter->filename &&
                  strcmp(data->filename,
                         iter->filename) != 0) && !(iter->flags & FL_SINGLE))
        {
            /*
