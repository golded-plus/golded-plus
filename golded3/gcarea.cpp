...
void FixEchoid(char * echoid, int type)
{
    ...
    {
        LOG_NULL_POINTER;
        LOG.printf("! Illegall call of function: FixEchoid(NULL,%i). File %s line %i ",
                   type,
                   __FILE__,
                   __LINE__);
        PointerErrorExit();
    ...
    *desc = NUL;
    ...
    for( ; ap != end; ap++)
    {
        ...
        else if(not (*ap)->isseparator())
        {
            ...
            bool eq_isfido  =
                (((aa->basetype == "OPUS") || (aa->basetype == "FTS1")) &&
...
//  Examples:
//
//    AREADEF NET.ALL  "Netmail, Line 1" N Net   Opus   R:\NETMAIL\             2:231/77 (PVT
// K/S)
//    AREADEF TEST     "Testing..."      D Echo  Hudson 67                      .        ()
//    AREADEF 1LOCAL   "Sysop <-> Users" L Local Squish R:\MAX\MSG\LOC\1LOCAL   .        (PVT)
//     "Your Sysop * The Goldware BBS Line 1"
...
void getdztoken(char ** key, char ** val)
{
    ...
    {
        LOG_NULL_POINTER;
        LOG.printf("! Illegall call of function: getdztoken(%s,%s). File %s line %i ",
                   key ? "key" : "NULL",
                   val ? "val" : "NULL",
                   __FILE__,
                   __LINE__);
...
void AreaList::ReadEcholist(char * val)
{
    ...
    if(val == NULL)
    {
        LOG_NULL_POINTER;
        LOG.printf(
            "! Illegall call of function: AreaList::ReadEcholist(NULL). File %s line %i ",
            __FILE__,
            __LINE__);
        PointerErrorExit();
        return;
...
void AreaList::GetAreaDesc(char * val)
{
    ...
    if(val == NULL)
    {
        LOG_NULL_POINTER;
        LOG.printf(
            "! Illegall call of function: AreaList::GetAreaDesc(NULL). File %s line %i ",
            __FILE__,
            __LINE__);
        PointerErrorExit();
        return;
...
char * MapPath(char * map, bool reverse)
{
    ...
    {
        LOG_NULL_POINTER;
        LOG.printf("! Illegall call of function: MapPath(NULL, %s). File %s line %i ",
                   reverse ? "TRUE" : "FALSE",
                   __FILE__,
                   __LINE__);
        PointerErrorExit();
    ...
    std::vector<std::pair<std::string,
                          std::string> >::iterator i = CFG->mappath.begin();
...
const char * AreaCfg::setautoid(const char * _echoid)
{
    if(_echoid == NULL)
    {
        LOG_NULL_POINTER;
        LOG.printf(
            "! Illegall call of function: AreaCfg::setautoid(NULL). File %s line %i ",
            __FILE__,
            __LINE__);
        PointerErrorExit();
        return NULL;
