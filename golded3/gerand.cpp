...
void Area::InitData()
{
    ...
    adat->akamatching =
        (CFG->switches.get(akamatchnet) and isnet()) or (CFG->switches.get(
                                                             akamatchecho) and isecho())
        or (CFG->switches.get(akamatchlocal) and islocal());
    ...
    strxcpy(adat->origin,
            CFG->origin.empty() ? "" : CFG->origin[CFG->originno].c_str(),
            sizeof(adat->origin));
    ...
    strcpy(adat->tagline,
           CFG->tagline.empty() ? "" : CFG->tagline[CFG->taglineno].c_str());
    adat->taglinechar    = CFG->taglinechar;
    adat->taglinesupport = CFG->taglinesupport;
    strcpy(adat->tearline, CFG->tearline);
    strcpy(adat->tpl,
           (CFG->tplno and (CFG->tplno <
                            CFG->tpl.size())) ? CFG->tpl[CFG->tplno].file : "");
    ...
    strcpy(adat->username.name,
           CFG->username.empty() ? "" : CFG->username[CFG->usernameno].name);
    ...
    if(*_groupid)
    {
        ...
        if(found and (strieql(found,
                              "*") or ((curgrp >= 0) and (curgrp < CFG->grp.currgrpno))))
        {
            found = NULL;
        }
...
//    adat = (AreaData*)throw_calloc(1, sizeof(AreaData));
    ...
    if(found)
    {
        ...
        CFG->grp.GetItm(GRP_AREAYOUWROTETO, adat->areayouwroteto,
                        sizeof(adat->areayouwroteto));
        ...
        CFG->grp.GetItm(GRP_INTERNETADDRESS, adat->internetaddress,
                        sizeof(adat->internetaddress));
        ...
        if(CFG->grp.GetItm(GRP_LOADLANGUAGE, adat->loadlanguage,
                           sizeof(adat->loadlanguage)))
        {
            LoadLanguage(adat->loadlanguage);
        }
        ...
        CFG->grp.GetItm(GRP_ORGANIZATION, adat->organization,
        ...
        CFG->grp.GetItm(GRP_QUOTEBUFFILE, adat->quotebuffile,
                        sizeof(adat->quotebuffile));
