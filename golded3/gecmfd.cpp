    ...
    if(Mark.Count())
    {
        ...
        if(n == 1)
        {
            ...
            for(n = 0; n < Mark.Count(); n++)
            {
                ...
                update_statuslinef(LNG->DeletingMsg,
                                   "ST_DELETINGMSG",
                                   x + 1,
                                   Mark.Count());
                        ...
                        if(delask)
                        {
                            if(msg->attr.uns() and not (msg->attr.rcv() or msg->attr.del()))
                            {
                                AA->LoadMsg(msg,
                                            msgno,
                                            CFG->dispmargin -
                                            (int)CFG->switches.get(disppagebar));
...
void CmfMsgs(GMsg * msg, bool torecycle)
{
    ...
    if(!torecycle &&
       (cmf == MODE_FORWARD ? not AA->Areaforwarddirect() : not AA->Areacopydirect()))
    {
...
//      throw_free(AA->adat);
    ...
    do
    {
        ...
        if(AA->LoadMsg(msg, loadmsgno,
                       CFG->dispmargin - (int)CFG->switches.get(disppagebar), loadmode))
        {
            // Handle unsent msgs
            mode = cmf;
            ...
            else
            {
                ...
                if(not need_netmail_kludges && (AAorig->basetype() == "SQUISH") &&
                   (AAdest->basetype() == "SQUISH") && AAdest->isnet() &&
                   AAorig->isnet())
                {
                ...
                if(need_netmail_kludges)
                {
                    ...
                    if(not strstr(msg->txt, "\001INTL"))
                    {
                                             ...
                                             YES or (msg->dest.zone != msg->orig.zone)))
                        {
                            gsprintf(PRINTF_DECLARE_BUFFER(buf),
                                     "\001INTL %u:%u/%u %u:%u/%u%c",
                                     msg->dest.zone ? msg->dest.zone :
                                     AA->Aka().addr.zone,
                                     msg->dest.net,
                                     msg->dest.node,
                                     msg->orig.zone ? msg->orig.zone :
                                     AA->Aka().addr.zone,
                                     msg->orig.net,
                                     msg->orig.node,
                                     CR);
                    ...
                    if(not strstr(msg->txt, "\001TOPT") or not strstr(msg->txt,
                                                                      "\001FMPT"))
                    {
                        ...
                        if(msg->orig.point and msg->orig.net ==
                           msg->oorig.net and msg->orig.node ==
                           msg->oorig.node and need_fmpt)
                        {
                            sprintf(buf + strlen(buf),
                                    "\001FMPT %u%c",
                                    msg->orig.point,
