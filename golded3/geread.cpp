                               ...
                               CFG->dispmargin - (int)CFG->switches.get(disppagebar)))
                {
                    do
                    {
                        ...
                        if(reader_msglistfirst == false)
                        {
                            if(AA->isreadmark or AA->isreadpm)
                            {
                                ...
                                sprintf(buf2,
                                        ...
                                        AA->Viewkludge() ? "K" : "",
                                        AA->Twitmode() ==
                                        TWIT_IGNORE ? "Ti" : AA->Twitmode() ==
                                        TWIT_SKIP ? "Ts" : AA->Twitmode() ==
                                        TWIT_BLANK ? "Tb" : AA->Twitmode() ==
                                        TWIT_KILL ? "Tk" : "",
                                        CFG->showdeleted ? "D" : "",
                                        AA->Viewquote() ? "Q" : "",
                                        AA->StripHTML() ? "S" : "");
                        ...
                        if(reader_msglistfirst == false)
                        {
                            ...
                            if((istwit ==
                                TWIT_SKIP) or (not CFG->showdeleted and msg->attr.del()))
                            {
                            ...
                            if(reader_rcv_noise)
                            {
                                ...
                                else if(reader_rcv_noise > 1 and CFG->beepyourmail ==
                                        true)
                                {
                                    HandleGEvent(EVTT_MSGTOYOU);
                                }
                            ...
                            if(AA->Msgn.Count() and CFG->switches.get(
                                   highlightunread) and (msg->orig_timesread == 0))
                            {
                        ...
                        switch(reader_keycode)
                        {
                            ...
                            case KK_ReadAddressbookAdd:
                            {
                                ...
                                int source =
                                    AA->Mark.Count() ?
                                    MenuDomarks.Run(LNG->Addressbook) : WRITE_CURRENT;
...
int LoadMessage(GMsg * msg, int margin)
{
    ...
    if(AA->LoadMsg(msg, AA->Msgn.CvtReln(AA->lastread()), margin))
    {
        ...
        {
            for(std::vector<Node>::iterator n = CFG->username.begin();
                n != CFG->username.end(); n++)
            {
                // Check TO:
                if(msg->dest.match(n->addr))
                {
                    ...
                    {
                        if(n->addr.net != GFTN_ALL or msg->dest.net ==
                           0 or not AA->isnet())
                        {
                ...
                if(msg->orig.match(n->addr))
                {
                    ...
                    {
                        if(n->addr.net != GFTN_ALL or msg->orig.net ==
                           0 or not AA->isnet())
                        {
            ...
            {
                if(strieql(msg->to, AA->Internetaddress()) or strieql(msg->idest,
                                                                      AA->Internetaddress()))
                {
                    msg->attr.tou1();
                }
            ...
            if(reader_msglistfirst == false)
            {
                ...
                {
                    msg->attr.upd1();   // Tell SaveHdr to "touch" the time stamp of the *.MSG
                                        // file
                }
...
void GotoReplies()
{
    ...
    for(int n = 0; n < list_max + 2; n++)
    {
        ...
        if(reln)
        {
            ...
            rlist[replies].isread = ((0 ==
                                      rmsg->timesread) and CFG->switches.get(
                                         highlightunread)) ? MMRK_MARK : ' ';

            if((cursorbar == -1) and (0 ==
                                      rmsg->timesread) and (CFG->switches.get(
                                                                highlightunread)))
            {
                cursorbar = replies;
            ...
            strftimei(rlist[replies].written,
                      CFG->disphdrdateset.len,
                      LNG->DateTimeFmt,
                      &tm);
