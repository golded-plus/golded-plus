                               ...
                               CFG->dispmargin - (int)CFG->switches.get(disppagebar)))
                {
                    do
                    {
                        ...
                        if(reader_msglistfirst == false)
                        {
                            ...
                            if(AA->Msgn.Count() and CFG->switches.get(highlightunread)
                        ...
                        switch(reader_keycode)
                        {
                            ...
                            case KK_ReadAddressbookAdd:
                            {
                                ...
                                int source = AA->Mark.Count() ? MenuDomarks.Run(
                                    LNG->Addressbook) : WRITE_CURRENT;
...
void GotoReplies()
{
    ...
    for(int n = 0; n < list_max + 2; n++)
    {
        uint32_t msgn =
            n ? ((n !=
                  list_max + 1) ? msg->link.list(n -
                                                 1) : msg->link.next()) : msg->link.first();
        ...
        if(reln)
        {
            ...
            rlist[replies].isread =
                ((0 ==
                  rmsg->timesread) and CFG->switches.get(highlightunread)) ? MMRK_MARK :
                ' ';
