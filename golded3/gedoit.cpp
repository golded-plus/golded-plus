...
void SaveLines(int mode, const char * savefile, GMsg * msg, int margin, bool clip)
{
    ...
    if(prnfp.isopen())
    {
        ...
        TemplateToText(((mode ==
                         MODE_WRITE) and prnheader) ? ((prnheader &
                                                        WRITE_ONLY_HEADER) ? MODE_HEADER
                                                       : MODE_WRITEHEADER) : MODE_WRITE,
                       msg,
                       msg,
                       AA->WTpl(),
                       CurrArea);
        msg->TextToLines(margin, false);
#endif // ifdef OLD_STYLE_HEADER
        int n       = 0;
        Line ** lin = msg->line;
        ...
        if(lin)
        {
            ...
            while(line)
            {
                ...
                if(not ((mode == MODE_SAVENOCTRL) and lineisctrl))
                {
                    if(mode == MODE_WRITE)
                    {
                        for(std::string::iterator p = line->txt.begin();
                            p != line->txt.end(); p++)
                        {
                    ...
                    if(mode == MODE_NEW)
                    {
                        if(EDIT->HardLines())
                        {
                            ...
                            {
                                if(not ((line->type &
                                         (GLINE_TEAR | GLINE_ORIG | GLINE_KLUDGE |
                                          GLINE_QUOT)) or strblank(ptr)))
                                {
                                    prnfp.Fwrite(EDIT->HardLine(),
                                                 strlen(EDIT->HardLine()));
                                }
                            }
                        }
                    }
                    ...
                    if(prn)
                    {
                        ...
                        if(lines % CFG->printlength ==
                           0 and CFG->switches.get(printformfeed))
                        {
                            prnfp.Fwrite("\f", 1);
                        }
                    }
        ...
        if((prn and CFG->switches.get(printformfeed)) or (not prn and not clip and CFG->
                                                          switches.get(formfeedseparator)))
        {
            prnfp.Fwrite("\f", 1);
        }
...
static void WriteMsgs(GMsg * msg)
{
    ...
    if(source != WRITE_QUIT)
    {
        ...
        {
            prnheader =
                (target & (WRITE_NO_HEADER | WRITE_ONLY_HEADER)) ^ WRITE_NO_HEADER;
            ...
            if(source == WRITE_MARKED)
            {
                if(target & WRITE_FILE)
                {
                    do
                    {
                        ...
                        if(not edit_pathname(CFG->outputfile, sizeof(Path),
                                             LNG->WriteMsgs, H_WriteMessage))
                        {
                            goto Finish;
                        }
                        ...
                        if(stricmp(AA->Outputfile(),
                                   "PRN") and strnicmp(AA->Outputfile(), "LPT", 3))
                        {
                ...
                for(uint n = 0; n < AA->Mark.Count(); n++)
                {
                    ...
                    update_statuslinef(LNG->WritingMsg,
                                       "ST_WRITINGMSG",
                                       n + 1,
                                       AA->Mark.Count());
                    ...
                    {
                        SaveLines(overwrite ? MODE_WRITE : MODE_APPEND,
                                  AA->Outputfile(),
                                  msg,
                                  prnmargin,
                                  make_bool(target & WRITE_CLIPBRD));
                    }
                }
            ...
            else if(source == WRITE_CURRENT)
            {
                if(target & WRITE_FILE)
                {
                    do
                    {
                        ...
                        if(edit_pathname(CFG->outputfile, sizeof(Path), LNG->WriteMsgs,
                                         H_WriteMessage))
                        {
                            if(is_dir(CFG->outputfile))
                            {
