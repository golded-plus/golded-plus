...
void SaveLines(int mode, const char * savefile, GMsg * msg, int margin, bool clip)
{
    ...
    if(prnfp.isopen())
    {
        ...
        if((prn and CFG->switches.get(
                printformfeed)) or (not prn and not clip and CFG->switches.get(
                                        formfeedseparator)))
        {
            prnfp.Fwrite("\f", 1);
        }
...
static void WriteMsgs(GMsg * msg)
{
    ...
    if(source != WRITE_QUIT)
    {
        ...
        {
            prnheader = (target &
                         (WRITE_NO_HEADER | WRITE_ONLY_HEADER)) ^ WRITE_NO_HEADER;
            ...
            if(source == WRITE_MARKED)
            {
                if(target & WRITE_FILE)
                {
                    do
                    {
                        ...
                        if(not edit_pathname(CFG->outputfile,
                                             sizeof(Path),
                                             LNG->WriteMsgs,
                                             H_WriteMessage))
                        {
                        ...
                        if(stricmp(AA->Outputfile(),
                                   "PRN") and strnicmp(AA->Outputfile(),
                                                       "LPT",
                                                       3))
                        {
            ...
            else if(source == WRITE_CURRENT)
            {
                if(target & WRITE_FILE)
                {
                    do
                    {
                        ...
                        if(edit_pathname(CFG->outputfile,
                                         sizeof(Path),
                                         LNG->WriteMsgs,
                                         H_WriteMessage))
                        {
