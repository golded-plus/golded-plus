...
void CheckSubject(GMsg * msg, char * subj)
{
    ...
    if(AA->isnet())
    {
        if(EDIT->AutoAttach())
        {
            ...
            if(not strieql(subj, msg->re))
            {
                ...
                if(not (msg->attr.frq() or msg->attr.att() or msg->attr.urq()))
                {
                    ...
                    if((in_range((char)g_tolower(*ptr), (char)'c',
                                 (char)'z') and (ptr[1] == ':')) or ((*ptr ==
                                                                      '\\') and (ptr[1]
                                                                                 ==
                                                                                 '\\')))
                    {
        ...
        if(msg->attr.frq())
        {
            ...
            if(*subj)
            {
                ...
                while(ptr)
                {
                    ...
                    {
                        fspec =
                            (FileSpec *)throw_realloc(fspec,
                                                      (fspecs + 1) * sizeof(FileSpec));
        ...
        else if(msg->attr.att())
        {
            ...
            {
                fspec =
                    (FileSpec *)throw_realloc(fspec, (fspecs + 1) * sizeof(FileSpec));
                memset(fspec + fspecs, 0, sizeof(FileSpec));
        ...
        if(specfiles)
        {
            ...
            *buf = NUL;
            ...
            for(int x = 0, n = 0; x < fspecs; x++)
            {
                ...
                {
                    ISub subject;
                    gsprintf(PRINTF_DECLARE_BUFFER(subject),
                             "%s%s%s%s%s ",
                             fspec[x].delsent ? "^" : "",
                             ReMapPath(
                                 fspec[x].path),
                             fspec[x].fblk ? (fspec[x].fblk[m].name ? fspec[x].fblk[m].
                                              name : "") : "",
                             *fspec[x].password ? " " : "",
                             fspec[x].password);
...
// msg->charsetlevel);
            gsprintf(PRINTF_DECLARE_BUFFER(msg->charset),
                     "%s %d",
                     AA->Xlatexport(),
                     CharTable->displaylevel);
        }
        else
        {
    ...
    {
        std::string temp;
        update_statuslinef(LNG->StatusCC, "ST_STATUSCC", msg->to,
                           msg->dest.make_string(temp).c_str());
    }

    msg->LinesToText();

    if(AA->isnet() and (msg->attr.frq() or msg->attr.att() or msg->attr.urq()) and
       specfiles >= 1)
    {
    ...
    if(cc)
    {
        ...
        for(n = 0; n < cc; n++)
        {
            ...
            {
                std::string temp;
                update_statuslinef(LNG->StatusCC,
                                   "ST_STATUSCC",
                                   cmsg->to,
                                   cmsg->dest.make_string(temp).c_str());
            }
            ...
            else
            {
                ...
                // Do zoneGating
                if(AA->isnet() and CFG->zonegating and (cmsg->dest.zone !=
                                                        cmsg->orig.zone))
                {
        ...
        do
        {
                    ...
                    if(ftbefore == ftafter)
                    {
                        if(streql(msg->by, cmpmsg->by))
                        {
                            if(streql(msg->to, cmpmsg->to))
                            {
                                ...
                                {
                                    if(not memcmp(&msg->attr, &cmpmsg->attr,
                                                  sizeof(Attr)))
                                    {
                                        status = MODE_QUIT;
                                    }
            ...
            else if(forwstat == NO and (EDIT->Internal() or not * EDIT->External()))
            {
                ...
                while(line)
                {
                    ...
                    if((line->type &
                        GLINE_KLUD) and not AA->Viewkludge() and not (line->kludge &
                                                                      GKLUD_RFC))
                    {
...
// msg->charsetlevel);
            ...
            switch(mode)
            {
                ...
                case MODE_REPLY:
                    ...
                    if(AA->Akamatching())
                    {
                        ...
                        if(AA->Aka().addr.equals(AA->aka()))
                        {
                            ...
                            if(CFG->akamatchfromto)
                            {
                                ...
                                for(a = CFG->aka.begin(); useto && (a != CFG->aka.end());
                                    a++)
                                {
                    ...
                    else
                    {
                        ...
                        msg->dest = omsg->orig;
                        strcpy(msg->idest,
                               *omsg->ireplyto ? omsg->ireplyto : *omsg->iorig ? omsg->iorig : omsg->iaddr);
                    }
                    ...
                    if(AA->Replyre() or AA->isinternet())
                    {
                        ...
                        if(r)
                        {
                            ...
                            {
                                ISub subj;
                                gsprintf(PRINTF_DECLARE_BUFFER(
                                             subj),
                                         "Re^%u:%s",
                    ...
                    else
                    {
                        ...
                        msg->references =
                            (char *)throw_calloc(1, reflen + 1 + midlen + 1);
                    ...
                    if(CurrArea == OrigArea)
                    {
                        ...
                        {
                            if((CFG->replylink == REPLYLINK_DIRECT) or (AA->basetype() ==
                                                                        "JAM"))
                            {
                ...
                case MODE_NEW:
                    ...
                    strcpy(msg->odom,
                           CFG->aka[AkaMatch(&msg->orig, &AA->Aka().addr)].domain);
                    break;
            ...
            if(status == NO)
            {
                ...
                else
                {
                    if(confirm)
                    {
                        ...
                        gfile fp(AddPath(CFG->goldpath, CFG->confirmfile),
                                 "rt",
                                 CFG->sharemode);
        ...
        if(status == MODE_SAVE)
        {
            ...
            if(CurrArea ==
               OrigArea and (mode == MODE_QUOTE or mode == MODE_REPLYCOMMENT or mode ==
                             MODE_REPLY))
            {
        ...
        {
            update_statuslinef(LNG->Crossposting, "ST_CROSSPOSTING",
                               AL[post_xparea.back()]->echoid());
        }
    }
