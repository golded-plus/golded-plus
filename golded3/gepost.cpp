            ...
            if(*subj)
            {
                ...
                while(ptr)
                {
                    if(*ptr == '!')     // Store password
                    {
                        if(fspecs)
                        {
                            strcpy(fspec[fspecs - 1].password,
                                   ptr);
                        }
                    }
                    else
                    {
                        fspec = (FileSpec *)throw_realloc(fspec,
                                                          (fspecs + 1) *
                                                          sizeof(FileSpec));
            ...
            for(int x = 0, n = 0; x < fspecs; x++)
            {
                for(int m = 0; m < fspec[x].files; m++)
                {
                    ...
                    gsprintf(PRINTF_DECLARE_BUFFER(subject),
                             "%s%s%s%s%s ",
                             fspec[x].delsent ? "^" : "",
                             ReMapPath(fspec[x].path),
                             fspec[x].fblk ?
                             (fspec[x].fblk[m].name ? fspec[x].fblk[m].name : "") : "",
                             *fspec[x].password ? " " : "",
                             fspec[x].password);
            ...
            switch(mode)
            {
                ...
                case MODE_REPLY:
                    ...
                    else
                    {
                        ...
                        msg->dest = omsg->orig;
                        strcpy(msg->idest,
                               *omsg->ireplyto ? omsg->ireplyto : *omsg->iorig ?
                               omsg->iorig : omsg->iaddr);
                    }
                    ...
                    if(AA->Replyre() or AA->isinternet())
                    {
                        ...
                        if(r)
                        {
                            ...
                            {
                                ISub subj;
                                gsprintf(PRINTF_DECLARE_BUFFER(subj),
                                         "Re^%u:%s",
                                         (((number + 1) >
                    ...
                    else
                    {
                        ...
                        msg->references = (char *)throw_calloc(1,
                                                               reflen + 1 + midlen + 1);
            ...
            if(status == NO)
            {
                ...
                else
                {
                    if(confirm)
                    {
                        ...
                        gfile fp(AddPath(CFG->goldpath, CFG->confirmfile), "rt",
                                 CFG->sharemode);
