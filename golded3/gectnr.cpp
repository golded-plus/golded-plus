...
void Container::StyleCodeHighlight(const char * text,
                                   int row,
                                   int col,
                                   bool dohide,
                                   vattr color)
    ...
    if(usestylies or CFG->highlighturls)
    {
        while(*ptr)
        {
            if(usestylies and isstylechar(*ptr))
            {
                if(strchr(punctchars, prevchar))
                {
                    ...
                    if((bb <= 1) and (bi <= 1) and (br <= 1) and (bu <= 1) and * ptr)
                    {
                        ...
                        if(isstylechar(*endstyle) and not strchr(stylemargins,
                                                                 *beginword))
                        {
                            ...
                            if(endword >= beginword and not strchr(stylemargins,
                                                                   *endword))
                            {
                                if((bb == eb) and (bi == ei) and (bu == eu) and (br ==
                                                                                 er))
                                {
                                    bool style_stops_present = strncont(beginword,
                                                                        stylestopchars,
                                                                        endword -
                                                                        beginword);
                                    ...
                                    {
                                        int colorindex = (bb ? 1 : 0) | (bi ? 2 : 0) |
                                                         (bu ? 4 : 0) | (br ? 8 : 0);
                                        strxcpy(buf,
                                                txptr,
                                                (uint)(beginstyle - txptr) + 1);
                                        prints(row, col + sclen, color, buf);
                                        sclen += strlen(buf);

                                        if(dohide)
                                        {
                                            strxcpy(buf, beginword,
                                                    (uint)(endword - beginword) + 2);
                                        }
                                        else
                                        {
                                            strxcpy(buf, beginstyle,
                                                    (uint)(endstyle - beginstyle) + 2);
                                        }

                                        prints(row, col + sclen, C_STYLE[colorindex],
                                               buf);
                                        sclen += strlen(buf);
                                        txptr  = end;
                                        ptr    = end - 1;
            ...
            else if(CFG->highlighturls)
            {
                ...
                if(((begin = url_begin(ptr)) !=
                    NULL) and ((ptr ==
                                text) or (not isxalnum(ptr[-1]) and (ptr[-1] != '@'))))
                {
                ...
                else if(((ptr == text) or not in_ftn_domained_address(ptr,
                                                                      text)) and (isascii
                                                                                      (*
                                                                                      ptr)
                                                                                  and not
                                                                                               ...
                                                                                               '@')))))
                {
                    ...
                    if((commerce_at != NULL) and (*commerce_at == '@'))
                    {
                        ...
                        while((*commerce_at !=
                               NUL) and (isalnum(*commerce_at) or (*commerce_at ==
                                                                   '.') or (*commerce_at
                                                                            == '-')))
                        {
                            if(*commerce_at == '.')
