...
//  ------------------------------------------------------------------
static bool tokenxchg(std::string & input,
                      ...
                      ...)
{
    ...
    if((strnieql(inp_str_tok, tok,
                 toklen)) &&
       (isspace(inp_str_tok[toklen]) || ispunct(inp_str_tok[toklen]) ||
        inp_str_tok[toklen] == '\0'))
    {
        std::string str = src;
...
void TokenXlat(int mode, std::string & input, GMsg * msg, GMsg * oldmsg, int __origarea)
{
    ...
    if(revbuf[0] == NUL)
    {
        gsprintf(PRINTF_DECLARE_BUFFER(revbuf), "%02d%02d", str2mon(__gver_date__),
                 atoi(&__gver_date__[4]));
    }
    ...
    char msgno[16];
    gsprintf(PRINTF_DECLARE_BUFFER(msgno),
             "%u",
             msg->attr.nwm() ? AA->Msgn.Count() + 1 : AA->Msgn.ToReln(msg->msgno));
    char msgs[16];
    gsprintf(PRINTF_DECLARE_BUFFER(msgs), "%u",
             AA->Msgn.Count() + (msg->attr.nwm() ? 1 : 0));
    ...
    for(dst = input.begin(); dst != input.end(); )
    {
        ...
        else
        {
            ...
            if(tokenxchg(input, dst, "@otzoffset",
                         (oldmsg->tzutc ==
                          -32767) ? "" : (gsprintf(PRINTF_DECLARE_BUFFER(buf), " %+05d",
                                                   oldmsg->tzutc), buf)))
            {
                continue;
            }
            ...
            if(tokenxchg(input, dst, "@omessageid",
                         oldmsg->messageid ? oldmsg->messageid : ""))
            {
                continue;
            }
            ...
            if((not origareaisinet or not currareaisinet) and (strlen(it2str(input,
                                                                             dst)) >= 6))
            {
                ...
                {
                    if(tokenxchg(input, dst, "@oaddr",
                                 oldmsg->orig.make_string(buf, dr ? oldmsg->odom : NULL),
                                 19, 1, 0))
                    {
                        continue;
                    }
                    ...
                    if(strnieql(it2str(input, dst), "@o3daddr", 8))
                    {
                        ...
                        tokenxchg(input,
                                  dst,
                                  "@o3daddr",
                                  boss.make_string(buf,
                                                   domain_requested(it2str(input, dst),
                                                                    8) ? oldmsg->odom :
                                                   NULL),
                                  19,
                                  1,
                    ...
                    if(tokenxchg(input, dst, "@daddr",
                                 oldmsg->dest.make_string(buf, dr ? oldmsg->ddom : NULL),
                                 19, 1, 0))
                    {
                        continue;
                    }
                ...
                if(not currareaisinet)
                {
                    ...
                    if(tokenxchg(input, dst, "@caddr",
                                 caka.addr.make_string(buf, dr ? caka.domain : NULL), 19,
                                 1, 0))
                    {
                        continue;
                    }
                    ...
                    if(tokenxchg(input, dst, "@taddr",
                                 msg->dest.make_string(buf, dr ? msg->ddom : NULL), 19,
                                 1, 0))
                    {
                        continue;
                    }
                    ...
                    if(tokenxchg(input, dst, "@faddr",
                                 msg->orig.make_string(buf, dr ? msg->odom : NULL), 19,
                                 1, 0))
                    {
                        continue;
                    }
            ...
            if(tokenxchg(input, dst, "@tfname", strlword(msg->To()), 0, 3, (int)false,
                         (int)false, (int)msg->to_all()))
            {
                continue;
            }

            if(tokenxchg(input, dst, "@tlname", strrword(msg->To()), 0, 3, (int)false,
                         (int)false, (int)msg->to_all()))
            {
                continue;
            }
            ...
            if(tokenxchg(input, dst, "@cfname",
                         strlword(strcpy(buf, AA->Username().name))))
            {
                continue;
            }

            if(tokenxchg(input, dst, "@clname",
                         strrword(strcpy(buf, AA->Username().name))))
            {
                continue;
            }
            ...
            if(tokenxchg(input, dst, "@ctzoffset",
                         AA->Usetzutc() ? (gsprintf(PRINTF_DECLARE_BUFFER(buf), " %+05d",
                                                    tzoffset()),
                                           buf) : ""))
            {
                continue;
            ...
            if(tokenxchg(input, dst, "@cpseudo",
                         *AA->Nickname() ? AA->Nickname() : strlword(strcpy(buf,
                                                                            AA->Username()
                                                                            .name),
                                                                     " @.")))
            {
            ...
            if(tokenxchg(input, dst, "@tagline",
                         HandleRandomLine(strxcpy(buf, AA->Tagline(), sizeof(buf)),
                                          sizeof(buf))))
            {
                continue;
            }
            ...
            if(tokenxchg(input, dst, "@tearline",
                         HandleRandomLine(strxcpy(buf, AA->Tearline(), sizeof(buf)),
                                          sizeof(buf))))
            {
                continue;
            }
            ...
            if(tokenxchg(input, dst, "@origin",
                         HandleRandomLine(strxcpy(buf, AA->Origin(), sizeof(buf)),
                                          sizeof(buf))))
            {
                continue;
            }
            ...
            if(strnieql(it2str(input, dst), "@pipe{", 6))
            {
                ...
                else
                {
                    ...
                    if(tokenxchg(input, dst, "@pipe", pipe_buff.c_str(), 0, 1,
                                 (int)false))
                    {
                        continue;
                    }
            ...
            if(strnieql(it2str(input, dst), "@pad{", 5))
            {
                ...
                else
                {
                    ...
                    if(tokenxchg(input, dst, "@pad", text.c_str(), 0, 2, (int)false,
                                 (int)false))
                    {
                        continue;
                    }
            ...
            if(strnieql(it2str(input, dst), "@uptime", 7))
            {
                ...
                useconds = size_t(
                    double(counter.QuadPart % frequency.QuadPart) * 1000 /
                    frequency.QuadPart);
 ...
 */
                ...
                LOG.printf("Macros \"@uptime\" is not suppoted on platform %s!",
                           __gver_platform__);
        ...
        while(fp.Fgets(buf, PBUFSIZE - 1))
        {
            ...
            else
            {
                ...
                if(*ptr != LF)
                {
                    ...
                    else if(not isspace(buf[0]) and (buf[0] == buf[1]) and (buf[0] ==
                                                                            buf[2]))
                    {
                        if(not isspace(buf[read_len - 1]))
                        {
