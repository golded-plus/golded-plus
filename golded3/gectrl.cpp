...
{
    return strieql("UUCP",
                   name) or (*AA->Internetgate().name and strieql(AA->Internetgate().name,
                                                                  name));
}
...
char * MakeOrigin(GMsg * msg, const char * orig)
{
    ...
    if(msg->orig.net)
    {
        msg->orig.make_string(buf,
                              *AA->Netname() ? (*msg->odom ? msg->odom : AA->Netname()) : NULL);
    }
    else
    {
        AA->Aka().addr.make_string(buf,
                                   *AA->Netname() ? (*AA->Aka().domain ? AA->Aka().domain
                                                     : AA->Netname()) : NULL);
    }
    ...
    gsprintf(PRINTF_DECLARE_BUFFER(msg->origin), "%.*s (%s)",
             (int)(79 - 11 - 2 - strlen(buf) - 1), origin, buf);
    return msg->origin;
}
                ...
                if(not inmime)
                {
                    if(msg->charset)
                    {
                        ...
                        else
                        {
                            ...
                            bp = stpcpy(bp,
                                        IsQuotedPrintable(
                                            msg->charset) ? ExtractPlainCharset(msg->
                                                                                charset) : strlword(
                                            msg->charset));
                            strlwr(pbp);
                        }
...
void DoKludges(int mode, GMsg * msg, int kludges)
{
    ...
    {
        kludges =
            (GKLUD_RFC | GKLUD_FWD | GKLUD_INTL | GKLUD_FMPT | GKLUD_TOPT | GKLUD_FLAGS |
             GKLUD_AREA | GKLUD_MSGID | GKLUD_REPLY | GKLUD_PID | GKLUD_CHARSET |
             GKLUD_KNOWN | GKLUD_PATH | GKLUD_SEENBY);
    }
    ...
    else
    {
        ...
        if(AA->isnet())
        {
            ...
            if(CFG->useintl and (CFG->useintl ==
                                 YES or (msg->dest.zone != msg->orig.zone)))
            {
                sprintf(buf,
                        "\001INTL %u:%u/%u %u:%u/%u",
            ...
            if(msg->orig.point and msg->orig.net == msg->oorig.net and msg->orig.node ==
               msg->oorig.node)
            {
        ...
        strxmerge(msg->pid, sizeof(msg->pid), __gver_shortpid__, " ", __gver_ver__,
                  NULL);

        if(CFG->usepid and (CFG->switches.get(emptytearline) or not (striinc(
                                                                         __gver_longpid__,
                                                                         msg->tearline))))
        {
        ...
        if(CFG->usecharset and * msg->charset)
        {
            *buf = NUL;
            ...
            else if(not strieql(msg->charset, "COMPOSED"))
            {
                sprintf(buf,
                        "\001%s: %s",
                        CFG->switches.get(kludgechrs) ? "CHRS" : "CHARSET",
                        msg->charset);
            }
        ...
        if(AA->isinternet())
        {
            ...
            if(*msg->ito or strchr(msg->to, '@'))
            {
                ...
                /*--- if(_toname[0] != NUL) {
                   ...
                   if((buf2[0] == '\"') or (strpbrk(buf2, " \t") == NULL))
                    quot[0] = NUL;
                   sprintf(buf, "%s%sTo: %s%s%s <%s>", rfc, AA->isnewsgroup() ? "X-" : "", quot,
                      buf2, quot, _toaddr);
                   }
            ...
            if(CFG->internetviagate)
            {
                ...
                if(AA->Internetrfcbody() and not AA->isnewsgroup() and line->next)
                {
                    do
                    {
                        ...
                        else if(not strnieql(nline_txt, "XPost:",
                                             6) and not strnieql(nline_txt, "Copy:",
                                                                 5) and not strnieql(
                                    nline_txt, "BCopy:", 6))
                        {
                            line         = AddKludge(line, "");
            ...
            else
            {
                ...
                if(AA->isnewsgroup())
                {
                    sprintf(buf, "%sNewsgroups: %s", rfc,
                            strlwr(strcpy(buf2, AA->echoid())));
                    line         = AddKludge(line, buf);
                    line->kludge = GKLUD_RFC;
                }
                ...
                strlwr(encoding);
                sprintf(buf, "%sContent-Type: text/plain; charset=%s", rfc,
                        strlword(encoding));
                line         = AddKludge(line, buf);
                line->kludge = GKLUD_RFC;
                sprintf(buf,
                        ...
                        rfc,
                        isqp ? "quoted-printable" : isusascii ? "7bit" : "8bit");
                line         = AddKludge(line, buf);
                line->kludge = GKLUD_RFC;
                ...
                if((*msg->ireplyto or *
                    CFG->soupreplyto) and (not streql(*msg->ireplyto ? msg->ireplyto :
                                                      CFG->
                                                      soupreplyto, msg->iorig)))
                {
                    sprintf(buf,
                            "%sReply-To: %s",
                            rfc,
                            *msg->ireplyto ? msg->ireplyto : CFG->soupreplyto);
                    line         = AddKludge(line, buf);
                    line->kludge = GKLUD_RFC;
                }
                ...
                if(AA->isnewsgroup() or AA->isemail())
                {
                    sprintf(buf,
                            "%sX-%s: %s",
                            rfc,
                            AA->isnewsgroup() ? "Newsreader" : "Mailreader",
                            get_informative_string());
                    line         = AddKludge(line, buf);
                    line->kludge = GKLUD_RFC;
                }

                if(AA->Internetrfcbody() and line->next and not strblank(line->next->txt.
                                                                         c_str()))
                {
                    line         = AddKludge(line, "");
                    line->kludge = GKLUD_RFC;
