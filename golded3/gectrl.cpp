...
{
    return strieql("UUCP",
                   name) or (*AA->Internetgate().name and strieql(
                                 AA->Internetgate().name,
                                 name));
}
...
char * MakeOrigin(GMsg * msg, const char * orig)
{
    ...
    if(msg->orig.net)
    {
        msg->orig.make_string(buf,
                              *AA->Netname() ? (*msg->odom ? msg->odom : AA->Netname()) :
                              NULL);
    }
    else
    {
...
void DoKludges(int mode, GMsg * msg, int kludges)
{
    ...
    if(kludges == 0)
    {
        kludges = (GKLUD_RFC | GKLUD_FWD | GKLUD_INTL | GKLUD_FMPT | GKLUD_TOPT |
                   GKLUD_FLAGS | GKLUD_AREA | GKLUD_MSGID | GKLUD_REPLY | GKLUD_PID |
                   GKLUD_CHARSET | GKLUD_KNOWN | GKLUD_PATH | GKLUD_SEENBY);
    }
    ...
    else
    {
        ...
        if(AA->isinternet())
        {
            ...
            else
            {
                ...
                if(AA->isnewsgroup())
                {
                    sprintf(buf, "%sNewsgroups: %s", rfc, strlwr(strcpy(buf2,
                                                                        AA->echoid())));
                    line         = AddKludge(line, buf);
                    line->kludge = GKLUD_RFC;
                }
                ...
                if(AA->Internetrfcbody() and line->next and not strblank(
                       line->next->txt.c_str()))
                {
                    line         = AddKludge(line, "");
                    line->kludge = GKLUD_RFC;
