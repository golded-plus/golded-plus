               ...
               int & xc2to)
{
    ...
    if(AA->isnet())
    {
        ...
        *addr2          = NUL;
        ...
        if(set_to_address(&(*carbon)[cc], &toname, &toaddr, &fromaddr, NULL, 6,
                          LNG->DestinationCC, true))
        {
            addr.net = 0;
        }
        else
    ...
    if(addr.net or AA->islocal())
    {
        ...
        if(memcmp(&addr, &msg->dest, sizeof(Addr)) or stricmp(name,
                                                              msg->to) or (A !=
                                                                           AA and not
                                                                           xc2to++))
        {
            ...
            switch(CFG->carboncopylist)
            {
                ...
                case CC_HIDDEN:
                    ...
                    for(n = 0; n < cc; n++)
                    {
                        ...
                        {
                            gsprintf(PRINTF_DECLARE_BUFFER(buf), "\001CC: %s %s",
                                     (*carbon)[n].To(),
            ...
            if(not (newline->type & (GLINE_KLUDGE | GLINE_TEAR | GLINE_ORIG)))
            {
                ...
                if(not ignorexc and (strnieql(ptr, "XC:", 3) or strnieql(ptr, "XP:", 3)))
                {
                    if(ask)
                    {
                        ...
                        ignorexc = make_bool_not((CFG->crosspost != ASK) or MenuCross.Run(
                                                     msg));
