...
void guserbase::refresh_maximum_index()
{
    ...
    if(need_update)
    {
        maximum_index = (uint(usrbase.FileLength()) - sizeof(gusrbaseheader)) /
                        sizeof(gusrbaseentry) - 1;
    }
...
} // guserbase::refresh_maximum_index
...
void guserbase::do_delayed()
{
    ...
    update_statuslinef(LNG->UserStatusline,
                       "ST_USERSTATUSLINE",
                       index + 1,
                       maximum_index + 1,
                       maximum_index - index);
...
bool guserbase::edit_entry(uint idx)
{
    ...
    form.g = this;
    form.setup(C_HEADW, C_HEADW, C_HEADE, _box_table(W_BHEAD,
                                                     13), true);
    read_entry(idx);
    form.LoadForm();
    form.add_field(addressbook_form::id_name, 0, 13, 59, form.name, sizeof(entry.name));
    form.add_field(addressbook_form::id_macro, 1, 13, 20, form.macro,
                   sizeof(entry.macro));
    ...
    form.add_field(addressbook_form::id_iaddr, 2, 46, 26, form.iaddr,
                   sizeof(entry.iaddr));
    form.add_field(addressbook_form::id_organisation,
                   3,
                   13,
                   59,
                   form.organisation,
                   sizeof(entry.organisation));
    form.add_field(addressbook_form::id_voicephone, 5, 13, 20, form.voicephone,
                   sizeof(entry.voicephone));
    form.add_field(addressbook_form::id_faxphone, 6, 13, 20, form.faxphone,
                   sizeof(entry.faxphone));
    form.add_field(addressbook_form::id_dataphone, 7, 13, 20, form.dataphone,
                   sizeof(entry.dataphone));
    form.add_field(addressbook_form::id_group,
                   8,
                   13,
                   20,
                   form.group,
                   sizeof(entry.group) + 1);
    form.add_field(addressbook_form::id_snail1, 5, 46, 26, form.snail1,
                   sizeof(entry.snail1));
    form.add_field(addressbook_form::id_snail2, 6, 46, 26, form.snail2,
                   sizeof(entry.snail2));
    form.add_field(addressbook_form::id_snail3, 7, 46, 26, form.snail3,
                   sizeof(entry.snail3));
    form.add_field(addressbook_form::id_homepage, 8, 46, 26, form.homepage,
                   sizeof(entry.homepage));
    form.add_field(addressbook_form::id_comment1, 9, 13, 59, form.comment1,
                   sizeof(entry.comment1));
    form.add_field(addressbook_form::id_comment2, 10, 13, 59, form.comment2,
                   sizeof(entry.comment2));
    form.add_field(addressbook_form::id_comment3, 11, 13, 59, form.comment3,
                   sizeof(entry.comment3));
...
void guserbase::update_addressbook(GMsg * msg, bool reverse, bool force)
{
    ...
    if(not strblank(name))
    {
        if(find_entry(name))
        {
            if(not force)
            {
                if((CFG->addressbookadd ==
                    NO) or ((CFG->addressbookadd ==
                             YES) and not (AA->isnet() or AA->isemail())))
                {
                    return;
                }
            }
        ...
        else
        {
            if(not force)
            {
                ...
                // 2. It's a robotname
                for(gstrarray::iterator n = CFG->robotname.begin();
                    n != CFG->robotname.end(); n++)
                {
        ...
        if((AA->isinternet() and strblank(
                iaddr)) or (not AA->isinternet() and not fidoaddr.valid()))
        {
            return;
        }
        ...
        if(not AA->isinternet() and not *
           iaddr and (not AA->Internetgate().addr.valid() or (AA->Internetgate().addr !=
                                                              fidoaddr)))
        {
...
bool guserbase::lookup_addressbook(GMsg * msg, char * name, char * aka, bool browser)
{
    ...
    if(not strblank(name) or browser)
    {
        ...
        if((find_entry(name, true) and not entry.is_deleted) or browser)
        {
            ...
            if(not entry.fidoaddr.valid() and not strblank(
                   entry.iaddr) and not AA->isinternet())
            {
                // do UUCP addressing
...
void build_pseudo(GMsg * msg, bool direction)
{
    ...
    if(direction)
    {
        p.build_pseudo(msg,
                       strbtrim(msg->To()),
                       AA->isinternet() ? msg->iaddr : msg->dest.make_string(buf));
    }
    else
    {
        p.build_pseudo(msg,
                       strbtrim(msg->By()),
                       AA->isinternet() ? msg->iorig : msg->orig.make_string(buf),
                       false);
    }
}
