...
int ExternUtil(GMsg * msg, const ExtUtil & extutil)
{
    ...
    int mode            =
        (extutil.options & EXTUTIL_KEEPCTRL) ? MODE_SAVE : MODE_SAVENOCTRL;
    ...
    if(extutil.options & EXTUTIL_RELOAD)
    {
        ...
        if(not strstr(msg->txt, "\001CHRS:") and not strstr(msg->txt, "\001CHARSET:"))
        {
            ...
            char * msg_txt_chrs  = (char *)throw_malloc(strlen(msg->txt) + strlen(
                                                            charset) +
        ...
        if(AA->Viewkludge())
        {
            ...
            int level         =
                LoadCharset(CFG->xlatlocalset,
                            IsQuotedPrintable(inpcharset) ? ExtractPlainCharset(
                                inpcharset) : inpcharset);
            ...
            if(level)
            {
                ...
                throw_free(msg->txt);
                msg->txt =
                    (char *)throw_realloc(msg_txt_chrs, strlen(msg_txt_chrs) + 16);
            }
        }
