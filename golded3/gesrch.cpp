...
lblLoopExit:
    ...
    if(what && !flag)
    {
        ...
        if(what & GFIND_SUBJECT)
        {
            item.where.subject = true;
        }

        if(what & GFIND_BODY)
        {
            item.where.body = true;
        }

        if(what & GFIND_TAGLINE)
        {
            item.where.tagline = true;
        }

        if(what & GFIND_TEARLINE)
        {
            item.where.tearline = true;
        }

        if(what & GFIND_ORIGIN)
        {
            item.where.origin = true;
        }

        if(what & GFIND_KLUDGES)
        {
            item.where.kludges = true;
        }

        if(what & GFIND_SIGNATURE)
        {
            item.where.signature = true;
        }
    }
...
void golded_search_manager::prepare_from_string(const char * prompt, int what)
{
    ...
    while(*p);
} // golded_search_manager::prepare_from_string
...
bool golded_search_manager::search(GMsg * msg, bool quick, bool shortcircuit)
{
    ...
    for(int i = 0; i < items.size(); i++, item++)
    {
        ...
        if(item->where.body or item->where.tagline or item->where.tearline or item->where
           .origin or item->where.signature or item->where.kludges)
        {
            Line * line = msg->lin;
            ...
            while(line)
            {
                ...
                if(item->where.body and not (type &
                                             (GLINE_TAGL | GLINE_TEAR | GLINE_ORIG |
                                              GLINE_SIGN | GLINE_KLUDGE)))
                {
                ...
                if(search_this_line)
                {
                    if(item->search(line->txt.c_str()))
                    {
                        ...
                        if(type &
                           (GLINE_TAGL | GLINE_TEAR | GLINE_ORIG | GLINE_SIGN |
                            GLINE_KLUDGE))
                        {
                            if(type & GLINE_TAGL)
                            {
...
void AdvancedSearch(GMsg *, int &, int &)
{
    ...
    for(int y = 0; y < 6; y++, i++)
    {
        iform.add_field(100 + y,
                        ...
                        width - 15 - 3,
                        gwinput::cvt_none,
                        y >= 4 ? gwinput::entry_conditional : gwinput::entry_noedit);
    }
    iform.run(0);
    window.close();
} // AdvancedSearch
