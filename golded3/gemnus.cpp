...
int GMenuNavigate::Run() {
  ...
  Begin(M_HORZ);
    SetBorder(W_BASK, C_ASKB);
    SetColor(C_ASKW, C_ASKQ, C_ASKN, C_ASKS, CFG->switches.get(screenshadows) ? C_SHADOW : -1);
    Item(TAG_MAIN, "M Main ");
    BeginPullDown();
      Item(TAG_MAIN_SHELL, "S Shell to OS ");
      Item(TAG_MAIN_QUIT,  "Q Quit GoldED ");
    End();
    ...
    BeginPullDown();
      Item(TAG_SEL_ORIGIN,    "O Origin    ");
      Item(TAG_SEL_USERNAME,  "U Username  ");
      Item(TAG_SEL_AKA,       "A Aka       ");
      Item(TAG_SEL_ATTRS,     "M Msg attrs ");
      Item(TAG_SEL_TEMPLATE,  "T Template  ");
    End();
    Item(TAG_UTIL, "U Util ");
    BeginPullDown();
      Item(TAG_UTIL_CMF,   "C Copy/Move/Forward     ");
