...
int GMenuEditHeader::Run(int mode, GMsg * msg)
{
    ...
    do
    {
        ...
        do
        {
            _again = false;
            _tag   =
                (EDIT->Internal() ? TAG_INTERNAL : (*EDIT->External() ? TAG_EXTERNAL :
                                                    TAG_INTERNAL));
    ...
    else if((finaltag > TAG_LANG) && (finaltag < TAG_MORE))
    {
        if(!schecker.IsLoaded(langs[finaltag - TAG_LANG - 1]->GetLangCode()))
        {
            int save_chartableno = LoadCharset(NULL, NULL, 1); // Workaround: internal for
                                                               // LoadCharset() charset table
                                                               // number changed in the
                                                               // schecker.Load()
            schecker.Load(langs[finaltag - TAG_LANG - 1]->GetLangCode(),
                          CFG->scheckeruserdic);

            if(save_chartableno != -1) // restore value of the default chaset table //
                                       // workaround: internal for LoadCharset() charset table
                                       // number changed in the schecker.Load()
            {
