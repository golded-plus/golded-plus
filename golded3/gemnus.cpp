...
void GMnuAsk::AskInit(int __row, int __col, char * __title, int __help)
{
    ...
    SetColor(C_ASKW,
             ...
             C_ASKN,
             C_ASKS,
             CFG->switches.get(screenshadows) ? C_SHADOW : DEFATTR);
...
int GMenuDelete::Run(int all, GMsg * __msg)
{
    ...
    HandleGEvent(EVTT_ASKYESNO);
    const char * _info =
        (__msg->attr.uns() and not __msg->attr.rcv()) ? LNG->WarnUnsent : "";
...
int GMenuDomarks::Run(char * dowhat)
{
    ...
    update_statuslinef("%i %s %s",
                       "",
                       AA->Mark.Count(),
                       AA->Mark.Count() == 1 ? LNG->is : LNG->are,
                       LNG->marked);
...
void do_togglehdr()
{
    ...
    gwin.cmenu->citem->str    = (AA->Writeheader() ==
                                 1 ? LNG->WriteToHdrYES : AA->Writeheader() ==
                                 2 ? LNG->WriteToHdrONLY : LNG->WriteToHdrNO) + 1;
...
int GMenuWriteMsg::Run()
{
    enum
    {
        TAG_DISKFILE = 100, TAG_PRINTER, TAG_CLIPBRD, TAG_TOGGLEHDR, TAG_QUIT
    };
    ...
    Item(TAG_TOGGLEHDR,
         (AA->Writeheader() == 1 ? LNG->WriteToHdrYES : AA->Writeheader() ==
          2 ? LNG->WriteToHdrONLY : LNG->WriteToHdrNO), 0, do_togglehdr);
    Item(TAG_QUIT, LNG->QuitWrite);
    SetTag(_prevtag);
    End();
    Start();
    _prevtag = finaltag;
    int header = AA->Writeheader() == 1 ? 0 : AA->Writeheader() ==
                 2 ? WRITE_ONLY_HEADER : WRITE_NO_HEADER;
...
int GMenuImportTxt::Run()
{
    enum
    {
        TAG_TEXT = 100, TAG_QUOTE, TAG_BINUUE, TAG_BINMIME, TAG_CLIPBRD, TAG_XLAT,
        TAG_QUIT
    };

    // Dirk: Don't put a menu in the middle of the screen.
    // AskInit((MAXROW-8)/2+3, (MAXCOL-strlen(LNG->ImportTxtText))/2, LNG->ImportTxt,
    // H_ImportTxt);
...
int GMenuEditfile::Run(GMsg * __msg)
{
    ...
    enum
    {
        TAG_YES = 100, TAG_NO, TAG_CONTINUE, TAG_ROT13, TAG_ATTRS, TAG_TAGLINE,
        TAG_ORIGIN, TAG_VIEW, TAG_HEADER, TAG_UTILS = 1000
    };
    ...
    for( ;; )
    {
        update_statuslinef(LNG->Quotepct,
                           "ST_QUOTEPCT",
                           __msg->quotepct,
                           __msg->quotepct > 80 ? "!!!" : "");
        char _title[80];
            ...
            case TAG_TAGLINE:
                ...
                if(ChangeTagline())
                {
                    ...
                    while(_line)
                    {
                        ...
                        {
                            gsprintf(PRINTF_DECLARE_BUFFER(_buf),
                                     "%c%c%c %s ",
                                     AA->Taglinechar(),
                                     AA->Taglinechar(),
                                     AA->Taglinechar(),
...
enum
{
    TAG_CCPROCESS = 100, TAG_CCIGNORE, TAG_CCATTRS, TAG_CCLIST, TAG_CCLISTKEEP,
    TAG_CCLISTNAMES, TAG_CCLISTVISIBLE, TAG_CCLISTHIDDEN, TAG_CCLISTREMOVE
};
...
//  ------------------------------------------------------------------
int GMnuAreaDo::With(char * __title,
                     char * __all,
                     char * __marked,
                     char * __current,
                     int __full,
                     ...
                     int __help)
{
    enum
    {
        TAG_ALL = 100, TAG_MARKED, TAG_CURRENT, TAG_MATCHING, TAG_UNSCANNED, TAG_GROUP,
        TAG_QWK, TAG_IMPORTQWK, TAG_EXPORTQWK, TAG_SOUP, TAG_EXPORTSOUP, TAG_IMPORTSOUP,
        TAG_NO, TAG_LIST = 1000
    };
...
{
    return With((pmscan ? LNG->ScanPM : LNG->Scan_Areas),
                LNG->Scan_All,
                LNG->Scan_Marked,
                LNG->Scan_Current,
                true,
...
int GMenuEditHeader::Run(int mode, GMsg * msg)
{
    enum
    {
        TAG_INTERNAL = 100, TAG_EXTERNAL, TAG_SAVE, TAG_ATTRS, TAG_TEMPLATE, TAG_TAGLINE,
        TAG_ORIGIN, TAG_QUIT
    };
    ...
    do
    {
        ...
        do
        {
            _again = false;
            _tag   =
                (EDIT->Internal() ? TAG_INTERNAL :
                 (*EDIT->External() ? TAG_EXTERNAL : TAG_INTERNAL));

            if(EDIT->Menu())
            {
                Init();
                SetColor(C_MENUW, C_MENUQ, C_MENUN, C_MENUS,
                         CFG->switches.get(screenshadows) ? C_SHADOW : DEFATTR);
...
int GMenuNavigate::Run()
{
    enum
    {
        TAG_MAIN                = 100, TAG_MAIN_SHELL = (int)KK_ReadDosShell,
        TAG_MAIN_QUIT           = (int)KK_ReadQuitNow, TAG_EDIT = 101,
        TAG_EDIT_ENTER          = (int)KK_ReadNewMsg,
        TAG_EDIT_QUOTE          = (int)KK_ReadQuoteMsg,
        TAG_EDIT_DIRQUOTE       = (int)KK_ReadDirQuoteMsg,
        TAG_EDIT_COMMENT        = (int)KK_ReadCommentMsg, TAG_EDIT_OTHER = 1010,
        TAG_EDIT_OTHER_QUOTE    = (int)KK_ReadMoveQuoteMsg,
        TAG_EDIT_OTHER_DIRQUOTE = (int)KK_ReadMoveQuoteMsg,
        TAG_EDIT_OTHER_COMMENT  = (int)KK_ReadMoveCommentMsg, TAG_SEL = 102,
...
int GMenuSChecker::Run(CSpellChecker & schecker, const char * word)
{
    enum
    {
        TAG_ADDWORD = 100, TAG_LANG = 0x10000, TAG_MORE = 0x20000, TAG_INDEX = 0x30000
    };
    ...
    else if((finaltag > TAG_LANG) && (finaltag < TAG_MORE))
    {
        ...
        {
            int save_chartableno = GetCurrentTable(); // Workaround: internal for LoadCharset()
                                                      // charset table number changed in the
                                                      // schecker.Load()
            schecker.Load(langs[finaltag - TAG_LANG - 1]->GetLangCode(), NULL); // User
                                                                                // dictionary
                                                                                // will be
                                                                                // loaded if was
                                                                                // loaded during
                                                                                // initial
            LoadCharset(save_chartableno); // restore value of the default chaset table //
                                           // workaround: internal for LoadCharset() charset
                                           // table number changed in the schecker.Load()
        }
        else
        {
