            ...
            if(inhdr)
            {
                ...
                else
                {
                    ...
                    *lbuf = CTRL_A;
                    ...
                    {
                        char * ccbuf =
                            (char *)throw_malloc(strlen(msg->icc) + strlen(mptr) + 3);
                    ...
                    {
                        char * bccbuf =
                            (char *)throw_malloc(strlen(msg->ibcc) + strlen(mptr) + 3);
        ...
        while(line)
        {
            if(line->type & GLINE_KLUDGE)
            {
                if((line->kludge == GKLUD_RFC) or (line->kludge == 0))
                {
                    ...
                    XlatStr(mbuf, (*ltxt == CTRL_A) ? (ltxt + 1) : ltxt, level,
                            CharTable);
                    msglen += fp.Printf("%s%s",
                                        mbuf,
