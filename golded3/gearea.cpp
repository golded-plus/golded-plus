...
void GPickArealist::do_delayed()
{
    ...
    tmp[tmplen + 1] = '\xFF';
    tmp[tmplen + 2] = '\0';
    update_statuslinef("%s: %u %s, %u %s, %u %s",
                       "",
                       area->echoid(),
                       area->Msgn.Count(),
                       (area->Msgn.Count() == 1 ? LNG->msg : LNG->msgs),
                       area->unread,
                       LNG->unread,
                       area->PMrk.Count(),
                       LNG->personal);
    ...
    if(buf[buflen] || buf[buflen + 1] != '\xFF' || buf[buflen + 2])
    {
        ...
        LOG.printf(
            "! Buffer overflow: buf in GPickArealist::do_delayed(), 8 or 9 lines above");
        PointerErrorExit();
    }
    ...
    if(tmp[tmplen] || tmp[tmplen + 1] != '\xFF' || tmp[tmplen + 2])
    {
        LOG.errpointer(__FILE__, __LINE__);
        LOG.printf(
            "! Buffer overflow: tmp in GPickArealist::do_delayed(), 15 or 16 lines above");
        PointerErrorExit();
    }

    delete[] buf;
    delete[] tmp;
} // GPickArealist::do_delayed
...
void GPickArealist::dispbuf(char * buf, int areano)
{
    ...
    buf[MAXCOL - 2] = NUL;
    char areabuf[33];
    gsprintf(PRINTF_DECLARE_BUFFER(areabuf),
             "%u",
             CFG->switches.get(arealistnos) ? area->board() : areanumbers[areano]);
    ...
    *pmarkbuf = area->PMrk.Count() ? pmark_char : ' ';
    ...
    if(area->isscanned)
    {
        gsprintf(PRINTF_DECLARE_BUFFER(unreadbuf), "%u",
                 (uint)((CFG->arealisttype ==
                         AL_TOTNEW) ? area->unread : area->lastread()));
    }
    else
    {
        strcpy(unreadbuf, "-");
    ...
    *changedbuf = area->isunreadchg ? changed_char : ' ';
    ...
    if(groupid_width)
    {
        if(area->groupid() & 0x8000u)
        {
            if(groupid_width > 2)
            {
                gsprintf(PRINTF_DECLARE_BUFFER(groupidbuf),
                         "%u",
                         area->groupid() & 0x7FFF);
            }
        }
...
void GPickArealist::jumpmatch()
{
    ...
    for( ; n <= maximum_index; n++)
    {
        if(not AL[n]->isseparator() and (strnieql(area_maybe, AL[n]->echoid(),
                                                  area_fuzidx) or striinc(area_maybe,
                                                                          AL[n]->echoid())))
        {
            found = true;
            break;
...
void GPickArealist::open()
{
    ...
    else
    {
        ...
        if(area_width)
        {
            wmessage(LNG->Area, TP_BORD,
                     1 + area_pos + (marked_pos == area_pos + area_width ? 1 : 0),
                     tattr);
        }
        ...
        if(count_width)
        {
            wmessage(LNG->Msgs,
                     TP_BORD,
                     1 + count_pos + count_width - strlen(LNG->Msgs),
                     tattr);
        }
        ...
        if(unread_width)
        {
            ...
            {
                wmessage(LNG->Last, TP_BORD,
                         1 + unread_pos + unread_width - strlen(LNG->Last), tattr);
            }
            else if(CFG->arealisttype == AL_TOTNEW)
            {
                wmessage(LNG->New, TP_BORD,
                         1 + unread_pos + unread_width - strlen(LNG->New), tattr);
            }
        }
    ...
    if(CFG->switches.get(areaautonext) and ypos == 0 and index >=
       minimum_index and not AL[index]->ismarked())
    {
        jump_to();
    }
    else
...
void GPickArealist::AreaCatchUp(uint n)
{
    ...
    if(mode != SCAN_QUIT)
    {
        ...
        {
            if((mode == SCAN_MARKED and (*AL.item)->ismarked()) or mode ==
               SCAN_ALL or (mode == SCAN_CURRENT and (*AL.item) == AL[n]))
            {
                ...
                if(CFG->switches.get(highlightunread) and CFG->switches.get(
                       areacatchupread))
                {
                    w_info(LNG->Wait);
...
void GPickArealist::AreaDropMsgMarks(uint n)
{
    ...
    CREATEBUFFER(char, buf, MAXCOL);
    gsprintf(PRINTF_DECLARE_BUFFER_AUTO(buf, MAXCOL), LNG->DropMarksInfo,
             longdotstr(nummarks));
    w_info(buf);
...
} // GPickArealist::AreaDropMsgMarks
    ...
    switch(key)
    {
        ...
        case KK_AreaHeat:
        {
            ...
            if(mode != SCAN_QUIT)
            {
                ...
                {
                    if((mode == SCAN_MARKED and (*AL.item)->ismarked()) or mode ==
                       SCAN_ALL or (mode == SCAN_CURRENT and (*AL.item) ==
                                    AL[index - 1]))
                    {
                        update_statuslinef("%s %s",
                                           "",
                                           1 + LNG->ScanningArea,
                                           (*AL.item)->echoid());
                ...
                if((area_fuzidx < area_maxfuz) or (key == Key_BS))
                {
                    ...
                    if(not iscntrl(n) or (key == Key_BS))
                    {
                        ...
                        if(area_fuzidx)
                        {
                            ...
                            if(x)
                            {
                                if(CFG->switches.get(arealistnos) and (isdigit(
                                                                           area_maybe[1])
                                                                       or (area_maybe[1]
                                                                           == NUL)))
                                {
