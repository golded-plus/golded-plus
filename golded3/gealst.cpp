    ...
    while(*ptr)
    {
        switch(*ptr)
        {
            ...
            case 'p':
                ...
                else if((cmp = compare_two(B->PMrk.Count() ? 1 : 0,
                                           A->PMrk.Count() ? 1 : 0)) != 0)
                {
                    return cmp;
                }
            ...
            case 't':
                ...
                if((cmp = compare_two(CFG->areatypeorder[A->type() & 0xFF],
                                      CFG->areatypeorder[B->type() & 0xFF])) != 0)
                {
                    return cmp;
                }
            ...
            case 'x':
                ...
                else if((cmp = strcmp(A->basetype().c_str(),
                                      B->basetype().c_str())) != 0)
                {
                    return cmp;
                }
...
int AreaList::AreaNoToId(int __areano)
{
    ...
    else
    {
        ...
        LOG.printf(": Tried to access record %i, but the range was only 0 - %u.",
                   __areano,
                   (uint)(idx.size() - 1));
...
bool SelMaskPick::handle_key()
{
    switch(key)
    {
        ...
        default:
            ...
            {
                kbput(key);
                edit_string(AL.alistselections[index],
                            DESC_LEN - 1,
                            LNG->SelectMarksEdit,
                            H_SelectMarks);
                display_bar();
