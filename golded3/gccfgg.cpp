...
bool ReadGoldedCfg(int & force)
{
    ...
    if(ReadCfg(golded_cfg))
    {
        ...
        if(*CFG->temppath == NUL)
        {
            strcpy(CFG->temppath, CFG->goldpath);
        }

        if(*CFG->areapath == NUL)
        {
            strcpy(CFG->areapath, CFG->goldpath);
        }

        if(*CFG->inboundpath == NUL)
        {
            strcpy(CFG->inboundpath, CFG->goldpath);
        }

        if(*CFG->soundpath == NUL)
        {
            strcpy(CFG->soundpath, CFG->goldpath);
        }

        if(*CFG->cookiepath == NUL)
        {
            strcpy(CFG->cookiepath, CFG->goldpath);
        }

        if(*CFG->templatepath == NUL)
        {
            strcpy(CFG->templatepath, CFG->goldpath);
        }
        ...
        MakePathname(CFG->semaphore.importlist, CFG->areapath,
                     CFG->semaphore.importlist);
        MakePathname(CFG->semaphore.exportlist, CFG->areapath,
                     CFG->semaphore.exportlist);
        ...
        MakePathname(CFG->semaphore.scannetmail,
                     CFG->goldpath,
                     CFG->semaphore.scannetmail);
        MakePathname(CFG->semaphore.pmscanall, CFG->goldpath, CFG->semaphore.pmscanall);
        MakePathname(CFG->semaphore.pmscanthis, CFG->goldpath,
                     CFG->semaphore.pmscanthis);
        MakePathname(CFG->semaphore.pmscannetmail,
                     CFG->goldpath,
                     CFG->semaphore.pmscannetmail);
        ...
        MakePathname(CFG->semaphore.soupimport, CFG->goldpath,
                     CFG->semaphore.soupimport);
        MakePathname(CFG->semaphore.soupexport, CFG->goldpath,
                     CFG->semaphore.soupexport);
        ...
        {
            STD_PRINTNL(
                "* Warning: SEMAPHORE EXPORTLIST must not be the same as ECHOMAIL.JAM!");
            SayBibi();
            cfgerrors++;
        }
    }
...
void WriteGoldGed()
{
    ...
    for(AL.item = AL.idx.begin(); AL.item != AL.idx.end(); AL.item++)
    {
        ...
        for(i = CFG->areascan.begin(); i != CFG->areascan.end(); i++)
        {
            ...
            if(((current[0] == '>') and ((*AL.item)->groupid() ==
                                         getgroup(current +
                                                  1))) or strwild((*AL.item)->echoid(),
                                                                  current))
            {
...
struct AF_entry idetect[] = {{
                             ...
                             {"DBRIDGE",    "dbridge.prm",
                              "D\'Bridge"               },
                             {"SUPERBBS",   "sconfig.bbs",
                              "SuperBBS"                },
                             {"SBBS",       "sconfig.bbs",
                              "SuperBBS"                },
                             {"QUICKBBS",   "quickcfg.dat",
                              "QuickBBS"                },
                             {"QBBS",       "quickcfg.dat",
                              "QuickBBS"                },
                             {"QUICKBBS",   "config.bbs",
    ...
    if(fp.isopen())
    {
        ...
        for(uint i = 0; i < sizeof(idetect) / sizeof(struct AF_entry); i++)
        {
            ...
            if(fexist(AddPath(pth, idetect[i].configname)))
            {
                ...
                STD_PRINTNL("Found " << idetect[i].name <<
                            (ptr ? "." : " (unreliable)."));
...
int InstallFinish()
{
    ...
    if(fp.isopen())
    {
        ...
        {
            if(EnterString("Please enter the path to your *.msg netmail area:", buf,
                           sizeof(buf)))
            {
                return -1;
            }
            ...
            if(EnterString("Please enter the path *and* filename of your areas.bbs file:",
                           buf, sizeof(buf)))
            {
                return -1;
            }
        ...
        {
            if(EnterString("Please enter the path to your Hudson msgbase files:", buf,
                           sizeof(buf)))
            {
                return -1;
            }
        ...
        {
            if(EnterString("Please enter the path to your Goldbase msgbase files:", buf,
                           sizeof(buf)))
            {
                return -1;
            }
        ...
        {
            if(EnterString("Please enter the path where net/echomail.jam can be placed:",
                           buf, sizeof(buf)))
            {
                return -1;
            }
...
CfgGed::CfgGed()
{
    ...
    *stylecodestops = 0;
    gsprintf(PRINTF_DECLARE_BUFFER(tasktitle),
             "%s%s%s%s",
             __gver_prename__,
             __gver_name__,
             __gver_postname__,
    ...
    externoptions      = EXTUTIL_CLS | EXTUTIL_CURSOR | EXTUTIL_RELOAD |
                         EXTUTIL_PAUSEONERROR | EXTUTIL_KEEPCTRL;
    ezycomuserno        = 0;
    fidomsgtype         = "OPUS";
    fidouserno          = 0;
    forcetemplate       = false;
