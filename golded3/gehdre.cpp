...
public: enum
    {
        id_from_name, id_from_addr, id_to_name, id_to_addr, id_subject
    };
...
bool GMsgHeaderEdit::handle_other_keys(gkey & key)
{
    ...
    switch(key)
    {
        case KK_HeaderAddressbook:
            vcurhide();
            lookup_addressbook(msg,
                               get_field(id_to_name)->buf,
                               get_field(id_to_addr)->buf,
                               true);
...
int EditHeaderinfo(int mode, GMsgHeaderView & view, bool doedithdr)
{
    ...
    if(doedithdr)
    {
        ...
        int name_cvt      =
            AA->Editmixcase() ? GMsgHeaderEdit::cvt_mixedcase : GMsgHeaderEdit::cvt_none;
        ...
        hedit.add_field(GMsgHeaderEdit::id_from_name,
                        2,
                        from_name_pos,
                        from_name_len,
                        from_name,
                        sizeof(INam),
                        name_cvt);
        hedit.add_field(GMsgHeaderEdit::id_from_addr,
                        2,
                        from_addr_pos,
                        from_addr_len,
                        from_addr,
                        sizeof(IAdr),
                        addr_cvt);
        hedit.add_field(GMsgHeaderEdit::id_to_name,
                        3,
                        to_name_pos,
                        to_name_len,
                        to_name,
                        sizeof(INam),
                        name_cvt);
        hedit.add_field(GMsgHeaderEdit::id_to_addr,
                        3,
                        to_addr_pos,
                        to_addr_len,
                        to_addr,
                        sizeof(IAdr),
                        addr_cvt);
        hedit.add_field(GMsgHeaderEdit::id_subject,
                        4,
                        subject_pos,
                        subject_len,
                        subject,
        ...
        view.window.prints(5,
                           view.width - strlen(LNG->HeaderEditHelp1) - 1,
                           view.title_color,
                           LNG->HeaderEditHelp1);
        view.window.prints(5,
                           view.width - strlen(LNG->HeaderEditHelp1) -
                           strlen(LNG->HeaderEditHelp2) - 3,
                           view.title_color,
                           LNG->HeaderEditHelp2);
        hedit.run(H_Header);
        vcurhide();
    ...
    if(not hedit.dropped and not gkbd.quitall)
    {
        ...
        if(AA->isinternet())
        {
            ...
            if(*msg->realby and * msg->iorig)
            {
                ...
                gsprintf(PRINTF_DECLARE_BUFFER(msg->ifrom),
                         "%s%s%s <%s>",
                         quot,
                         msg->realby,
                         quot,
            ...
            if(*msg->realto)
            {
                ...
                gsprintf(PRINTF_DECLARE_BUFFER(msg->ito),
                         "%s%s%s <%s>",
                         quot,
                         msg->realto,
                         quot,
        ...
        else
        {
            ...
            if(*msg->iaddr)
            {
                if(not (CFG->internetgateexp ==
                        RFCAddress) and * msg->To() and (strpbrk(msg->iaddr,
                                                                 "<>()\"") ==
                                                         NULL) and not isuucp(msg->To()))
                {
                    Name name;
                    strcpy(name, msg->To());
                    StripQuotes(name);
            ...
            // Try to match akas with the dest address, but only if the orig address was NOT
            // changed
