            ...
            if(ofp.isopen())
            {
                ...
                while(ifp.Fgets(buf, sizeof(buf)))
                {
                    if(strnieql(buf, "*B ", 3))
                    {
                        ...
                        ptr = strchr(buf, ',');
                        strbtrim(strxcpy(HlpL[counter].category, ptr ? ptr + 1 : "",
                            ...
                            if(line < 6)
                            {
                                switch(line)
                                {
                                    ...
                                    case 4:
                                    {
                                        ...
                                        {
                                            STD_PRINTNL("* " <<
                                                        AddPath(CFG->xlatpath,
                                                                xlt->mapfile) << ": At line 4 charset name '" << tp << "' too long. It is supposed no more than " <<
                                ...
                                while(*ptr and x < 5)
                                {
                                    if(*ptr == '\\')
                                    {
                                        ...
                                        switch(*ptr)
                                        {
                                            ...
                                            case 'd':
                                                EscTable.t[n][x++] = (uint8_t)atoi(
                                                    ptr + 1);
                                                ptr = strskip_txt(ptr);
                                                break;

                                            case 'x':
                                                EscTable.t[n][x++] = (uint8_t)atox(
                                                    ptr + 1);
                                                ptr = strskip_txt(ptr);
                                                break;
