...
bool PopupLocked(long tries, int isopen, const char * file)
{
    ...
    update_statuslinef(LNG->RetryOrESC,
                       "ST_RETRYORESC",
                       isopen ? LNG->RetryLock : LNG->RetryOpen,
                       tries);
...
void doinvalidate(char * text, const char * find, const char * replace, bool is_tearline)
{
    ...
    if(strnieql(text, find,
                n) and (not is_tearline or (text[n] == NUL) or isspace(text[n])))
    {
        char buf[256];
        strcpy(buf, text);
...
int AkaMatchOne(const ftn_addr * mask, const ftn_addr * addr)
{
    ...
    if((mask->zone == 0) or (addr->zone == 0) or (mask->zone ==
                                                  GFTN_ALL) or (mask->zone ==
                                                                addr->zone))
    {
        match++;
...
void ScreenBlankIdle()
{
    ...
    struct tm tm;
    glocaltime(&tm, &t);
    sprintf(blankmsg1, " %s %s %s ", __gver_longpid__, __gver_ver__,
            strftimei(blankmsg2, 40, LNG->StatusLineTimeFmt, &tm));
    ...
    if(blanked and (whb == -1))
    {
        ...
        {
            whh =
                wopen_(ry, rx, windowheight, blankmsglen + 2, W_BINFO, C_INFOB, C_INFOW);
        }
