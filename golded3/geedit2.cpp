...
void IEclass::BlockDel(Line * anchor)
{
    ...
    if(firstcutline != lastcutline)
    {
        ...
        prow = __prow;
        Undo->PushItem(EDIT_UNDO_DEL_TEXT | BATCH_MODE,
                       firstcutline,
                       firstcol,
                       __len - firstcol);
    ...
    if(firstcutline != lastcutline)
    {
        do
        {
            Undo->PushItem(EDIT_UNDO_DEL_LINE | BATCH_MODE,
                           firstcutline = firstcutline->next);
        }
    ...
    if(Edit__pastebuf)
    {
        ...
        while(_pasteline)
        {
            ...
            if(_pasteline->txt.find('\n') != _pasteline->txt.npos)
            {
                ...
                Line * _newline = insertlinebelow(currline,
                                                  currline->txt.c_str() + col,
                                                  BATCH_MODE);
...
void IEclass::editimport(Line * __line, char * __filename, bool imptxt)
{
    ...
    if(in_range(ImportMode, 0, 3) and edit_pathname(CFG->inputfile, sizeof(Path),
                                                    LNG->ImportWhichFile, H_ImportFile))
    {
        ...
        else
        {
            ...
            if(strpbrk(filenamebuf.c_str(), "*?"))
            {
                ...
                update_statuslinef(LNG->ImportStatus,
                                   "ST_IMPORTSTATUS",
                                   filenamebuf.c_str());
                // Start the file picker
                fileselected = wpickfile(win_minrow,
                                         win_mincol,
                                         win_maxrow,
                                         win_maxcol,
                                         W_BMENU,
    ...
    if(fileselected or getclip)
    {
        ...
        if(getclip ? clipbrd.openread() : (fp.Fopen(filenamebuf.c_str(),
                                                    binary ? "rb" : "rt",
                                                    CFG->sharemode)) != NULL)
        {
            if(isPipe)
            ...
            const char * imp_filename =
                (getclip or isPipe) ? filenamebuf.c_str() : CleanFilename(
                    filenamebuf.c_str());
            ...
            if(delta > 0)
            {
                ...
                imp_filename = (getclip or isPipe) ? filenamebuf.c_str() : CleanFilename(
                    filenamebuf.c_str());
            }

            update_statuslinef(LNG->ImportStatus, "ST_IMPORTSTATUS",
                               filenamebuf.c_str());
                ...
                while(true)
                {
                    ...
                    *optr++ = uuencode_enc(n);
                    ...
                    for(int i = 0; i < n; i += 3, iptr += 3)
                    {
                        ...
                        *optr++ =
                            uuencode_enc(((*iptr << 4) & 060) | ((iptr[1] >> 4) & 017));
                        *optr++ =
                            uuencode_enc(((iptr[1] << 2) & 074) | ((iptr[2] >> 6) & 03));
            ...
            else
            {
                ...
                // Read paragraphs
                while(getclip ? clipbrd.read(_parabuf,
                                             EDIT_PARABUFLEN - 7) : fp.Fgets(_parabuf,
                                                                             EDIT_PARABUFLEN
                                                                             - 7))
                {
                    ...
                    else
                    {
                        ...
                        {
                            doinvalidate(buf,
                                         CFG->invalidate.cc.first.c_str(),
                                         CFG->invalidate.cc.second.c_str());
                        }
                        ...
                        {
                            doinvalidate(buf,
                                         CFG->invalidate.xc.first.c_str(),
                                         CFG->invalidate.xc.second.c_str());
                        }
                        ...
                        {
                            doinvalidate(buf,
                                         CFG->invalidate.xp.first.c_str(),
                                         CFG->invalidate.xp.second.c_str());
                        }
                    }
                    ...
                    uint _wrapmargin =
                        (_newline->type & GLINE_QUOT) ? marginquotes : margintext;
...
void IEclass::editexport(Line * __exportline, int __endat)
{
    ...
    if(edit_string(Edit__exportfilename, sizeof(Path), LNG->ExportWhatFile,
                   H_ExportFile))
    {
        ...
        gfile _fp(_filenameptr,
                  (*Edit__exportfilename == '+') ? "at" : "wt",
                  CFG->sharemode);

        if(_fp.isopen())
        {
            update_statuslinef(LNG->ExportStatus, "ST_EXPORTSTATUS",
                               Edit__exportfilename);
            _fp.Fputc('\n');
    ...
    if(lines[0] == 0)
    {
        if(lines[1] == 0)
        {
            if(lines[2] == 0)
            {
                if(lines[3] == 1)
                {
                    return _box_table(0, 3);                //vs
                }

                if(lines[3] == 2)
                {
                    return _box_table(1, 3);                //vd
                }
            }
            else if(lines[2] == 1)
            {
        ...
        else if(lines[1] == 1)
        {
            if(lines[2] == 0)
            {
                if(lines[3] == 0)
                {
                    return _box_table(0, 1);                //hs
                }

                if(lines[3] == 1)
                {
                    return _box_table(0, 0);                //lsus
                }

                if(lines[3] == 2)
                {
                    return _box_table(2, 0);                //ldus
                }
            }
            else if(lines[2] == 1)
            {
                if(lines[3] == 0)
                {
                    return _box_table(0, 5);                //lsds
                }

                if(lines[3] == 1)
                {
                    return _box_table(0, 9);                //lshs
                }
            }
            else if(lines[2] == 2)
            {
                if(lines[3] == 0)
                {
                    return _box_table(2, 5);                //ldds
                }

                if(lines[3] == 2)
                {
                    return _box_table(2, 9);                //ldhs
                }
            }
        }
        else if(lines[1] == 2)
        {
            if(lines[2] == 0)
            {
                if(lines[3] == 0)
                {
                    return _box_table(1, 1);                //hd
                }

                if(lines[3] == 1)
                {
                    return _box_table(3, 0);                //lsud
                }

                if(lines[3] == 2)
                {
                    return _box_table(1, 0);                //ldud
                }
            }
            else if(lines[2] == 1)
            {
                if(lines[3] == 0)
                {
                    return _box_table(3, 5);                //lsdd
                }

                if(lines[3] == 1)
                {
                    return _box_table(3, 9);                //lshd
                }
            }
            else if(lines[2] == 2)
            {
                if(lines[3] == 0)
                {
                    return _box_table(1, 5);                //lddd
                }

                if(lines[3] == 2)
                {
                    return _box_table(1, 9);                //ldhd
                }
            }
        }
    }
    else if(lines[0] == 1)
    {
        if(lines[1] == 0)
        {
            if(lines[2] == 0)
            {
                ...
                if(lines[3] == 1)
                {
                    return _box_table(0, 2);                //rsus
                }

                if(lines[3] == 2)
                {
                    return _box_table(2, 2);                //rdus
                }
            }
            else if(lines[2] == 1)
            {
                if(lines[3] == 0)
                {
                    return _box_table(0, 7);                //rsds
                }

                if(lines[3] == 1)
                {
                    return _box_table(0, 10);               //rshs
                }
            }
            else if(lines[2] == 2)
            {
                if(lines[3] == 0)
                {
                    return _box_table(2, 7);                //rdds
                }

                if(lines[3] == 2)
                {
                    return _box_table(2, 10);               //rdhs
                }
            }
        }
        else if(lines[1] == 1)
        {
            if(lines[2] == 0)
            {
                ...
                if(lines[3] == 1)
                {
                    return _box_table(0, 11);               //vsus
                }

                if(lines[3] == 2)
                {
                    return _box_table(2, 11);               //vdus
                }
            }
            else if(lines[2] == 1)
            {
                if(lines[3] == 0)
                {
                    return _box_table(0, 12);               //vsds
                }

                if(lines[3] == 1)
                {
                    return _box_table(0, 8);                //vshs
                }
            }
            else if(lines[2] == 2)
            {
                if(lines[3] == 0)
                {
                    return _box_table(2, 12);               //vdds
                }

                if(lines[3] == 2)
                {
                    return _box_table(2, 8);                //vdhs
                }
            }
        }
    }
    else if(lines[0] == 2)
    {
        if(lines[1] == 0)
        {
            if(lines[2] == 0)
            {
                ...
                if(lines[3] == 1)
                {
                    return _box_table(3, 2);                //rsud
                }

                if(lines[3] == 2)
                {
                    return _box_table(1, 2);                //rdud
                }
            }
            else if(lines[2] == 1)
            {
                if(lines[3] == 0)
                {
                    return _box_table(3, 7);                //rsdd
                }

                if(lines[3] == 1)
                {
                    return _box_table(3, 10);               //rshd
                }
            }
            else if(lines[2] == 2)
            {
                if(lines[3] == 0)
                {
                    return _box_table(1, 7);                //rddd
                }

                if(lines[3] == 2)
                {
                    return _box_table(1, 10);               //rdhd
                }
            }
        }
        else if(lines[1] == 2)
        {
            if(lines[2] == 0)
            {
                ...
                if(lines[3] == 1)
                {
                    return _box_table(3, 11);              //vsud
                }

                if(lines[3] == 2)
                {
                    return _box_table(1, 11);              //vdud
                }
            }
            else if(lines[2] == 1)
            {
                if(lines[3] == 0)
                {
                    return _box_table(3, 12);              //vsdd
                }

                if(lines[3] == 1)
                {
                    return _box_table(3, 8);               //vshd
                }
            }
            else if(lines[2] == 2)
            {
                if(lines[3] == 0)
                {
                    return _box_table(1, 12);              //vddd
                }

                if(lines[3] == 2)
                {
                    return _box_table(1, 8);               //vdhd
                }
            }
        }
    }
