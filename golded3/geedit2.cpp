...
void IEclass::editimport(Line * __line, char * __filename, bool imptxt)
{
    ...
    if(in_range(ImportMode, 0, 3) and edit_pathname(CFG->inputfile,
                                                    sizeof(Path),
                                                    LNG->ImportWhichFile,
                                                    H_ImportFile))
    {
    ...
    if(fileselected or getclip)
    {
                                                    ...
                                                    CFG->sharemode)) != NULL)
        {
            ...
            const char * imp_filename =
                (getclip or isPipe) ? filenamebuf.c_str() :
                CleanFilename(filenamebuf.c_str());
                ...
                while(true)
                {
                    ...
                    *optr++ = uuencode_enc(n);
                    ...
                    for(int i = 0; i < n; i += 3, iptr += 3)
                    {
                        ...
                        *optr++ = uuencode_enc(((*iptr << 4) & 060) | ((iptr[1] >>
                                                                        4) & 017));
                        *optr++ = uuencode_enc(((iptr[1] << 2) & 074) | ((iptr[2] >>
                                                                          6) & 03));
            ...
            else
            {
                                                                             ...
                                                                             - 7))
                {
                    ...
                    uint _wrapmargin = (_newline->type &
                                        GLINE_QUOT) ? marginquotes : margintext;
