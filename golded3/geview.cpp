...
void GMsgHeaderView::Paint()
{
    ...
    strxmerge(top,
              ...
              " ",
              buf1,
              strtrim(strtmp(area->desc())),
              " (",
              area->isinternet() ? area->Internetaddress() : area->Aka().addr.make_string(
                  buf),
              ") ",
              NULL);
    ...
    if(CFG->switches.get(disprealmsgno))
    {
        ptr += sprintf(ptr,
                       ...
                       LNG->Msg,
                       msg->msgno,
                       area->Msgn.Count() + (msg->attr.nwm() ? 1 : 0));
        replyto    = msg->link.to();
    ...
    else
    {
        ...
        ptr += sprintf(ptr,
                       " %-5.5s: %u %s %u",
                       LNG->Msg,
                       msg->attr.nwm() ? active : area->Msgn.ToReln(msg->msgno),
                       LNG->of,
                       active);
    ...
    for(int replyn = 0, plus = 0;
        (replyn <
         (list_max + 1)) and (not attrsgenerated or ((ptr - buf) <
                                                     CFG->disphdrnodeset.pos));
        replyn++)
    {
        if(replies[replyn])
        {
    ...
    {
        strxmerge(buf, (namewidth + nodewidth), msg->realby, " <", msg->iorig, ">",
                  NULL);
    }
    else if((not area->isecho() or area->isnewsgroup()) and * msg->ifrom and *
            msg->iorig)
    {
    ...
    vattr color =
        ((msg->foundwhere &
          GFIND_FROM) or msg->attr.fmu() or (msg->attr.loc() and CFG->switches.get(
                                                 displocalhigh))) ? highlight_color :
        from_color;
    ...
    if(not area->isinternet())
    {
        if(not (*msg->ito and (*msg->realto or * msg->idest)))
        {
            if(msg->dest.net and area->isnet())
            {
                ...
                if(msg->odest.net)
                {
                    if((msg->odest.net != msg->dest.net) or (msg->odest.node !=
                                                             msg->dest.node))
                    {
    ...
    {
        strxmerge(buf, (namewidth + nodewidth), msg->realto, " <", msg->idest, ">",
                  NULL);
    }
    ...
    color =
        ((msg->foundwhere & GFIND_TO) or msg->attr.tou()) ? highlight_color : to_color;
    {
        Addr zero;
    ...
    strxcpy(buf,
            (msg->attr.att() or msg->attr.frq() or msg->attr.urq()) ? LNG->File : LNG->Subj,
            10);
    ...
    window.prints(4,
                  lngsubjlen,
                  (msg->foundwhere & GFIND_SUBJECT) ? highlight_color : subject_color,
                  buf);
    ...
    if(msg->attr.att())
    {
        if(CFG->dispattachsize)
        {
            ...
            while(ptr)
            {
                ...
                if((g_isalpha(*ptr) and (ptr[1] == ':')) or (ptr[0] ==
                                                             '\\') or (ptr[0] == '/'))
                {
                    strcpy(buf2, ptr);
                }
    ...
    if(CFG->disphdrlocation && !_in_editor)
    {
        ...
        if(AA->isnet())
        {
            ...
            if(loc.length())
            {
                loc += CFG->locationdelimiter + loc2;
            }
        }
...
void GMsgBodyView::PaintLine(int row, Line * line)
{
    ...
    if(not SearchHighlight(line, vrow, visible_width, highlight_color))
    {
        if(line->type & GLINE_ORIG and strneql(line->txt.c_str(), " * Origin: ", 11))
        {
            ...
            StyleCodeHighlight(line->txt.c_str() + 11,
                               vrow,
                               11,
                               not AA->attr().hex() and AA->adat->hidestylies,
                               color);
        }
        else
        {
            StyleCodeHighlight(line->txt.c_str(),
                               vrow,
                               0,
                               not AA->attr().hex() and AA->adat->hidestylies,
                               color);
