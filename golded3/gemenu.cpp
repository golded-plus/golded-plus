...
void DispHeadAttrs(GMsg * msg)
{
    ...
    HeaderView->window.prints(1, CFG->disphdrnodeset.pos, HeaderView->window_color,
                              atrs);
    HeaderView->window.prints(4,
                              0,
                              HeaderView->window_color,
                              (msg->attr.att() or msg->attr.frq() or msg->attr.urq()) ?
                              LNG->File : LNG->Subj);
}
...
//  ------------------------------------------------------------------
int SelectFromFile(const char * file,
                   char * selection,
                   const char * title,
                   const char * nolines)
{
...
int ChangeTagline()
{
    ...
    if(not CFG->tagline.empty())
    {
        ...
        if(n != -1)
        {
            ...
            if(tagl[0] == '@')
            {
                strxmerge(buf, MAXCOL - 2 - 2, LNG->Taglines, " [",
                          CleanFilename(tagl + 1), "] ", NULL);
...
int ChangeUsername()
{
    if(not CFG->username.empty())
    {
        ...
        if(n != -1)
        {
            ...
            for(std::vector<gaka>::iterator a = CFG->aka.begin(); a != CFG->aka.end();
                a++)
            {
    ...
    if(not CFG->xlatcharsets.empty())
    {
        ...
        for(size_t xlatimports = 1; xlt != end; xlt++)
        {
            if(strieql(xlt->first.second.c_str(), CFG->xlatlocalset))
            {
                ...
                if((CFG->ignorecharset == true) and strieql(xlt->first.first.c_str(),
                                                            AA->Xlatimport()))
                {
                    startat = xlatimports;
                }
        ...
        for(xlt = CFG->xlatcharsets.begin(); xlt != end; ++xlt)
        {
            if(strieql(xlt->first.second.c_str(), CFG->xlatlocalset))
            {
                gsprintf(PRINTF_DECLARE_BUFFER(buf),
                         ...
                         maximport,
                         maximport,
                         xlt->first.first.c_str(),
                         maxexport,
                         maxexport,
                         xlt->first.second.c_str());
                Listi.push_back(buf);
            }
        }
        size_t n = MinV(Listi.size(), (MAXROW - 10));
