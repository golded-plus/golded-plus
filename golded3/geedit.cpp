    ...
    *_quotebuf = NUL;
    ...
    while(_thisline)
    {
        ...
        if(_thisline->txt.find('\n') != _thisline->txt.npos)
        {
            ...
            _wrapmargin = (_thisline->next->type &
       ...
       insert)
    {
        if(not isspace(__ch) and (col == mincol))
        {
            ...
            if(currline->prev and not currline->prev->txt.empty() and (currline->prev->
                                                                       ...
                                                                       currline->prev->
                                                                       txt.npos) and not
               isspace(currline->prev->txt[currline->prev->txt.length() - 1]))
            {
                if(not batch_mode)
                {
...
void IEclass::statusline()
{
    ...
    update_statuslinef(LNG->EditStatus,
                       "ST_EDITSTATUS",
                       1 + thisrow,
                       1 + col,
                       chr,
...
//  ------------------------------------------------------------------
UndoStack::UndoStack(IEclass * this_editor) : editor(this_editor), row(editor->row),
    col(editor->col), pcol(editor->pcol), prow(editor->prow), minrow(editor->minrow),
    maxrow(editor->maxrow), thisrow(editor->thisrow), currline(editor->currline),
...
void UndoStack::PlayItem()
{
    ...
    if(last_item)
    {
        ...
        if(curr_row_num != thisrow)
        {
            ...
            undo_ready = ((abs(int(curr_row_num - thisrow)) <
                           2) and ((curr_col_num == col) or (col + 1 >
                                                             currline->txt.length())));
        ...
        {
            undo_ready = ((abs(int(curr_col_num - col)) < 2) or (col + 1 >
                                                                 currline->txt.length()));
        }
            ...
            do
            {
                ...
                switch(undo_type)
                {
                    ...
                    case EDIT_UNDO_TEXT:
                    {
                        ...
                        editor->setlinetype((undo_action ==
                                             EDIT_UNDO_WRAP_TEXT) ? currline->next :
                                            currline);
                        break;
                    }
