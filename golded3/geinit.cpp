    ...
    if(fp.isopen())
    {
        ...
        while(fp.Fgets(buf, sizeof(buf)))
        {
            if(not strblank(buf))
            {
                ...
                strcpy(echoin[echonums - 1],
                       strtrim(strsetsz(buf, sizeof(Echo) - 1)));
            }
        }
...
static void ReadEscsets()
{
    ...
    for(int n = 0; it != CFG->xlatescsets.end(); ++it, ++n)
    {
        ...
        if(fp.isopen())
        {
            fp.FseekSet(((long)CFG->xlatcharsets.size() * (long)sizeof(Chs)) +
                        ((long)n * (long)sizeof(Esc)));
...
static void w_brag()
{
    ...
    else
    {
        ...
        wprints_box(1,
                    0,
                    C_BRAGT | ACSET,
                    "     ÉËÍÍË» ÉËÍÍË» ºº ÉËÍÍ¹º ÉËÍÍË» ÉËÍÍ¹º  Ë  ");
        wprints_box(2,
                    0,
                    C_BRAGT | ACSET,
                    "     ºº  ºº ºº  ºº ºº ºº  ºº ºÌÍÍÊ¼ ºº  ºº ÍÎÍ ");
        wprints_box(3,
                    0,
                    C_BRAGT | ACSET,
                    "     ÈÊÍÍ¹º ÈÊÍÍÊ¼ È¼ ÈÊÍÍÊ¼ ÈÊÍÍÊ¼ ÈÊÍÍÊ¼  Ê  ");
        wprints_box(4,
                    0,
                    C_BRAGT | ACSET,
                    " É»      ºº                                    ");
    ...
    wprints(4,
            46 - strlen(__gver_longpid__) - 1 - strlen(__gver_ver__),
            C_BRAGW,
            __gver_longpid__);
...
static bool ExistCfg(char * path, char * file)
{
    if(veryverbose)
    {
        STD_PRINT("Try config file \"" << path << file << "\"... ");
    }
    ...
    if(veryverbose)
    {
        STD_PRINTNL((found ? ("Found!") : ("not found.")));
    }
...
static bool FindCfg(char * path)
{
    ...
    if(!is_dir(path))
    {
        if(veryverbose)
        {
            STD_PRINT("Try config file \"" << path << "\"... ");
        }
    ...
    {
        STD_PRINTNL("Try config dir \"" << path << "\":");
    }
...
void Initialize(int argc, char * argv[])
{
    ...
    strxmerge(CFG->goldcfg, sizeof(Path), CFG->goldpath, CleanFilename(cmdlinecfg),
              NULL);
    ...
    if(cfgerrors)
    {
        STD_PRINTNL("* Total CFG errors found: " << cfgerrors <<
    ...
    if(CFG->disphdrnameset.pos < 0)
    {
        CFG->disphdrnameset.pos += MAXCOL;
    }

    if(CFG->disphdrnodeset.pos < 0)
    {
        CFG->disphdrnodeset.pos += MAXCOL;
    }

    if(CFG->disphdrdateset.pos < 0)
    {
        CFG->disphdrdateset.pos += MAXCOL;
    }

    if(CFG->disphdrnameset.len < 0)
    {
        CFG->disphdrnameset.len += MAXCOL;
    }

    if(CFG->disphdrnodeset.len < 0)
    {
        CFG->disphdrnodeset.len += MAXCOL;
    }

    if(CFG->disphdrdateset.len < 0)
    {
        CFG->disphdrdateset.len += MAXCOL;
    }

    if(EDIT->HdrNamePos() < 0)
    {
        EDIT->HdrNamePos(EDIT->HdrNamePos() + MAXCOL);
    }

    if(EDIT->HdrNameLen() < 0)
    {
        EDIT->HdrNameLen(EDIT->HdrNameLen() + MAXCOL);
    }

    if(EDIT->HdrNodePos() < 0)
    {
        EDIT->HdrNodePos(EDIT->HdrNodePos() + MAXCOL);
    }

    if(EDIT->HdrNodeLen() < 0)
    {
        EDIT->HdrNodeLen(EDIT->HdrNodeLen() + MAXCOL);
    }
    ...
    for(int pass = 1; pass <= 2; pass++)
    {
        if(pass == 2)
        {
            ...
            if(desc_width == -1)
            {
                desc_width = MAXCOL - 2 - spaces - area_width - marked_width -
    ...
    if(find(AL.basetypes, "OPUS") or find(AL.basetypes, "FTS1"))
    {
        ...
        FidoInit(CFG->fidolastread, CFG->switches.get(fidohwmarks),
                 CFG->switches.get(fidonullfix), CFG->fidouserno, CFG->squishuserpath);
    }
    ...
    if(find(AL.basetypes, "SQUISH"))
    {
        ...
        SquishInit(CFG->squishuserpath,
                   CFG->squishuserno,
                   CFG->switches.get(squishdirect),
                   true,
                   CFG->squishscan);
    }
