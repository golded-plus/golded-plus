...
void gfileselect::do_delayed()
{
    char buf[200];
    char tmp[80];
    gsprintf(PRINTF_DECLARE_BUFFER(buf), "%s %10s", LNG->SelectedFiles, longdotstr(tmp,
                                                                                   selfiles));
    wprints(ylen - 4, MAXCOL - 34, wattr, buf);
    gsprintf(PRINTF_DECLARE_BUFFER(buf), "%s %10s", LNG->SelectedBytes, longdotstr(tmp,
                                                                                   selbytes));
    wprints(ylen - 3, MAXCOL - 34, wattr, buf);
    gsprintf(PRINTF_DECLARE_BUFFER(buf), "%s %10s", LNG->TotalFiles, longdotstr(tmp,
                                                                                totfiles));
    wprints(ylen - 2, MAXCOL - 34, wattr, buf);
    gsprintf(PRINTF_DECLARE_BUFFER(buf), "%s %10s", LNG->TotalBytes, longdotstr(tmp,
                                                                                totbytes));
...
void FileSelect(GMsg * msg, char * title, FileSpec * fspec)
{
    ...
    const char * wildlistname = (isslash(fbuf[strlen(fbuf) -
                                              1]) or is_dir(fbuf)) ? "*" :
                                CleanFilename(fbuf);
    ...
    if(totfiles)
    {
        ...
        else
        {
            ...
            fspec->fblk = fblk = (FFblk *)throw_realloc(fblk,
                    ...
                    while(strchr(CFG__frqskipwordchars, *ptr) and * ptr)
                    {
                        if(not * filesize)
                        {
                            ...
                            if(p - ptr > 1 and not strchr("-/.", *p))
                            {
                                ...
                                if(strlen(p) >= 6 and p[2] == p[5] and strchr("-/.",
                                                                              p[2]))
                                {
                                    *filesize = NUL;
                                }
