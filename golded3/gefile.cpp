...
void gfileselect::do_delayed()
{
    ...
    gsprintf(PRINTF_DECLARE_BUFFER(buf), "%s %10s", LNG->SelectedFiles,
             longdotstr(tmp, selfiles));
    wprints(ylen - 4, MAXCOL - 34, wattr, buf);
    gsprintf(PRINTF_DECLARE_BUFFER(buf), "%s %10s", LNG->SelectedBytes,
             longdotstr(tmp, selbytes));
    wprints(ylen - 3, MAXCOL - 34, wattr, buf);
    gsprintf(PRINTF_DECLARE_BUFFER(buf), "%s %10s", LNG->TotalFiles,
             longdotstr(tmp, totfiles));
    wprints(ylen - 2, MAXCOL - 34, wattr, buf);
    gsprintf(PRINTF_DECLARE_BUFFER(buf), "%s %10s", LNG->TotalBytes,
             longdotstr(tmp, totbytes));
...
const char * gfileselect::gensize(uint32_t size)
{
    ...
    {
        size += 5242880;
        gsprintf(PRINTF_DECLARE_BUFFER(ret), "%3d.%02dG", (int)size / 1073741824,
                 KFIX((size % 1073741824) / 1024));
    }
    else if(size >= 1024000)
    {
        size += 5120;
        gsprintf(PRINTF_DECLARE_BUFFER(ret), "%3d.%02dM", (int)size / 1048576,
                 KFIX((size % 1048576) / 1024));
    }
    else if(size >= 1000)
    {
        size += 5;
        gsprintf(PRINTF_DECLARE_BUFFER(ret), "%3d.%02dk", (int)size / 1024,
                 KFIX(size % 1024));
    }
    else
    {
...
void FileSelect(GMsg * msg, char * title, FileSpec * fspec)
{
    ...
    const char * wildlistname =
        (isslash(fbuf[strlen(fbuf) - 1]) or is_dir(fbuf)) ? "*" : CleanFilename(fbuf);
    ...
    if(totfiles)
    {
        ...
        else
        {
            ...
            fspec->fblk = fblk =
                              (FFblk *)throw_realloc(fblk,
                                                     (selfiles + 1) * sizeof(FFblk));
    ...
    for(n = 0, x = 0; x < fspecs; x++)
    {
        for(m = 0; m < fspec[x].files; m++, n++)
        {
            if(specfiles > 1)
            {
                ...
                update_statuslinef(LNG_Fileing,
                                   tokenstr,
                                   n + 1,
                                   specfiles,
                                   msg->dest.zone,
            ...
            gsprintf(PRINTF_DECLARE_BUFFER(
                     ...
                     fspec[x].delsent ? "^" : "",
                     fspec[x].path,
                     fspec[x].fblk ? (fspec[x].fblk[m].name ? fspec[x].fblk[m].name : "") : "",
                     *fspec[x].password ? " " : "",
                     fspec[x].password);
        ...
        for(size_t n = tline; n < msg->lines; n++)
        {
            ...
            if(not * file)
            {
                ...
                while(ptr2)
                {
                    ...
                    {
                        if(strnicmpw(e->c_str(), ptr2,
                                     MinV(e->length(), strlen(ptr2))) == 0)
                        {
            ...
            if(not esc)
            {
                ...
                for( ; it != end; it++)
                {
                    if((*it)[0] != ' ')
                    {
                        ...
                        if((strlen(msg->re) + strlen(tmpbuf)) < sizeof(ISub)) // We can only
                                                                              // fill one
                                                                              // subject line in
                                                                              // this version...
                        {
                ...
                if(freqs == 0)
                {
                    ...
                    if((strlen(msg->re) + strlen(tmpbuf)) < sizeof(ISub)) // We can only fill
                                                                          // one subject line in
                                                                          // this version...
                    {
                        strcat(msg->re, tmpbuf);
