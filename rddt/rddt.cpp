...
int main(int argc, char ** argv)
{
    ...
    // set locale
    setlocale(LC_CTYPE, "");
    cout << __gver_name__ << " v." << __gver_preversion__ << __gver_shortver__ <<
    __gver_platform__ << __gver_postversion__;
#ifdef KOI8
    cout << " (koi8)";
#endif
    cout << endl << "Copyright (C) 1999 Odinn Sorensen" << endl <<
    ...
    if(argc < 2)
    {
        ...
        "\twhere options may be:" << endl << "\t\t-d             - Debug mode" << endl <<
        "\t\t-p             - Decode path" << endl <<
        "\t\t-n ownaddress  - Your own address" << endl <<
        "\t\t-l ownuplink   - Your own uplink" << endl <<
        "\t\t-i num         - indent <num> spaces" << endl << endl;
        return 1;
    }
    ...
    if(fp)
    {
        ...
        while(not fp.eof())
        {
            ...
            if((*buf != ';') and (*buf != '/'))
            {
                ...
                if(links.size() > 1)
                {
                    ...
                    strchg(&links[0][0], '_', ' ');
                    anode.name    = links[0];
                    atree.maxname = maximum_of_two(anode.name.length(),
                                                   (size_t)atree.maxname);
        ...
        if(address.valid())
        {
            if(debug)
            {
                ...
                for(list_node::iterator n = atree.nodes.begin(); n != atree.nodes.end();
                    n++)
                {
                    xnode = *n;
